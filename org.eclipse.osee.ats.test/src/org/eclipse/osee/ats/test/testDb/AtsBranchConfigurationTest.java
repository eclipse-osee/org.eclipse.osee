...
public class AtsBranchConfigurationTest extends TestCase {
   ...
   public AtsBranchConfigurationTest(XNavigateItem parent) {
   ...
   public void run(boolean prompt) throws Exception {
      try {
         ...
         if (prompt && AtsPlugin.isProductionDb()) throw new IllegalStateException(
         ...
         if (SkynetDbInit.isDbInit() || (!SkynetDbInit.isDbInit() && (!prompt || (prompt && MessageDialog.openConfirm(
            ...
            rd.log(getName() + "\n\n");
            try {
               testBranchViaVersions();
               rd.log("\n\n");
               testBranchViaTeamDefinition();
            } catch (Exception ex) {
   ...
   private void testBranchViaVersions() throws Exception {
   ...
   private void testBranchViaTeamDefinition() throws Exception {
