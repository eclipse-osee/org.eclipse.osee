...
public class ConflictTestManager {
      ...
      for (int i = 0; i < NUMBER_OF_ARTIFACTS; i++) {
         ...
         if (!conflictDefs[i].destDelete && !conflictDefs[i].sourceDelete) {
            for (AttributeValue value : conflictDefs[i].values) {
               ...
               if (!expected.equals(destArtifacts[i].getSoleAttributeValueAsString(value.attributeName, " "))) {
                  System.err.println("Expected the " + value.attributeName + " attribute to have a value of " + expected + " but got " + destArtifacts[i].getSoleAttributeValueAsString(
   ...
   public static void createConflictDefinitions() {
      ...
      conflictDefs[2].values.add(new AttributeValue("Page Type", "Landscape", "Portrait", "Source",
            StringAttribute.class));
