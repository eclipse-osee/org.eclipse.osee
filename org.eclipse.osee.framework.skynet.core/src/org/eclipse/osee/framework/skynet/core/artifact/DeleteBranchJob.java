...
class DeleteBranchJob extends Job {
   private static final String COUNT_CHILD_BRANCHES = "SELECT " + Table.alias("count(branch_id)", "child_branches") + " FROM " + BRANCH_TABLE + " WHERE " + BRANCH_TABLE.column("parent_branch_id") + "=?";
   ...
   private static final String SEARCH_FOR_DELETABLE_GAMMAS = " SELECT ?, gamma_id FROM " + T1 + ", " + T2 + " WHERE " + T1.column("branch_id") + " = ? AND " + T1.column("transaction_id") + " = " + T2.column("transaction_id") + " AND " + T1.column("transaction_id") + " <> (SELECT MIN(transaction_id) FROM " + TRANSACTION_DETAIL_TABLE + " WHERE branch_id = ?) and NOT EXISTS (SELECT 'not_matter' FROM " + TRANSACTION_DETAIL_TABLE + ", " + TRANSACTIONS_TABLE + " WHERE " + T2.column("gamma_id") + " = " + TRANSACTIONS_TABLE.column("gamma_id") + " AND " + TRANSACTION_DETAIL_TABLE.column("transaction_id") + " = " + TRANSACTIONS_TABLE.column("transaction_id") + " AND " + T1.column("transaction_id") + " <> " + TRANSACTION_DETAIL_TABLE.column("transaction_id") + ")";
   private static final String POPULATE_BRANCH_DELETE_HELPER_WITH_GAMMAS = " INSERT INTO " + SkynetDatabase.BRANCH_DELETE_HELPER + "(branch_id, gamma_id) " + SEARCH_FOR_DELETABLE_GAMMAS;
   private static final LocalAliasTable T2_BDH = new LocalAliasTable(SkynetDatabase.BRANCH_DELETE_HELPER, "t2");
   private static final String DELETE_GAMMAS_RIGHT_HAND_SIDE = " where exists ( select 'x' from " + T2_BDH + " where t2.branch_id = ? and t2.gamma_id = ";
   private static final String DELETE_ATTRIBUTE_VERSIONS = "delete from " + ATTRIBUTE_VERSION_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + ATTRIBUTE_VERSION_TABLE.column("gamma_id") + ")";
   private static final String DELETE_RELATION_VERSIONS = "delete from " + RELATION_LINK_VERSION_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + RELATION_LINK_VERSION_TABLE.column("gamma_id") + ")";
   private static final String DELETE_ARTIFACT_VERSIONS = "delete from " + ARTIFACT_VERSION_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + ARTIFACT_VERSION_TABLE.column("gamma_id") + ")";
   private static final String DELETE_ARTIFACT_TYPE = "delete from " + ARTIFACT_TYPE_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + ARTIFACT_TYPE_TABLE.column("gamma_id") + ")";
   private static final String DELETE_ATTRIBUTE_TYPE = "delete from " + ATTRIBUTE_TYPE_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + ATTRIBUTE_TYPE_TABLE.column("gamma_id") + ")";
   private static final String DELETE_RELATION_TYPE = "delete from " + RELATION_LINK_TYPE_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + RELATION_LINK_TYPE_TABLE.column("gamma_id") + ")";
   private static final String DELETE_VLAID_RELATION = "delete from " + VALID_RELATIONS_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + VALID_RELATIONS_TABLE.column("gamma_id") + ")";
   private static final String DELETE_VALID_ATTRIBUTE = "delete from " + VALID_ATTRIBUTES_TABLE + DELETE_GAMMAS_RIGHT_HAND_SIDE + VALID_ATTRIBUTES_TABLE.column("gamma_id") + ")";
   private static final String DELETE_FROM_BRANCH_TABLE = "DELETE FROM " + BRANCH_TABLE + " WHERE " + BRANCH_TABLE.column("branch_id") + " = ?";
   ...
   private final class DeleteBranchTx extends AbstractDbTxTemplate {
      ...
      protected void handleTxWork() throws Exception {
         ...
         chStmt = ConnectionHandler.runPreparedQuery(SEARCH_FOR_DELETABLE_GAMMAS, SQL3DataType.INTEGER,
