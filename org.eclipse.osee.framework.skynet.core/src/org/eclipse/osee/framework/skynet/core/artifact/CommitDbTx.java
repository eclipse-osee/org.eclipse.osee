...
public class CommitDbTx extends DbTransaction {
   ...
   //More robust but will revert artifacts that aren't on parent and may cause problems with multiple commits.
         // "SELECT atr1.attr_id, atr1.art_id, td1.branch_id FROM osee_tx_details td1, osee_txs tx1,  osee_attribute atr1 WHERE td1.branch_id = 524 AND td1.transaction_id = tx1.transaction_id AND tx1.gamma_id = atr1.gamma_id AND tx1.tx_current in ( " + TxChange.DELETED.getValue() + " , " + TxChange.ARTIFACT_DELETED.getValue() + " ) AND NOT EXISTS (SELECT 'x' FROM osee_txs tx2, osee_tx_details td2, osee_attribute atr2 WHERE atr2.attr_id = atr1.attr_id AND atr2.gamma_id = tx2.gamma_id AND tx2.transaction_id = td2.transaction_id AND td2.branch_id = ?)"
         "SELECT atr1.attr_id, atr1.art_id, td1.branch_id FROM osee_txs tx1, osee_txs tx2, osee_tx_details td1, osee_tx_details td2, osee_attribute atr1, osee_attribute atr2 WHERE td1.branch_id = ? AND td1.tx_type = " + TransactionDetailsType.NonBaselined.getId() + " AND td1.transaction_id = tx1.transaction_id AND tx1.gamma_id = atr1.gamma_id AND td2.branch_id = ? AND td2.tx_type = " + TransactionDetailsType.NonBaselined.getId() + " AND td2.transaction_id = tx2.transaction_id AND tx2.tx_current in ( " + TxChange.DELETED.getValue() + " , " + TxChange.ARTIFACT_DELETED.getValue() + " ) AND tx2.gamma_id = atr2.gamma_id AND atr1.attr_id = atr2.attr_id";
