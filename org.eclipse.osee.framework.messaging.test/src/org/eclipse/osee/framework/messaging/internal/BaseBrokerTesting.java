...
public class BaseBrokerTesting {
	...
	protected void stopBroker() {
		try {
			getMessaging().get(DefaultNodeInfos.OSEE_JMS_DEFAULT).send(SystemTopic.KILL_TEST_JMS_BROKER,
...
//			fail(ex.getMessage());
	...
	protected void testJMSSendShouldFail(MessageService messaging) throws OseeCoreException {
	...
	protected void testJMSSendShouldPass(MessageService messaging) throws OseeCoreException {
	...
	protected void testJMSSubscribeShouldFail(MessageService messaging) throws OseeCoreException {
		MessageStatusTest status = new MessageStatusTest(false);
		messaging.get(DefaultNodeInfos.OSEE_JMS_DEFAULT).subscribe(TestMessages.test2,
