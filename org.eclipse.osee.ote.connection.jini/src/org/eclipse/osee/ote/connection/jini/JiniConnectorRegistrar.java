...
public class JiniConnectorRegistrar implements IJiniConnectorRegistrar, IConnectorListener, ServiceDiscoveryListener, DiscoveryListener {
    ...
    public JiniConnectorRegistrar(ClassLoader loader,
	    IConnectionService connectionService) throws Exception,
	...
	try {
	    ...
	    lookupDiscoveryManager = new LookupDiscoveryManager(groups, null,
		    this, config);
	    serviceDiscoveryManager = new ServiceDiscoveryManager(
		    lookupDiscoveryManager, null, config);

	    lookupCache = serviceDiscoveryManager
		    .createLookupCache(new ServiceTemplate(null,
			    new Class[] {}, null), null, this);
	} finally {
     ...
     * (non-Javadoc)
     * 
     * @seeorg.eclipse.osee.connection.service.IConnectorListener#
     * onConnectionServiceStopped()
     */
    @Override
     ...
     * 
     * @see
     * org.eclipse.osee.connection.service.IConnectorListener#onConnectorAdded
	...
	for (IServiceConnector connector : connectors) {
		...
		for (ServiceRegistrar registrar : list) {
			...
			final ServiceRegistration registration = registrar
				.register(jiniConnector.getServiceItem(),
					Long.MAX_VALUE);
	...
	if (!isShutdown) {
	    ...
	    for (JiniServiceSideConnector connector : serverSideConnectors
		    .keySet()) {
		try {
		    connector.removeAllRegistrations();
		} catch (Exception ex) {
		    Activator.log(Level.WARNING,
			    "Error removing registrations on shutdown", ex);
		}
	    }
		...
		for (JiniServiceSideConnector connector : serverSideConnectors
			...
			Activator.log(Level.WARNING,
				"Error removing connectors on shutdown", ex);
		    }
		}
     ...
     * 
     * @see
     * org.eclipse.osee.connection.service.IConnectorListener#onConnectorRemoved
     * (org.eclipse.osee.connection.service.IServiceConnector)
     */
    @Override
	...
	} else if (connector.getConnectorType().equals(
		JiniClientSideConnector.TYPE)) {
	    Iterator<Entry<ServiceID, JiniClientSideConnector>> iter = clientSideConnectors
		    .entrySet().iterator();
	    while (iter.hasNext()) {
		final Entry<ServiceID, JiniClientSideConnector> entry = iter
			.next();
	...
	try {
		...
		System.out.println("Lookup Discovered: Service ID= "
			+ registrar.getServiceID());
		try {
		    for (String group : registrar.getGroups()) {
			Activator.log(Level.INFO,
				"found JINI registrar group: " + group);
		    }
		} catch (RemoteException ex) {
		    ex.printStackTrace();
		}
		serviceRegistrars.add(registrar);
		for (Map.Entry<JiniServiceSideConnector, HashSet<ServiceRegistrar>> entry : serverSideConnectors
			.entrySet()) {
		    final ServiceRegistration registration = registrar
			    .register(entry.getKey().getServiceItem(),
				    Long.MAX_VALUE);
	    ...
	    Activator.log(Level.SEVERE, "Error processing service discovery",
		    ex);
	}
    }
     ...
     * 
     * @see
     * net.jini.lookup.ServiceDiscoveryListener#serviceAdded(net.jini.lookup
	...
	JiniClientSideConnector connector = new JiniClientSideConnector(
		serviceItem);
	...
	JiniClientSideConnector connector = clientSideConnectors.remove(event
		.getPreEventServiceItem().serviceID);
	...
	if (connector != null) {
		...
		Activator.log(Level.SEVERE,
			"error processing removed service event", e);
	    }
	}
    }
     ...
     * 
     * @see
     * net.jini.lookup.ServiceDiscoveryListener#serviceChanged(net.jini.lookup
     * .ServiceDiscoveryEvent)
     */
	...
	String serviceGroup = System
		.getProperty(OseeProperties.OSEE_JINI_SERVICE_GROUPS);
    ...
    @Override
    public void addLocators(String... hosts) throws MalformedURLException,
