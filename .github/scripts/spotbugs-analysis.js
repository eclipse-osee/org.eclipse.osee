...
function parseBugs(file) {
  ...
  for (let i = 1; i < chunks.length; i++) {
    ...
    const b =
      "<BugInstance " +
      chunks[i].substring(0, endIdx + "</BugInstance>".length);
...
function ghPost(url, body) {
  ...
  if (parseInt(httpCode, 10) >= 400) {
    ...
    console.warn(
      `GitHub API error (${httpCode}): ${responseBody.substring(0, 500)}`,
    );
    throw err;
  }
...
${bugTable(fileBugs)}
  ...
  const fileNav = Object.keys(byFile)
    ...
    .map(
      (file) =>
        `<span class="nav-item">${escapeHtml(file.split("/").pop())} (${byFile[file].length})</span>`,
    )
    .join("\n");
...
function generateFullHtml(bugs, version) {
  ...
  for (const bug of bugs) {
    ...
    const pkg = sp.includes("/")
      ? sp.substring(0, sp.lastIndexOf("/")).replace(/\//g, ".")
      : "(default)";
...
${bugTable(pkgBugs, false)}
  ...
  const categoryNav = Object.keys(byCategory)
    ...
    .map(
      (cat) =>
        `<a class="nav-item" href="#cat-${escapeHtml(cat)}">${escapeHtml(cat)} (${byCategory[cat].length})</a>`,
    )
    .join("\n");
  ...
  const packageNav = Object.keys(byPackage)
    ...
    .map(
      (pkg) =>
        `<a class="nav-item" href="#pkg-${escapeHtml(pkg)}">${escapeHtml(pkg)} (${byPackage[pkg].length})</a>`,
    )
    .join("\n");
...
function main() {
  ...
  try {
    const vOut = execSync(`${spotbugsBin} -version`, {
      encoding: "utf8",
    }).trim();
  ...
  console.log(
    `Generated spotbugs-report-full.html with ${allBugs.length} issue(s)`,
  );
  ...
  console.log(
    `Filtered bugs: ${filteredBugs.length} of ${allBugs.length} are in changed files`,
  );

  // --- Generate HTML report for changed files (no extra SpotBugs run) ---
  const changedHtml = generateChangedHtml(
    filteredBugs,
    srcToRepo,
    spotbugsVersion,
  );
  ...
  console.log(
    `Generated spotbugs-report-changed.html with ${filteredBugs.length} issue(s)`,
  );
  ...
  if (filteredBugs.length > 0) {
    ...
    lines.push(`> :white_check_mark: No issues in files changed by this PR.`);
    lines.push(``);
  }
  ...
  const isSpotbugsComment = (body) =>
    body &&
    (body.includes(SPOTBUGS_MARKER) ||
      body.includes(":beetle: **SpotBugs**") ||
      body.includes(":beetle: SpotBugs"));
      ...
      for (const c of existingComments) {
        if (isSpotbugsComment(c.body)) {
          try {
            ...
            console.warn(
              `  Could not delete old comment ${c.id}: ${err.message}`,
            );
          }
        }
      }
  ...
  for (const [filePath, bugs] of Object.entries(bugsByFile)) {
    ...
    for (let i = 0; i < bugs.length; i++) {
      const bug = bugs[i];
      const docsUrl = `https://spotbugs.readthedocs.io/en/stable/bugDescriptions.html#${bug.type.toLowerCase()}`;
      const method = bug.methodName ? `\`${bug.methodName}()\`` : "_unknown_";
      const desc =
        bug.longMsg && bug.longMsg !== bug.message ? bug.longMsg : bug.message;
  ...
  try {
    ...
    console.warn(
      `Could not fetch current PR head SHA, using event payload: ${e.message}`,
    );
  }
