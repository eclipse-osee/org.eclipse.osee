...
CREATE TABLE osee.OSEE_RELATION(REL_TYPE BIGINT NOT NULL, A_ART_ID BIGINT NOT NULL, B_ART_ID BIGINT NOT NULL, REL_ART_ID BIGINT NOT NULL, REL_ORDER INTEGER NOT NULL, GAMMA_ID BIGINT NOT NULL,CONSTRAINT osee_relation_pk PRIMARY KEY (REL_TYPE, A_ART_ID, B_ART_ID, REL_ART_ID, REL_ORDER));
CREATE INDEX OSEE_RELATION__B2_IDX ON osee.OSEE_RELATION USING btree(B_ART_ID ASC NULLS LAST);
CREATE INDEX OSEE_RELATION__G_IDX ON osee.OSEE_RELATION USING btree(gamma_id ASC NULLS LAST);
...
CREATE INDEX OSEE_ATTR_FTS_IDX ON osee.OSEE_ATTRIBUTE USING GIN(to_tsvector('english', coalesce(VALUE, '')));
...
CREATE TABLE osee.OSEE_JOIN_ID(ID BIGINT,QUERY_ID BIGINT NOT NULL);
CREATE INDEX OSEE_JOIN_ID__Q_I_IDX ON osee.OSEE_JOIN_ID(QUERY_ID,ID);
CREATE TABLE osee.OSEE_JOIN_CLEANUP(TABLE_NAME VARCHAR(28) NOT NULL,EXPIRES_IN BIGINT NOT NULL,ISSUED_AT BIGINT NOT NULL,QUERY_ID BIGINT NOT NULL,CONSTRAINT OSEE_JOIN_CLEANUP__PK PRIMARY KEY(QUERY_ID));
...
CREATE TABLE osee.OSEE_VALIDATE_GAMMA_ID(GAMMA_ID BIGINT NOT NULL, CONSTRAINT OSEE_VALIDATE_GAMMA_ID_PK PRIMARY KEY(GAMMA_ID));
...
CREATE TABLE osee.OSEE_ACTIVITY(ENTRY_ID BIGINT NOT NULL,ACCOUNT_ID BIGINT NOT NULL,CLIENT_ID BIGINT NOT NULL,TYPE_ID BIGINT NOT NULL,SERVER_ID BIGINT NOT NULL,MSG_ARGS VARCHAR(4000),START_TIMESTAMP TIMESTAMP NOT NULL,STATUS INTEGER NOT NULL,PARENT_ID BIGINT NOT NULL,DURATION BIGINT NOT NULL,CONSTRAINT OSEE_ACTIVITY__ENTRY_ID_PK PRIMARY KEY(ENTRY_ID));
...
CREATE TABLE osee.OSEE_BRANCH_CATEGORY(BRANCH_ID BIGINT NOT NULL, CATEGORY BIGINT NOT NULL, GAMMA_ID BIGINT NOT NULL,CONSTRAINT osee_branch_category_pk PRIMARY KEY (category, branch_id));
CREATE INDEX OSEE_BRANCH_CATEGORY__G_IDX ON osee.OSEE_BRANCH_CATEGORY(GAMMA_ID);
...
CREATE TABLE osee.OSEE_ATTRIBUTE_SOURCE(GAMMA_ID BIGINT NOT NULL, ART_ID BIGINT NOT NULL, TYPE_ID BIGINT NOT NULL, ATTR_ORDER BIGINT NOT NULL, STRING_VALUE VARCHAR(4000) NOT NULL, NUMBER_VALUE BIGINT NOT NULL, CONSTRAINT OSEE_ATTRIBUTE_SOURCE_PK PRIMARY KEY(TYPE_ID, ART_ID, ATTR_ORDER));
CREATE TABLE osee.OSEE_ATTRIBUTE_BINARY(GAMMA_ID BIGINT NOT NULL, VALUE BYTEA NOT NULL, CONSTRAINT OSEE_ATTRIBUTE_BINARY_PK  PRIMARY KEY (GAMMA_ID));
CREATE TABLE osee.OSEE_API_KEY(KEY_UID BIGINT GENERATED ALWAYS AS IDENTITY, KEY_NAME VARCHAR(20) NOT NULL, API_KEY_VALUE VARCHAR(4000) NOT NULL, SCOPES VARCHAR(4000) NOT NULL, CREATION_DATE TIMESTAMP NOT NULL, EXPIRATION_DATE TIMESTAMP NOT NULL, USER_ART_ID BIGINT NOT NULL, CONSTRAINT OSEE_API_KEY_PK PRIMARY KEY(KEY_UID));
CREATE INDEX OSEE_API_KEY__API_KEY_NAME_IDX ON osee.OSEE_API_KEY(KEY_NAME);
CREATE INDEX API_KEYS_API_KEY_VALUE_IDX ON osee.OSEE_API_KEY(API_KEY_VALUE);
CREATE INDEX OSEE_API_KEY__EXPIRATION_DATE_IDX ON osee.OSEE_API_KEY(EXPIRATION_DATE);
CREATE INDEX OSEE_API_KEY__ARTIFACT_ID_IDX ON osee.OSEE_API_KEY(USER_ART_ID);