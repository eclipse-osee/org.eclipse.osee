...
public enum OseeSql {
   ...
   TX_GET_ALL_TRANSACTIONS("SELECT * FROM osee_tx_details WHERE transaction_id = ?", Strings.HINTS__ORDERED__FIRST_ROWS),
   TX_GET_MAX_AS_LARGEST_TX(
         "SELECT max(transaction_id) as largest_transaction_id FROM osee_tx_details WHERE branch_id = ?",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   TX_GET_TX_GAMMAS(
   ...
   TX_GET_PREVIOUS_TX_NOT_CURRENT_ARTIFACTS(
         "SELECT txs1.transaction_id, txs1.gamma_id FROM osee_artifact_version arv1, osee_txs txs1, osee_tx_details txd1 WHERE arv1.art_id = ? AND arv1.gamma_id = txs1.gamma_id AND txs1.transaction_id = txd1.transaction_id AND txd1.branch_id = ? AND txs1.tx_current <> " + TxChange.NOT_CURRENT.getValue(),
         Strings.HINTS__ORDERED__FIRST_ROWS),
   TX_GET_PREVIOUS_TX_NOT_CURRENT_ATTRIBUTES(
         "SELECT txs1.transaction_id, txs1.gamma_id FROM osee_attribute atr1, osee_txs txs1, osee_tx_details txd1 WHERE atr1.attr_id = ? AND atr1.gamma_id = txs1.gamma_id AND txs1.transaction_id = txd1.transaction_id AND txd1.branch_id = ? AND txs1.tx_current <> " + TxChange.NOT_CURRENT.getValue(),
         Strings.HINTS__ORDERED__FIRST_ROWS),
   TX_GET_PREVIOUS_TX_NOT_CURRENT_RELATIONS(
         "SELECT txs1.transaction_id, txs1.gamma_id FROM osee_relation_link rel1, osee_txs txs1, osee_tx_details txd1 WHERE rel1.rel_link_id = ? AND rel1.gamma_id = txs1.gamma_id AND txs1.transaction_id = txd1.transaction_id AND txd1.branch_id = ? AND txs1.tx_current <> " + TxChange.NOT_CURRENT.getValue(),
         Strings.HINTS__ORDERED__FIRST_ROWS),

   MERGE_GET_ARTIFACTS_FOR_BRANCH(
   ...
   MERGE_GET_ATTRIBUTES_FOR_BRANCH(
         ...
         Strings.HINTS__ORDERED__FIRST_ROWS),
   MERGE_GET_RELATIONS_FOR_BRANCH(
   ...
   CONFLICT_GET_ARTIFACTS(
         ...
         Strings.HINTS__ORDERED__INDEX__ARTIFACT_CONFLICT),
   CONFLICT_GET_ATTRIBUTES(
         ...
         Strings.HINTS__ORDERED__INDEX__ATTRIBUTE_CONFLICT),
   CONFLICT_GET_HISTORICAL_ATTRIBUTES(
   ...
   LOAD_HISTORICAL_ATTRIBUTES(
         ...
         Strings.HINTS__NO_INDEX),
   LOAD_CURRENT_ATTRIBUTES(
         Strings.SELECT_CURRENT_ATTRIBUTES_PREFIX + "= 1 order by al1.branch_id, al1.art_id, att1.attr_id, txd1.transaction_id desc",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   LOAD_CURRENT_ATTRIBUTES_WITH_DELETED(
         Strings.SELECT_CURRENT_ATTRIBUTES_PREFIX + "IN (1, 3) order by al1.branch_id, al1.art_id, att1.attr_id, txd1.transaction_id desc",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   LOAD_ALL_CURRENT_ATTRIBUTES(
         Strings.SELECT_CURRENT_ATTRIBUTES_PREFIX + "IN (1, 2, 3) order by al1.branch_id, al1.art_id, att1.attr_id, txd1.transaction_id desc",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   LOAD_RELATIONS(
         "SELECT%s txs1.mod_type, rel_link_id, a_art_id, b_art_id, rel_link_type_id, a_order, b_order, rel1.gamma_id, rationale, al1.branch_id FROM osee_join_artifact al1, osee_relation_link rel1, osee_txs txs1, osee_tx_details txd1 WHERE al1.query_id = ? AND (al1.art_id = rel1.a_art_id OR al1.art_id = rel1.b_art_id) AND rel1.gamma_id = txs1.gamma_id AND txs1.tx_current = " + TxChange.CURRENT.getValue() + " AND txs1.transaction_id = txd1.transaction_id AND txd1.branch_id = al1.branch_id",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   LOAD_CURRENT_ARTIFACTS(Strings.SELECT_CURRENT_ARTIFACTS_PREFIX + "= 1", Strings.HINTS__ORDERED__FIRST_ROWS),
   LOAD_CURRENT_ARTIFACTS_WITH_DELETED(
         Strings.SELECT_CURRENT_ARTIFACTS_PREFIX + "in (1, 2)",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   CHANGE_BRANCH_ATTRIBUTE_WAS(
   ...
   CHANGE_TX_ATTRIBUTE_IS_PREFIX(
         ...
         Strings.HINTS__ORDERED__FIRST_ROWS),
   CHANGE_TX_ATTRIBUTE_IS(
         CHANGE_TX_ATTRIBUTE_IS_PREFIX.sql + " AND txd2.tx_type = 0",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   CHANGE_TX_ATTRIBUTE_IS_FOR_SPECIFIC_ARTIFACT(
         CHANGE_TX_ATTRIBUTE_IS_PREFIX.sql + " and attr1.art_id =?",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   ...
   CHANGE_TX_RELATION_FOR_SPECIFIC_ARTIFACT(
         CHANGE_TX_RELATION_PREFIX.sql + " and (rel1.a_art_id =? or rel1.b_art_id=?)",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   ...
   CHANGE_TX_ARTIFACT_PREFIX(
         ...
         Strings.HINTS__ORDERED__FIRST_ROWS),
   CHANGE_TX_ARTIFACT(
         CHANGE_TX_ARTIFACT_PREFIX.sql + " AND txd1.tx_type = " + TransactionDetailsType.NonBaselined.getId(),
         Strings.HINTS__ORDERED__FIRST_ROWS),
   CHANGE_TX_ARTIFACT_FOR_SPECIFIC_ARTIFACT(
         CHANGE_TX_ARTIFACT_PREFIX.sql + " and art1.art_id =?",
         Strings.HINTS__ORDERED__FIRST_ROWS),
   CHANGE_TX_MODIFYING(
   ...
   private static class Strings {
      ...
      private static final String HINTS__NO_INDEX = "/*+ NO_INDEX(txs1 OSEE_TXS_TX_IDX) */";
