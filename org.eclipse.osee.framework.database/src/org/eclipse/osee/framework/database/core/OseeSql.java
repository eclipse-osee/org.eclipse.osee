...
public enum OseeSql {
   ...
   TX_GET_MAX_AS_LARGEST_TX("SELECT max(transaction_id) as largest_transaction_id FROM osee_tx_details WHERE branch_id = ?", Strings.HINTS__ORDERED__FIRST_ROWS),
   TX_GET_TX_GAMMAS("SELECT txs.transaction_id, txs.gamma_id FROM osee_tx_details txd, osee_txs txs WHERE txd.transaction_id = txs.transaction_id AND txd.branch_id = ? AND txd.transaction_id = ? ORDER BY txs.transaction_id, txs.gamma_id", Strings.HINTS__ORDERED__FIRST_ROWS),
   TX_GET_TX_GAMMAS_RANGE("SELECT txs.transaction_id, txs.gamma_id FROM osee_tx_details txd, osee_txs txs WHERE txd.transaction_id = txs.transaction_id AND txd.branch_id = ? AND txd.transaction_id > ? AND txd.transaction_id <= ? ORDER BY txs.transaction_id, txs.gamma_id", Strings.HINTS__ORDERED__FIRST_ROWS),
   TX_GET_PREVIOUS_TX_NOT_CURRENT_ARTIFACTS("SELECT txs1.transaction_id, txs1.gamma_id FROM osee_artifact_version arv1, osee_txs txs1, osee_tx_details txd1 WHERE arv1.art_id = ? AND arv1.gamma_id = txs1.gamma_id AND txs1.transaction_id = txd1.transaction_id AND txd1.branch_id = ? AND txs1.tx_current <> " + TxChange.NOT_CURRENT.getValue(), Strings.HINTS__ORDERED__FIRST_ROWS),
   ...
   CHANGE_TX_ATTRIBUTE_IS(CHANGE_TX_ATTRIBUTE_IS_PREFIX.sql + " AND txd2.tx_type = 0", Strings.HINTS__ORDERED__FIRST_ROWS),
   CHANGE_TX_ATTRIBUTE_IS_FOR_SPECIFIC_ARTIFACT(CHANGE_TX_ATTRIBUTE_IS_PREFIX.sql + " and attr1.art_id =?", Strings.HINTS__ORDERED__FIRST_ROWS),
