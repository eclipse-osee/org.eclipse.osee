...
public class RelLinkTxCurrent extends DatabaseHealthTask {
      ...
      if (operation.equals(Operation.Verify)) {
                  ...
                  if (showDetails) {
                     String str =
                           AHTML.addRowMultiColumnTable(new String[] {String.valueOf(count),
                                 chStmt1.getString("rel_link_id"), chStmt1.getString("branch_id")});
                     sbFull.append(str);
                  }
               }
            } finally {
         ...
         try {
            ...
            try {
               ...
               while (chStmt1.next()) {
                  count++;
                  ConnectionHandler.runPreparedUpdate(NO_TX_CURRENT_CLEANUP, chStmt1.getInt("rel_link_id"),
                        chStmt1.getInt("branch_id"), chStmt1.getInt("rel_link_id"));
                  if (showDetails) {
                     chStmt2 =
                           ConnectionHandler.runPreparedQuery(QUERY_TX_CURRENT_SET, chStmt1.getInt("branch_id"),
                                 chStmt1.getInt("rel_link_id"));
                     String trans_id = "Not Found", gamma_id = "Not Found";
                     if (chStmt2.next()) {
                        trans_id = chStmt2.getString("transaction_id");
                        gamma_id = chStmt2.getString("gamma_id");
                     }
                     sbFull.append(AHTML.addRowMultiColumnTable(new String[] {chStmt1.getString("rel_link_id"),
                           gamma_id, trans_id, chStmt1.getString("branch_id")}));
   ...
   protected void showTxCurrentText(ConnectionHandlerStatement chStmt, int x, StringBuffer builder, int transaction_id) throws OseeDataStoreException {
      builder.append(AHTML.addRowMultiColumnTable(new String[] {chStmt.getString("rel_link_id"),
