 ...
 */
public class ChangeReportView extends ViewPart implements IActionable,
		IEventReceiver {
	public static final String VIEW_ID = "org.eclipse.osee.framework.ui.skynet.changeReport.ChangeReportView";
	private static final String INPUT = "input";

	private static final ArtifactClipboard artifactClipboard = new ArtifactClipboard(
			VIEW_ID);
	private static final AccessControlManager accessManager = AccessControlManager
			.getInstance();
	private static final ArtifactPersistenceManager artifactManager = ArtifactPersistenceManager
			.getInstance();
	private static final BranchPersistenceManager branchPersistenceManager = BranchPersistenceManager
			.getInstance();
	private static final SkynetEventManager eventManager = SkynetEventManager
			.getInstance();
	private static final String[] columnNames = { "", "Name", "", "" };
	private static final RevisionManager revisionManager = RevisionManager
			.getInstance();
	...
	public void createPartControl(Composite parent) {
		...
		try {
			attributesAction = new ShowAttributeAction(changeTable,
					SkynetGuiPlugin.CHANGE_REPORT_ATTRIBUTES_PREF);
			attributesAction
					.addToView(
							this,
							SkynetViews
									.loadAttrTypesFromPreferenceStore(SkynetGuiPlugin.CHANGE_REPORT_ATTRIBUTES_PREF));
		...
		changeTable
				.addSelectionChangedListener(new SelectionCountChangeListener(
						getViewSite()));
	...
	IGlobalMenuHelper globalMenuHelper = new IGlobalMenuHelper() {
		...
		public Collection<GlobalMenuItem> getValidMenuItems() {
			return Arrays
					.asList(new GlobalMenuItem[] { GlobalMenuItem.TagArtifacts });
		}

	};

	private void setHelpContexts() {
		SkynetGuiPlugin.getInstance().setHelp(changeTable.getControl(),
				"change_report_table");
	}
	...
	private void createActions() {
			...
			public void run() {
				...
				if (input != null) {
					try {
						...
						((BranchContentProvider) changeTable
								.getContentProvider()).refresh(true);
						changeTable.refresh();
		...
		refreshAction.setImageDescriptor(SkynetGuiPlugin.getInstance()
				.getImageDescriptor("refresh.gif"));
		refreshAction.setToolTipText("Refresh");
		IToolBarManager toolbarManager = getViewSite().getActionBars()
				.getToolBarManager();
		toolbarManager.add(refreshAction);
		OseeAts.addBugToViewToolbar(this, this, SkynetGuiPlugin.getInstance(),
				VIEW_ID, "Change Report");
	}
	...
	/**
	 * Compares the first and last version for the range of the selected
	 * artifact.
	 * 
	 * @param popupMenu
	 */
	private void createDiffMenuItem(Menu popupMenu) {
		...
		diffMenuItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent event) {
				ArtifactChange selectedItem = (ArtifactChange) ((ITreeNode) ((IStructuredSelection) changeTable
						.getSelection()).getFirstElement()).getBackingData();

				try {
					if (selectedItem.getArtifact() != null) {
						Artifact firstArtifact = selectedItem.getModType() == NEW ? null
								: artifactManager
										.getArtifactFromId(
												selectedItem.getArtifact()
														.getArtId(),
												selectedItem
														.getBaselineTransactionId());
						Artifact secondArtifact = selectedItem.getModType() == DELETE ? null
								: artifactManager.getArtifactFromId(
										selectedItem.getArtifact().getArtId(),
										selectedItem.getToTransactionId());

						RendererManager.getInstance().compareInJob(
								firstArtifact, secondArtifact, DIFF_ARTIFACT);
					}
				} catch (Exception ex) {
					OSEELog.logException(getClass(), ex, false);
				}
	...
	private void createWordChangeReportMenuItem(Menu popupMenu) {
		...
		wordChangeMenuItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent event) {
				IStructuredSelection selection = (IStructuredSelection) changeTable
						.getSelection();
				Iterator<?> iterator = selection.iterator();
				int listSize = selection.size();
				ArtifactChange selectedItem = null;

				ArrayList<Artifact> baseArtifacts = new ArrayList<Artifact>(
						listSize);
				ArrayList<Artifact> newerArtifacts = new ArrayList<Artifact>(
						listSize);

				while (iterator.hasNext()) {
					selectedItem = (ArtifactChange) ((ITreeNode) iterator
							.next()).getBackingData();

					try {
						Artifact baseArtifact = selectedItem.getModType() == NEW ? null
								: artifactManager
										.getArtifactFromId(
												selectedItem.getArtifact()
														.getArtId(),
												selectedItem
														.getBaselineTransactionId());
						Artifact newerArtifact = selectedItem.getModType() == DELETE ? null
								: artifactManager.getArtifactFromId(
										selectedItem.getArtifact().getArtId(),
										selectedItem.getToTransactionId());
				...
				WordRenderer renderer = (WordRenderer) RendererManager
						.getInstance().getRendererById(
								"org.eclipse.osee.framework.ui.skynet.word");

				try {
					renderer.compareArtifacts(baseArtifacts, newerArtifacts,
							DIFF_ARTIFACT, null, selectedItem
									.getBaselineTransactionId().getBranch());
	...
	private void createParentDiffMenuItem(Menu popupMenu) {
		...
		diffOnParentBranchMenuItem
				.setText("Word changes made to parent branch since creating current branch");
		diffOnParentBranchMenuItem.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				try {
					Artifact firstArtifact = selectedItem.getModType() == NEW ? null
							: artifactManager.getArtifactFromId(selectedItem
									.getArtifact().getArtId(), selectedItem
									.getBaselineTransactionId());

					Artifact secondArtifact = null;
					Branch parentBranch = firstArtifact.getBranch()
							.getParentBranch();

					TransactionId transactionId = TransactionIdManager
							.getInstance().getEditableTransactionId(
									parentBranch);
					secondArtifact = selectedItem.getModType() == DELETE ? null
							: artifactManager.getArtifactFromId(selectedItem
									.getArtifact().getArtId(), transactionId);

					RendererManager.getInstance().compareInJob(firstArtifact,
	...
	private void createDiffConflictsMenuItem(Menu popupMenu) {
		...
		diffConflictsMenuItem
				.setText("Word differences between current/parent branches");
		diffConflictsMenuItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				try {
					Artifact secondArtifact = artifactManager
							.getArtifactFromId(selectedItem.getArtifact()
									.getArtId(), toTransactionId);
					RendererManager.getInstance().compareInJob(
							selectedItem.getConflictingModArtifact(),
							secondArtifact, DIFF_ARTIFACT);
				} catch (Exception ex) {
					OSEELog.logException(getClass(), ex, false);
				}
				...
				try {
					...
					RevisionHistoryView revisionHistoryView = (RevisionHistoryView) page
							.showView(RevisionHistoryView.VIEW_ID,
									selectedArtifact != null ? selectedArtifact
											.getGuid() : Integer
											.toString(selectedItem.getArtId()),
		...
		revertMenuItem.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (MessageDialog
					...
					Jobs.startJob(new RevertJob(selectedItem.getName(),
							selectedItem.getArtId()));
				}
			}
		});
	}
		...
		if (selection != null && !selection.isEmpty()) {
			for (Object object : selection.toArray()) {
				...
				if (object instanceof ITreeNode
						&& ((ITreeNode) object).getBackingData() instanceof ArtifactChange) {
					try {
						artifact = ((ArtifactChange) ((ITreeNode) object)
								.getBackingData()).getArtifact();
						if (accessManager.checkObjectPermission(artifact,
								PermissionEnum.READ)) {
			...
			if (artifactTransferData.size() > 0)
		...
		if (selection != null && !selection.isEmpty()) {
			...
			for (Object object : selectedObjects) {
				if (object instanceof ITreeNode
						&& ((ITreeNode) object).getBackingData() instanceof ArtifactChange) {
					try {
						Artifact artifact = ((ArtifactChange) ((ITreeNode) object)
								.getBackingData()).getArtifact();
						Artifact headArtifact = artifactManager.getArtifact(
	...
	private void performCompression() throws SQLException {
		...
		if (selection != null && !selection.isEmpty()) {
			for (Object object : selection.toArray()) {
				...
				if (object instanceof ITreeNode) {
					ArtifactChange artifactChange = (ArtifactChange) ((ITreeNode) object)
							.getBackingData();
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					...
					for (Integer artifact : artifacts) {
						if (WordUtil.revertNonusefulWordChanges(artifact,
								branch, "osee_compression_gammas"))
							count++;
						monitor.worked(1);
					...
					Displays.ensureInDisplayThread(new Runnable() {
						public void run() {
							MessageDialog.openInformation(changeTable
									.getControl().getShell(),
									"Compression Data", finalCount + " of the "
											+ total
											+ " artifacts need compression");
						}
					});
					...
					return new Status(Status.ERROR, SkynetGuiPlugin.PLUGIN_ID,
	...
	public static void openViewUpon(final Branch branch) {
		Job job = new Job("Open Change Report") {
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					Pair<TransactionId, TransactionId> transactionToFrom = TransactionIdManager
							.getInstance().getStartEndPoint(branch);
					if (transactionToFrom.getKey().equals(
							transactionToFrom.getValue())) {
						AWorkbench.popup("Information",
								"There are no changes on this branch.");
						monitor.done();
						return Status.OK_STATUS;
					}
					...
					Displays.ensureInDisplayThread(new Runnable() {
						public void run() {
							try {
								IWorkbenchPage page = AWorkbench
										.getActivePage();
								ChangeReportView changeReportView = (ChangeReportView) page
										.showView(VIEW_ID, String
												.valueOf(branch.getBranchId()),
												IWorkbenchPage.VIEW_ACTIVATE);

								changeReportView.explore(branch);
							} catch (PartInitException ex) {
								OSEELog.logException(ChangeReportView.class,
										ex, true);
							}
						}
					});
	...
	public static void openViewUpon(ChangeReportInput input)
			throws PartInitException {
		IWorkbenchPage page = AWorkbench.getActivePage();
		ChangeReportView changeReportView = (ChangeReportView) page.showView(
				VIEW_ID, String.valueOf(input.getToTransaction()
						.getTransactionNumber()), IWorkbenchPage.VIEW_ACTIVATE);
		changeReportView.explore(input);
	}

	/**
	 * Explores the changes between to transactions. The transactions must not
	 * be null, must be on the same branch, and the toTransaction must be after
	 * the baseTransaction. Conflicting changes against the origination branch
	 * can also be reported. Conflicts can only be detected for a transactions
	 * on a branch that have a parent branch.
	 */
		...
		Displays.ensureInDisplayThread(new Runnable() {
			public void run() {
				...
				if (input.isEmptyChange())
					setContentDescription("Changes on " + input.getBranch()
							+ " at " + new Date());
				else if (baseParentTransactionId == null)
					setContentDescription("Changes on "
							+ baseTransactionId.getBranch().getBranchName()
							+ " from transaction " + baseNum
							+ " to transaction " + toNum + " at " + new Date());
				else
					setContentDescription("Changes on "
							+ baseTransactionId.getBranch().getBranchName()
							+ " from transaction " + baseNum
							+ " to transaction " + toNum + " against "
							+ baseParentTransactionId.getBranch() + ":"
							+ baseParentTransactionId.getTransactionNumber()
							+ " at " + new Date());

				((BranchContentProvider) changeTable.getContentProvider())
						.refresh(true);
			}
		});
	}
		...
		public void menuShown(MenuEvent e) {
			...
			while (iter.hasNext() && (readPermission || writePermission)) {
				...
				if (object instanceof ArtifactChange) {
					try {
						Artifact artifact = ((ArtifactChange) object)
								.getArtifact();

						if (artifact != null) {
							readPermission &= accessManager
									.checkObjectPermission(artifact,
											PermissionEnum.READ);
							writePermission &= accessManager
									.checkObjectPermission(artifact,
			...
			try {
				artifactSelected = obj1 instanceof ArtifactChange
						&& obj2 == null
						&& ((ArtifactChange) obj1).getArtifact() != null;
			} catch (SQLException ex) {
				OSEELog.logException(getClass(), ex, false);
			}
			...
			if (artifactSelected) {
				try {
					...
					boolean wordArtifactSelected = artifactSelected
							&& changedArtifact instanceof WordArtifact;
					boolean modifiedWordArtifactSelected = wordArtifactSelected
							&& change.getModType() == CHANGE;
					boolean conflictedWordArtifactSelected = modifiedWordArtifactSelected
							&& change.getChangeType() == ChangeType.CONFLICTING;
					boolean validDiffParent = wordArtifactSelected
							&& parentBranch != null;

					showInExplorer.setEnabled(artifactSelected
							&& reportBranch == branchPersistenceManager
									.getDefaultBranch());

					copyMenuItem.setEnabled(readPermission);
					// showFinalWordVersionMenuItem.setEnabled(wordArtifactSelected
					// && readPermission);
					diffMenuItem.setEnabled(wordArtifactSelected
							&& readPermission);
					diffMenuItem.setText("Word changes made to \""
							+ reportBranch.getBranchShortName() + "\"");

					diffConflictsMenuItem
							.setEnabled(conflictedWordArtifactSelected
									&& readPermission);
					if (conflictedWordArtifactSelected) {
						diffConflictsMenuItem
								.setText("Word differences between \""
										+ reportBranch.getBranchShortName()
										+ "\" and \""
										+ parentBranch.getBranchShortName()
										+ "\"");
					}

					revertMenuItem.setEnabled(artifactSelected
							&& writePermission);
					compressWordAttributes.setEnabled(artifactSelected
							&& writePermission
							&& OseeProperties.getInstance().isDeveloper());

					diffOnParentBranchMenuItem.setEnabled(validDiffParent
							&& modifiedWordArtifactSelected && readPermission);

					if (diffOnParentBranchMenuItem.getEnabled()) {
						diffOnParentBranchMenuItem
								.setText("Word changes made to \""
										+ parentBranch.getBranchShortName()
										+ "\" since creating \""
										+ reportBranch.getBranchShortName()
										+ "\"");
					...
					if (showFinalVersionMenuItem.getEnabled()) {
						showFinalVersionMenuItem.setText(SHOW_FINAL_VERSION_TXT
								+ " ("
								+ change.getToTransactionId()
										.getTransactionNumber() + ")");
					} else {
						showFinalVersionMenuItem
								.setText(SHOW_FINAL_VERSION_TXT);
					}
			...
			showFinalVersionMenuItem.setEnabled(artifactSelected
					&& readPermission);
	...
	public void init(IViewSite site, IMemento memento) throws PartInitException {
		...
		try {
			...
			OSEELog.logWarning(getClass(), "Change report error on init: "
		...
		public Artifact[] getArtifacts() throws SQLException {
			...
			if (selection != null && !selection.isEmpty()) {
				for (Object object : selection.toArray()) {
					...
					if (object instanceof ITreeNode) {
						artifacts.add(((ArtifactChange) ((ITreeNode) object)
								.getBackingData()).getArtifact());
					}
				}
			}
	...
	public void onEvent(Event event) {
		...
		ChangeReportInput changeReportInput = (ChangeReportInput) changeTable
				.getInput();

		if (changeReportInput != null
				&& changeReportInput.getBranch().getBranchId() == branchId) {
	...
	/**
	 * Revert changes for a given artifact Id up to the transaction point being
	 * viewed by this report.<br/><br/> <i>Note that this job is constrained
	 * to being able to only revert at most 1000 attribute and 1000 link changes
	 * do to its direct use of the SQL IN clause.</i>
	 * 
	 * @author Robert A. Fisher
	 */
	...
	private final class RevertDbTx extends AbstractDbTxTemplate {
		...
		protected void handleTxWork() throws Exception {
			...
			Collection<RevisionChange> revisionChanges = revisionManager
					.getAllTransactionChanges(OUTGOING, baseTransactionId
							.getTransactionNumber(), toTransactionId
							.getTransactionNumber(), artId, null);
			...
			if (allGammas.size() > 0) {
				ConnectionHandler.runPreparedUpdate("DELETE FROM "
						+ TRANSACTIONS_TABLE + " WHERE "
						+ TRANSACTIONS_TABLE.column("gamma_id") + " IN"
						+ Collections.toString(allGammas, "(", ",", ")"));
			}
			monitor.worked(1);
			isCanceled();
			...
			if (artifactGammas.size() > 0) {
				ConnectionHandler.runPreparedUpdate("DELETE FROM "
						+ ARTIFACT_VERSION_TABLE + " WHERE "
						+ ARTIFACT_VERSION_TABLE.column("gamma_id") + " IN "
						+ Collections.toString(artifactGammas, "(", ",", ")"));
			}
			monitor.worked(1);
			isCanceled();
			...
			if (attributeGammas.size() > 0) {
				ConnectionHandler.runPreparedUpdate("DELETE FROM "
						+ ATTRIBUTE_VERSION_TABLE + " WHERE "
						+ ATTRIBUTE_VERSION_TABLE.column("gamma_id") + " IN "
						+ Collections.toString(attributeGammas, "(", ",", ")"));
			}
			monitor.worked(1);
			isCanceled();
			...
			if (linkGammas.size() > 0) {
				ConnectionHandler.runPreparedUpdate("DELETE FROM "
						+ RELATION_LINK_VERSION_TABLE + " WHERE "
						+ RELATION_LINK_VERSION_TABLE.column("gamma_id")
						+ " IN "
						+ Collections.toString(linkGammas, "(", ",", ")"));
			}
			monitor.worked(1);
			isCanceled();
			...
			ConnectionHandler.runPreparedUpdate("DELETE FROM "
					+ TRANSACTION_DETAIL_TABLE + " WHERE "
					+ TRANSACTION_DETAIL_TABLE.column("branch_id") + " = ?"
					+ " AND "
					+ TRANSACTION_DETAIL_TABLE.column("transaction_id")
					+ " NOT IN " + "(SELECT "
					+ TRANSACTIONS_TABLE.column("transaction_id") + " FROM "
					+ TRANSACTIONS_TABLE + ")", SQL3DataType.INTEGER,
