...
public class RelationOrderAnalysisOnBranch extends AbstractBlam {
	...
	private static Matcher getIds = Pattern.compile(
			".*?artId\\[(\\d+?)\\].*?relTypeId\\[(\\d+?)\\].*").matcher(
			"");
	final List<String> messages = new ArrayList<String>();
	private Map<Branch, Map<Integer, ArtifactOrder>> branchToArtifactsToSort = new HashMap<Branch, Map<Integer, ArtifactOrder>>(); 
	 ...
	 * runOperation(org.eclipse.osee.framework.ui.skynet.blam.BlamVariableMap,
	...
	public void runOperation(BlamVariableMap variableMap,
		...
		for(Branch branch:branchToArtifactsToSort.keySet()){
			appendResultLine(String
					.format(
							"We have %d artifacts that have unsorted relations on branch %s.",
		...
		if (widget.getLabel().equals("Branch")) {
			...
			branchSelection.addListener(new Listener() {
				...
				public void handleEvent(Event event) {
					...
					Jobs.startJob(new Job(String.format(
							"Analyizing %s for unsorted relations.", branch
									.getBranchName())) {
						...
						protected IStatus run(IProgressMonitor monitor) {
							try {
								...
								ArtifactQuery.reloadArtifactsFromBranch(branch,
										false);
								OseeLog.unregisterLoggerListener(listener);

								File branchReport = OseeData.getFile(String
										.format("relationOrder_%s.txt", branch
												.getBranchShortName()));
								...
								OseeLog.log(
										RelationOrderAnalysisOnBranch.class,
										Level.SEVERE, th);
