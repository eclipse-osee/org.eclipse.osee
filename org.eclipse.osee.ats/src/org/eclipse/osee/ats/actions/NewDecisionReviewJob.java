...
public class NewDecisionReviewJob extends Job {
	...
	public NewDecisionReviewJob(TeamWorkFlowArtifact teamParent,
			boolean againstCurrentState) {
	...
	public IStatus run(final IProgressMonitor monitor) {
		try {
			AbstractSkynetTxTemplate newDecisionReviewTx = new AbstractSkynetTxTemplate(
					branchManager.getAtsBranch()) {

				@Override
				protected void handleTxWork() throws Exception {
					decisionReviewArtifact = createNewDecisionReview(
							teamParent, againstCurrentState);
					decisionReviewArtifact.persist(true);
				}

			};
			newDecisionReviewTx.execute();
			AtsLib.openAtsAction(decisionReviewArtifact,
					AtsOpenOption.OpenOneOrPopupSelect);
		} catch (Exception ex) {
			monitor.done();
			return new Status(Status.ERROR, AtsPlugin.PLUGIN_ID, -1,
					"Error creating Decision Review", ex);
		} finally {
			monitor.done();
		}
	...
	public static DecisionReviewArtifact createNewDecisionReview(
			StateMachineArtifact teamParent, boolean againstCurrentState)
			throws SQLException {
		DecisionReviewArtifact decRev = (DecisionReviewArtifact) ConfigurationPersistenceManager
				.getInstance().getArtifactSubtypeDescriptor(
						DecisionReviewArtifact.ARTIFACT_NAME,
						branchManager.getAtsBranch()).makeNewArtifact();

		if (teamParent != null)
			teamParent.relate(RelationSide.TeamWorkflowToReview_Review, decRev);
		if (againstCurrentState)
		...
		decRev
				.setDescriptiveName("Should we do this?  Yes will require followup, No will not");
		decRev.setSoleAttributeValue(ATSAttributes.DESCRIPTION_ATTRIBUTE
				.getStoreName(), "Enter description of the decision, if any");
		decRev.setSoleAttributeValue(
		...
		decRev.getLog().addLog(LogType.StateEntered,
