      ...
      table.addListener(SWT.PaintItem, new Listener() {
         ...
         public void handleEvent(Event event) {
            if (event.index == 1) {
               ...
               // Draw first portion
               percent = 100 - percents[index];
               foreground = gc.getForeground();
               background = gc.getBackground();
               gc.setForeground(display.getSystemColor(SWT.COLOR_RED));
               gc.setBackground(display.getSystemColor(SWT.COLOR_YELLOW));
               int width1 = (column2.getWidth() - 1) * percent / 100;
               gc.fillGradientRectangle(event.x + width, event.y, width1, event.height, true);
               rect2 = new Rectangle(event.x + width, event.y, width1 - 1, event.height - 1);
               gc.drawRectangle(rect2);
               gc.setForeground(display.getSystemColor(SWT.COLOR_LIST_FOREGROUND));
               text = percent + "%";
               size = event.gc.textExtent(text);
               offset = Math.max(0, (event.height - size.y) / 2);
               gc.drawText(text, event.x + width + 5, event.y + offset, true);
               gc.setForeground(background);
               gc.setBackground(foreground);
            }
         }
