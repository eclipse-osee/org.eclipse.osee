...
public class AtsWorkFlowFactory {
   ...
   private static Map<String, NativeArtifact> artifactTypeToNativeArtifact = new HashMap<String, NativeArtifact>();
   ...
   public static AtsWorkFlow getWorkflow(StateMachineArtifact sma) throws Exception, IOException, MultipleAttributesExist, ArtifactDoesNotExist, MultipleArtifactsExist {
      if (AtsPlugin.isAtsUseWorkflowFiles()) {
         ...
         NativeArtifact nativeArt = getWorkflowNativeArtifactFromArtifactType(sma);
         if (nativeArt != null)
   ...
   private static NativeArtifact getWorkflowNativeArtifactFromArtifactType(StateMachineArtifact sma) throws Exception {
      if (!artifactTypeToNativeArtifact.containsKey(sma.getArtifactTypeName())) {
         // If TaskArtifact, walk up team definition tree till find a workflow diagram or exception if not found
         ...
         artifactTypeToNativeArtifact.put(sma.getArtifactTypeName(), nativeArt);
      }
      return artifactTypeToNativeArtifact.get(sma.getArtifactTypeName());
   }

   private static NativeArtifact getTeamDefinitionWorkflowDiagram(CoreRelationEnumeration side, TeamDefinitionArtifact teamDef) throws SQLException {
