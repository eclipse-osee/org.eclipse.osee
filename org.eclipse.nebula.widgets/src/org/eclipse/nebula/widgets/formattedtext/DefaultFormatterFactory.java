 ...
 * <p>
 * 
 * This factory is called by <code>FormattedText</code> when a value is setted and no formatter
 * has been provided by constructor or <code>setFormatter()</code>. The default formatter is
 * based on the class of the value. The cache is searched first for the exact value's class. If no
 * formatter is found, then it is searched for any class that is an ancestor of the value's class.
 * The cache is a <code>HashMap</code> and the order of search cannot be garanteed. The first
 * valid ancestor found is returned.
 * <p>
 * 
 ...
 * </ul>
 * 
 * Other formatters can be added with the <code>register()</code> method. Each formatter class
 * must implement <code>ITextFormatter</code> and must have a default constructor.
 * <p>
 * 
...
public abstract class DefaultFormatterFactory {
   ...
   /**
    * Creates a new default formatter for the given value. The formatter is based on the value's
    * class.
    * 
    * @param value Value for which to create a formatter
    * @return New formatter corresponding to the value's class, or null if the class is unknown.
    */
   ...
   /**
    * Registers a new formatter class for a given class of values. The formatter class must
    * implement the <code>ITextFormatter</code> interface.
    * 
