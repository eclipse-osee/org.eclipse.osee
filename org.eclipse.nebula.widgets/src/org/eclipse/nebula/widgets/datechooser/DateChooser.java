 ...
 * <p>
 * Calendar is composed of a header and a grid for date selection. The header display the current
 * month and year, and the two buttons for navigation (previous and next month). An optional footer
 ...
 * <ul>
 * <li>The <code>getSelectedDate()</code> method returns the currently selected date (single
 * selection). The <code>getSelectedDates()</code> returns a list of all selected dates (multi
 * selection).</li>
 * <li>Add a <code>CalendarListener</code> to the calendar. This listener will be notified of
 * selection. <code>event.data</code> contain the selection if in single selection mode.</li>
 * </ul>
 * <p>
 * Keyboard navigation :
 * <ul>
 ...
 * <li>SPACE: Select the cell having the focus. If in multi selection mode, all previous selected
 * dates are cleared.</li>
 * <li>Ctrl-SPACE: Add the cell having the focus to the selection (multi selection mode).</li>
 * <li>Shift-SPACE: Select all dates in the interval between the current focus and the last
 * selected date.</li>
 * <li>HOME: Set the focus on the today date, and select it if autoselectOnFooter is true.</li>
 * </ul>
 */
public class DateChooser extends Composite {
   ...
   private ArrayList<ClearListener>clearListeners = new ArrayList<ClearListener>();
    ...
    * <p>
    * The calendar is initialized by default with the default Locale, and the current date for today
    * and selected date attributes.
    * 
   ...
   /**
    * Displays a new month in the grid. The new month is specified by delta from the currently
    * displayed one.
    * 
   ...
   /**
    * Clears the selection. The refresh flag allowes to indicate must be refreshed or not.
    * 
   ...
   public Point computeSize(int wHint, int hHint, boolean changed) {
      if (resize) {
         ...
         width += prevMonth.computeSize(SWT.DEFAULT, SWT.DEFAULT, changed).x * 2 + HEADER_SPACING * 4
               + gc.textExtent(" 9999").x; //$NON-NLS-1$
         width = Math.max(width, (gc.textExtent("99").x + theme.cellPadding * 2) * colCount + linesCount); //$NON-NLS-1$
   ...
   /**
    * Creates the footer of the calendar. The footer contains the label displaying the today date.
    * It is not visible by default and can be made visible with setFooterVisible(true).
    */
   private void createFooter() {
      ...
      Button clearButton = new Button(footer, SWT.CENTER);
      clearButton.setText("none");
   ...
   /**
    * Creates the grid of labels displying the days numbers. The grid is composed of a header row,
    * displaying days initiales, and 6 row for the days numbers. All labels are empty (no text
    * displayed), the content depending of both the locale (for first day of week) and the current
    * displayed month.
    */
   ...
   /**
    * Creates the header of the calendar. The header contains the label displaying the current month
    * and year, and the two buttons for navigation : previous and next month.
    */
    ...
    * list is returned, with no garanty of the selection order by the user. If no selection, return
    * <code>null</code>.
    * 
   ...
   /**
    * Returns all the selected dates. The collection returned is a copy of the internal selection
    * list.
    * <p>
   ...
   /**
    * Returns <code>true</code> if the given date is selected, else returns <code>false</code>.
    * 
    * @param date
   ...
   /**
    * Refreshes the display of the grid and header. This can be need because of a month display, a
    * locale or color model change.
    */
   ...
   /**
    * Removes the given date from the selection. The refresh flag allowes to indicate must be
    * refreshed or not.
    * 
    * @param d date to remove
    * @param refresh true to refresh display, else false
   ...
   /**
    * Removes the listener from the collection of listeners who will be notified when the receiver's
    * selection changes.
    * 
   ...
   /**
    * Manages the selection based on the current selected cell, specified by index, and the keyboard
    * mask.
    * 
   ...
   /**
    * Sets to <code>true</code> to enable the automatic change of current month when an adjacent
    * day is clicked in the grid.
    * <p>
   ...
   /**
    * Set the autoSelectOnFooter mode. If true, the today date is automatically selected on the
    * footer selection event. This mode is <code>false</code> by default.
    * 
   ...
   private void setCellColors(Cell cell) {
      if (cell.selected) {
         ...
         cell.label.setForeground(cell.adjacent != 0 ? theme.extraMonthForeground
               : (cell.weekend ? theme.weekendForeground : theme.dayCellForeground));
      }
   }
    ...
    * <p>
    * This method allowes to change the default first day of the week setted from the locale. It
    * must be called after <code>setLocale()</code>.
    * 
   ...
   /**
    * Causes the receiver to have the <em>keyboard focus</em>, such that all keyboard events will
    * be delivered to it.
    * 
    * @return <code>true</code> if the control got focus, and <code>false</code> if it was
    *         unable to.
    */
    ...
    * <p>
    * The new font is applied to all elements (labels) composing the calendar. The width of cells is
    * adjusted.
    * 
   ...
   /**
    * Sets the footer visible or not. The footer displays the today date. It is not visible by
    * default.
    * 
   ...
   /**
    * Sets a new locale to use for calendar. Locale will define the names of months and days, and
    ...
    * <p>
    * This method allowes to change the default value setted from the locale. It must be called
    * after <code>setLocale()</code>.
    * 
    ...
    * Sets the today date.
    * <p>
    * By default the today date is initialized to the current system date. But it can be needed to
    * ajust it for specifics needs.
    * 
   ...
   /**
    * Sets the weeks numbers visible or not. By default, the weeks are NOT visibles.
    * 
