...
public class ExtensionDefinedObjects<T> {
	...
	public ExtensionDefinedObjects(String extensionPointId, String elementName,
			String classNameAttribute) {
		if (true != Strings.isValid(extensionPointId)) {
			throw new IllegalStateException(
					"The extension point id is not valid");
		}
		...
		if (true != Strings.isValid(classNameAttribute)) {
			throw new IllegalStateException(
					"The class name attribute is not valid");
		}
	...
	private void initialize(String extensionPointId, String elementName,
			String classNameAttribute) {
		loadedObjects = new ArrayList<T>();
		objectsByID = new HashMap<String, T>();
		List<IConfigurationElement> elements = ExtensionPoints
				.getExtensionElements(extensionPointId, elementName);
		...
		if (loadedObjects.size() == 0) {
			throw new IllegalArgumentException(
					String
							.format(
									"No Objects loaded for [%s] with element name [%s] and attribute [%s]",
