...
import org.eclipse.osee.framework.plugin.core.config.data.DbInformation.DbObjectType;
...
public class ServerConfigUtil {
   private static Logger logger = ConfigUtil.getConfigFactory().getLogger(ServerConfigUtil.class);
   ...
   public DbInformation getDefaultService() {
      ...
      if (dbConnectionId != null) {
         if (servicesMap.containsKey(dbConnectionId)) {
            logger.log(Level.INFO, "Using DEFAULT_DB_CONNECTION: " + dbConnectionId);
            return buildDbInformation(dbConnectionId);
         }
         // Don't go to default if db connection id is specified but not valid
         logger.log(Level.SEVERE, "Invalid DB Connection Id=> " + dbConnectionId);
         return null;
      }
      ...
      for (String key : keys) {
         ...
         if (serviceData.isDefault()) {
            logger.log(Level.INFO, "Using DEFAULT_DB_CONNECTION: " + key);
