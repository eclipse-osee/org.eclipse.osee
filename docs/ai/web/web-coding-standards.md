...
- **On SSE-connected pages, never use `waitForLoadState('networkidle')`.** OSEE opens its real-time stream via `fetchEventSource` (a long-lived request), so the page never reaches network-idle and the wait hangs until timeout. Use the shared SSE-aware helpers in `playwright/shared/sse-helpers.ts` instead:
