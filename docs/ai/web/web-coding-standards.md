...
- **On SSE-connected pages, never use `waitForLoadState('networkidle')`.** OSEE opens its real-time stream via `fetchEventSource` (a long-lived request), so the page never reaches network-idle and the wait hangs until timeout. When you genuinely need "the page's data calls have gone quiet" (e.g. before interacting after a mutation-driven refetch, or to confirm two editors have both finished loading before asserting cross-user SSE propagation), use the shared `waitForNetworkIdleIgnoringSse(page)` from `playwright/shared/sse-helpers.ts` — it tracks only discrete requests and ignores the real-time stream (`/events`, presence). Prefer a concrete web assertion when one cleanly captures readiness; reach for this only when no single UI signal does.
