...
- `plugins/org.eclipse.osee.orcs.rest.model/src/org/eclipse/osee/orcs/rest/model/ReportEndpoint.java` — REST endpoint interface (single method with `format` and `email` query params)
...
- **The `GenericReport` builder uses a shared mutable `QueryBuilder` reference.** Each `level()` or `relationLevel()` call chains onto or replaces the query. This is intentional for the builder pattern — `report.query().follow(...)` mutates the shared query state. `relationLevel` records the main relation on the `ReportLevel` and defers the actual `follow`/`followFork` query calls until the level is finalized (when the next level is started or when `getDataRowsFromQuery` is called). This ensures `followFork` calls are emitted before `follow` on the QueryBuilder, matching the required ordering.
...
- **Do NOT add per-artifact queries in the data row loop.** `GenericReportBuilder.getArtsForLevel()` fetches related artifacts via `art.getRelated(relation)` for all relations in `level.getAllRelations()` (main + fork relations). The initial query (`query.asArtifacts()`) with properly ordered follow/followFork calls pre-loads all related artifacts in memory, so `getRelated` reads from the already-loaded relation hash maps without additional database calls.
- **`ReportLevel` owns its relation state.** `relationLevel` sets the main relation on the level via `setRelation()`. `followFork` adds fork relations via `addForkRelation()`. The actual QueryBuilder calls are deferred until `finalizeCurrentLevel()` is called (triggered by the next level being added, or by `getDataRowsFromQuery`). At that point, all `followFork` calls are emitted first, then the main `follow` — satisfying the QueryBuilder ordering rule. `getAllRelations()` returns main + fork relations for use in `getArtsForLevel()`.
...
- **`followFork` is level-scoped and position-sensitive.** It can only be called on a level created by `relationLevel`, and only before any columns are added to that level. The builder records fork relations on the `ReportLevel` without immediately calling `query.followFork()` — the actual query operations are batched and emitted by `finalizeCurrentLevel()` in the correct order (forks first, then follow). Duplicate relation names within the same level are rejected via `currentLevel.hasForkRelationName()`.
...
When the `email` query parameter is present, the endpoint spawns a background `Thread` that:
