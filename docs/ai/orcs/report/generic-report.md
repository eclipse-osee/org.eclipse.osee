...
7. `GenericReportBuilder` accumulates `ReportLevel` objects, each with columns and an associated query/relation.
8. `getDataRowsFromQuery()` executes the built query, recursively traverses levels via relations, fills data rows.
...
- **The `GenericReport` builder uses a shared mutable `QueryBuilder` reference.** Each `level()` or `relationLevel()` call chains onto or replaces the query. This is intentional for the builder pattern — `report.query().follow(...)` mutates the shared query state.
...
- **Do NOT add per-artifact queries in the data row loop.** `GenericReportBuilder.getArtsForLevel()` fetches related artifacts via `art.getRelated(relation)` which uses already-loaded relation data. The initial query (`query.asArtifacts()`) is the only database call during data generation.
...
- **Unit test** (`GenericReportBuilderTest`): uses Mockito to verify level/column/query construction, header generation, and data row filling with mocked artifacts and relations.
- **Integration test** (`ReportEndpointTest`): creates a ReportTemplate with `relationLevel` code, sets up test artifacts with Code-Requirement relations on the SAW working branch, calls the endpoint, and verifies Excel XML content contains expected data. Also tests the async endpoint returns proper JSON.
