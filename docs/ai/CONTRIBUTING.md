...
```
docs/ai/
├── CONTRIBUTING.md                  ← This file (always loaded by Kiro)
└── <subsystem>/<feature>.md         ← Max 3 levels. Kebab-case. No empty folders.
```

Each doc gets its own steering pointer. One doc, one pointer. No sharing.

## Adding a doc

1. Create your `.md` file with this frontmatter:

```yaml
---
summary: "One-line description"
tags: [tag1, tag2]
fileMatch: "glob matching relevant source files"
---
```

2. Create a steering pointer at `.kiro/steering/conditional/docs/<same-filename-as-doc>.md`:

```markdown
---
inclusion: fileMatch
fileMatchPattern: "<your fileMatch value>"
---

#[[file:docs/ai/<your-doc-path>.md]]
```

Pointer filename = doc filename. Always 1:1.

If you forget step 2, Kiro will create it next session (self-healing).

## Removing a doc

Delete the file. Kiro will clean up the orphaned pointer next session.
...
.kiro/steering/
├── auto/
│   └── docs/           ← Always-loaded doc steering (CONTRIBUTING.md + self-healing)
└── conditional/
    └── docs/           ← One pointer per doc, filename matches the doc it points to
```

The `docs/` subfolder separates documentation pointers from other steering (coding rules, PR policies, etc.) that your team may add later.

## fileMatch patterns

Be specific. Match only the source files the doc covers.
- Good: `"**/SoftwareReqVolatility*"`, `"web/apps/osee/src/app/mim/**"`
- Bad: `"**/*"`, `"**/*.ts"`

Broad docs (like web coding standards) can use broad patterns (`"web/**"`).

## Multi-agent contract

1. **Read before you write.** Check docs/ai/ before changing a subsystem.
2. **Write after significant changes.** If another agent would need to know, doc it.
3. **Fix stale docs.** If a doc contradicts source code, update it.
4. **Formalize raw docs.** If you find a `.md` in `docs/ai/` without frontmatter, read it, infer summary/tags/fileMatch, add the frontmatter, and create the steering pointer at `.kiro/steering/conditional/docs/`. If it's in the wrong subfolder, move it to the right `<subsystem>/<feature>.md` path.
5. **Trust, but verify.** Docs are a starting point. Always check source.
