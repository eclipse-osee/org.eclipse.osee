...
# Publish Launcher Configuration

## Overview

This change adds support for **Launcher–driven publishing** by introducing a Java-side **artifact token** whose attribute contains a `publishingJson` payload. That JSON is consumed by the Book Explorer UI and drives the publishing page layout, inputs, and publish actions.
...
To support publishing, a server side **artifact token** with an attribute that stores the `publishingJson` used by the publishing page has been defined and added - the artifact is referenced directly by the publish-launcher component, and contains the publishingJson that populates the information in the page.
...
## UI Design in `publish-launcher.component.ts`

The Angular component `src/app/ple/publish-launcher/publish-launcher.component.ts` is designed as a **configuration-driven publishing UI**.
...
The returned JSON is treated as `PublishLauncherConfig`.
...
```json docs/publish-launcher.md
{
  ...
  "tabs": [
    ...
    },
    {
      ...
      "instructions": ["Pick filters and click Publsih color."],
      "dropdowns": [],
      "checkboxes": [
        {
          "key": "showOnlyChanged",
