...
export class CreateActionFormComponent implements OnDestroy {
	...
	workTypes = toSignal(
		this.createActionService.workTypes.pipe(
			...
			tap(([types, data]) => {
				types.forEach((t) => {
					if (t.name === data.defaultWorkType) {
						this.workType.set(t);
						this.createActionService.workTypeValue = t.name;
						data.createBranchDefault = t.createBranchDefault;
						return;
					}
				});
			}),
	...
	ngOnDestroy(): void {
		this.createActionService.workTypeValue = '';
	}
}
