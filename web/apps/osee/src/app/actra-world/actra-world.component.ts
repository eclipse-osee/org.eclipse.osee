...
export class ActraWorldComponent implements AfterViewInit {
	...
	private _worldData = toSignal(
		this.params.pipe(
			switchMap((value) => {
				// Default route (no parameters) to My World
				if (this.paramsSignal.length == 0 || value.op === 'my') {
					const worldData = this.worldService.getWorldDataMy();
					/** How to print out the worldData structure (Observable is a stream) */
					worldData
						.pipe(
							map((data) => {
								console.log(data);
	...
	openWorkflow(wfId: String) {
		console.log(wfId);
		this.openLink('/actra-teamwf?id=' + wfId);
	}
}
