...
import { FocusLostInputComponent, MarkdownEditorComponent } from '@osee/shared/components';
		...
		@switch (attr().storeType) {
			...
			@default {
				@if (attr().name === 'Markdown Content') {
					<div (focusin)="onInput()" (focusout)="onMarkdownBlur()">
					...
					<span class="tw-block tw--ml-2" (focusin)="onInput()">
...
export class PersistedArtifactAttributeEditorComponent implements OnDestroy {
	...
	/** Called when focus leaves the markdown editor — saves if changed. */
	onMarkdownBlur() {
		if (this.pendingMarkdown !== null && this.pendingMarkdown !== this.previousValue()) {
