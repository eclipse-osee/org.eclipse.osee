...
export class MessagePeriodicityService {
	...
	getFiltered(
		...
		return this.http.get<NamedId[]>(
			apiURL + '/mim/branch/' + branchId + '/messagePeriodicities',
			{
				params: params,
			}
		);
	...
	getCount(branchId: string, filter?: string, viewId?: string) {
		...
		return this.http.get<number>(
			apiURL + '/mim/branch/' + branchId + '/messagePeriodicities/count',
			{
				params: params,
			}
		);
