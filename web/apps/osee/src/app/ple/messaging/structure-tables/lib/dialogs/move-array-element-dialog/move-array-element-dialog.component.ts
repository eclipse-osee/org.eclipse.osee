...
import { element, elementSentinel } from '@osee/messaging/shared/types';
...
export class MoveArrayElementDialogComponent {
	data = signal(
		inject<{ element: element; parent: element }>(MAT_DIALOG_DATA)
	);

	filter = signal('');

	elements = computed(() =>
		this.data().parent.arrayElements.filter(
			(e) =>
				!e.autogenerated &&
	...
	afterArtifact = computed(() => {
		...
		if (this.selectedLocation() === 'Before Element') {
			const selectedElementIndex =
				this.data().parent.arrayElements.findIndex(
			...
			return this.data().parent.arrayElements[selectedElementIndex - 1]
				.id;
		}
		return 'end';
	});
