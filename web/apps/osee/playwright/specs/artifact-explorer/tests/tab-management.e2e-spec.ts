...
test.describe('Tab Management', () => {
	...
	test('should close a tab when clicking the × button', async ({
		page,
	}) => {
		await openBranch(page, BRANCH);
		...
		await expect(
			tabGroup.getByText('AE Tab Parent').first()
		).toBeVisible({ timeout: 10000 });

		await page.getByRole('button', { name: 'Close tab' }).first().click();

		await expect(
			tabGroup.getByText('AE Tab Parent')
		).not.toBeVisible();
		await expect(
			page.getByText(
				'Select an artifact from the hierarchy to begin'
			)
		).toBeVisible();
	});
