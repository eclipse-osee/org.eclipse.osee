...
test.describe('Script Table', () => {
	test.beforeEach(async ({ page }) => {
		await navigateToScriptTable(page);
	});

	test('should load with data', async ({ page }) => {
		await expect(
			page.locator('[data-cy^="script-def-table-row-"]').first()
		).toBeVisible();
	});

	test('should narrow results when name filter is applied', async ({
		page,
	}) => {
		await expect(
			page.locator('[data-cy^="script-def-table-row-"]').first()
	...
	test('should restore results when filter is cleared', async ({ page }) => {
		await expect(
			page.locator('[data-cy^="script-def-table-row-"]').first()
		).toBeVisible();
