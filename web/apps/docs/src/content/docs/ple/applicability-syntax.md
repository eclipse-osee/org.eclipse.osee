...
A view is the bill of features required for a certain projection to be valid. View is often interchangably used with the term(s) Configuration or Configuration Group.
...
Applicability expression keys and values are matched/compared with the same capitalization i.e. Included does not equal INCLUDED.
...
An applicability expression which contains a key and a value can only be seperated by an equals sign, spaces, tabs, newlines, and carriage returns.
...
included_text is included in the projection when feature A is not set to Included, as Feature[!A=Included] expands out to FeatureNot[A=Included].
...
Featurization syntax does not currently support programming syntax for value comparison.

This evaluates to Feature[A=Included] and Feature[""=Included].
:::
