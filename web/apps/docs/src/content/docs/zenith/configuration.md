...
This setting specifies the number of results that are kept for each script, for each CI Set. When test results are imported, if the number of results for a specific test script on a CI Set goes over this value, the least recent results (based on text execution time) will be deleted.
