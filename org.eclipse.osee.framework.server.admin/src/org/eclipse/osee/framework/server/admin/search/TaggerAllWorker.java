...
class TaggerAllWorker extends BaseCmdWorker {
	...
	private static final String GET_TAGGABLE_SQL_BODY = " FROM osee_define_attribute attr1, osee_define_attribute_type type1,  osee_define_txs txs1, osee_define_tx_details txd1, osee_define_branch br1 WHERE txs1.transaction_id = txd1.transaction_id AND txs1.gamma_id = attr1.gamma_id AND txd1.branch_id = br1.branch_id AND br1.archived <> 1 AND attr1.attr_type_id = type1.attr_type_id AND type1.tagger_id IS NOT NULL";

	private static final String FIND_ALL_TAGGABLE_ATTRIBUTES = "SELECT DISTINCT attr1.gamma_id"
			+ GET_TAGGABLE_SQL_BODY;
	private static final String COUNT_TAGGABLE_ATTRIBUTES = "SELECT count(DISTINCT attr1.gamma_id)"
			+ GET_TAGGABLE_SQL_BODY;
	...
	private int getTotalItems(Connection connection, int branchId)
			throws SQLException {
		int total = -1;
		ConnectionHandlerStatement stmt = null;
		try {
			stmt = ConnectionHandler.runPreparedQuery(connection, getQuery(
					connection, branchId, true), branchId > -1 ? new Object[] {
					SQL3DataType.INTEGER, branchId } : new Object[0]);
			if (stmt.next()) {
	...
	private String getQuery(Connection connection, int branchId,
			boolean isCountQuery) throws SQLException {
		StringBuilder builder = new StringBuilder();
		builder.append(isCountQuery ? COUNT_TAGGABLE_ATTRIBUTES
				: FIND_ALL_TAGGABLE_ATTRIBUTES);
	...
	private void fetchAndProcessGammas(Connection connection, int branchId,
			TagProcessListener processor) throws SQLException {
		ConnectionHandlerStatement stmt = null;
		try {
			stmt = ConnectionHandler.runPreparedQuery(connection, getQuery(
					connection, branchId, false), branchId > -1 ? new Object[] {
					SQL3DataType.INTEGER, branchId } : new Object[0]);
	...
	protected void doWork(long startTime) throws Exception {
		...
		try {
			...
			println(String.format("Tagging Attributes For: [%s]",
					branchId > -1 ? "Branch " + branchId : "All Branches"));
			connection = OseeDbConnection.getConnection();

			int totalAttributes = getTotalItems(connection, branchId);
			processor = new TagProcessListener(startTime,
					totalAttributes);
	...
	private final class TagProcessListener implements ITagListener {
		...
		public TagProcessListener(long startTime, int totalAttributes) {
			this.queryIdMap = Collections
					.synchronizedMap(new HashMap<Integer, TagQueueJoinQuery>());
		...
		public void cancelProcessing(Connection connection) {
			...
			Activator.getInstance().getSearchTagger()
					.stopTaggingByQueueQueryId(toStop);
		}

		public void storeAndAddQueryId(Connection connection,
				TagQueueJoinQuery joinQuery) throws SQLException {
			if (joinQuery.size() > 0) {
		...
		public void printStats() {
			if (isVerbose()) {
				println(String
						.format(
								"QueryIds: [ %d of %d] Attributes: [%d of %d] - Elapsed Time = %s.",
								queriesProcessed, totalQueries(),
								attributesProcessed, totalAttributes,
		 ...
		 */
		@Override
		public void onAttributeTagComplete(int queryId, long gammaId,
				int totalTags, long processingTime) {
		 ...
		 */
		@Override
		synchronized public void onTagQueryIdTagComplete(int queryId,
				long waitTime, long processingTime) {
