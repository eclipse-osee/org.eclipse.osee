...
public class BranchExportWorker extends BaseCmdWorker {
   ...
   private static final String ALL_BRANCHES_QUERY =
         "SELECT x1.branch_id FROM (SELECT br1.branch_id FROM osee_branch br1%s AND br1.branch_type <> 3 UNION SELECT om1.merge_branch_id FROM osee_merge om1, osee_branch ob1 WHERE om1.dest_branch_id = ob1.branch_id%s UNION SELECT om2.source_branch_id from osee_merge om2, osee_branch ob2 WHERE om2.dest_branch_id = ob2.branch_id%s) x1 ORDER BY x1.branch_id";
