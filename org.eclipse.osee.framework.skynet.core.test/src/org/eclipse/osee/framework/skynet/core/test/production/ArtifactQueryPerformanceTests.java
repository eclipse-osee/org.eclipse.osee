...
public class ArtifactQueryPerformanceTests extends TestCase {
   ...
   public ArtifactQueryBuilder(ArtifactType artifactType, Branch branch, ArtifactLoad loadLevel, List<AbstractArtifactSearchCriteria> criteria) {
	...
	public void testGetArtifactByHRID() throws OseeCoreException {
	      ...
	      assertTrue(String.format("Elapsed time for artifact by hrid query took %dms.  It should take less than 50ms.", elapsedTime),elapsedTime < 50);
	}
	...
	public void testGetArtifactsByHRIDNoDeleted() throws OseeCoreException {
	      ...
	      List<Artifact> result = builder.getArtifacts(children.size()+1, null);
	      long elapsedTime = System.currentTimeMillis() - startTime;
	      System.out.println(String.format("testGetArtifactsByHRIDNoDeleted took %dms for %d artifacts", elapsedTime, result.size()));
	      assertTrue("No artifacts found", result.size() > 0);
	      assertTrue(String.format("Elapsed time for artifact by hrid query took %dms.  It should take less than 50ms.", elapsedTime),elapsedTime < 50);
	}
   ...
   public void testGetArtifactsByArtType() throws OseeCoreException {
		  Branch common = BranchManager.getCommonBranch();
	      ArtifactQueryBuilder builder = new ArtifactQueryBuilder(ArtifactTypeManager.getType("Team Definition"), common, FULL);
	      long startTime = System.currentTimeMillis();
	      List<Artifact> result = builder.getArtifacts(1000, null);
	      long elapsedTime = System.currentTimeMillis() - startTime;
	      System.out.println(String.format("testGetArtifactsByArtType took %dms for %d artifacts", elapsedTime, result.size()));
	      assertTrue("No artifacts found", result.size() > 0);
	      assertTrue(String.format("Elapsed time for testGetArtifactsByArtType took %dms.  It should take less than 750ms.", elapsedTime),elapsedTime < 750);
	}
   ...
   public void testGetArtifactsByArtTypes() throws OseeCoreException {
	      ...
	      long elapsedTime = System.currentTimeMillis() - startTime;
	      System.out.println(String.format("testGetArtifactsByArtTypes took %dms for %d artifacts", elapsedTime, result.size()));
	      assertTrue("No artifacts found", result.size() > 0);
	      assertTrue(String.format("Elapsed time for testGetArtifactsByArtTypes took %dms.  It should take less than 1100ms.", elapsedTime),elapsedTime < 1100);
	}
   ...
   public void testGetArtifactsByArtTypesAllowDeleted() throws OseeCoreException {
	      ...
	      List<Artifact> result = builder.getArtifacts(5000, null);
	      long elapsedTime = System.currentTimeMillis() - startTime;
	      System.out.println(String.format("testGetArtifactsByArtTypesAllowDeleted took %dms for %d artifacts", elapsedTime, result.size()));
	      assertTrue("No artifacts found", result.size() > 0);
	      assertTrue(String.format("Elapsed time for testGetArtifactsByArtTypesAllowDeleted took %dms.  It should take less than 3750ms.", elapsedTime),elapsedTime < 3750);
	}
   	...
   	public void testLoadAllBranch() throws OseeCoreException {
	      ...
	      assertTrue("No artifacts found", result.size() > 0);
	      assertTrue(String.format("Elapsed time for loadAllBranch took %dms.  It should take less than 500000ms.", elapsedTime),elapsedTime < 500000);
   	}
}
