...
pub fn get_schema_from_file(
    ...
    get_schema_from_file_name_and_extension(name, ext, start_comment_syntax, end_comment_syntax)
}

pub fn get_schema_from_file_name_and_extension(
    ...
    match file_extension {
        ...
        Some("cpp" | "cxx" | "cc" | "c" | "hpp" | "hxx" | "hh" | "h" | "java") => {
            get_schema("c".into())
        }
...
mod schema_resolution_tests {
    use super::{SupportedSchema, get_schema_from_file};
    use std::path::Path;

    fn is_build_file(path: &str) -> bool {
        matches!(
            get_schema_from_file(Path::new(path), "", ""),
            SupportedSchema::BuildFile
        )
    }

    #[test]
    fn python_files_use_build_file_schema() {
        assert!(is_build_file("script.py"));
        assert!(is_build_file("pkg/module.py"));
    }

    #[test]
    fn text_files_use_build_file_schema() {
        assert!(is_build_file("notes.txt"));
    }

    #[test]
    fn dockerfiles_use_build_file_schema() {
        // Extensionless, by file name.
        assert!(is_build_file("Dockerfile"));
        assert!(is_build_file("docker/Dockerfile"));
        // With a .dockerfile extension.
        assert!(is_build_file("service.dockerfile"));
    }

    #[test]
    fn unknown_extension_without_custom_syntax_is_unsupported() {
        assert!(matches!(
            get_schema_from_file(Path::new("image.png"), "", ""),
            SupportedSchema::NotSupported
        ));
    }
}
