...
type TokenizeCommentsError<I1> =
    ApplicabilityParserInternalErrorWithNomInputs<LocatedSpan<I1, TokenPosition>>;
    ...
    match tokenize_comments_parser(doc).parse_complete(input) {
        ...
        Result::Err(e) => match e {
            Err::Incomplete(needed) => match needed {
                nom::Needed::Unknown => {
                    Err(ApplicabilityParserInternalErrorWithNomInputs::NeedsUnknownMoreData)
                }
                nom::Needed::Size(non_zero) => Err(
                    ApplicabilityParserInternalErrorWithNomInputs::NeedsMoreData(non_zero.into()),
                ),
            },
            Err::Error(e) => Err(e),
            Err::Failure(e) => Err(e),
...
type TokenizeErrorType<I1> = ApplicabilityParserInternalErrorWithNomInputs<
