    ...
    thread::scope(|scope| {
        for input in &args.srcs {
            ...
            let _outer_thread = scope.spawn(move || {
                ...
                let (start_syntax, end_syntax) = get_comment_syntax(
                    input,
                    start_comment_syntax,
                    end_comment_syntax
                );
                let content_result = parse_applicability(
                    &file_contents,
                    start_syntax.as_str(),
                    end_syntax.as_str()
                );
                ...
                for config in applic_config_for_file.iter().cloned() {
                    ...
                    let _s2 = scope.spawn(move || {
                        ...
                        let mut out_dirs = match fs::canonicalize(out_dir) {
                            ...
                            Err(e) => {
                                println!(
                                    "Error finding output directory {:#?} .Received {:#?}",
                                    out_dir,
                                    e
                                );
                                out_dir.to_path_buf()
                        ...
                        let input_path = match fs::canonicalize(input) {
                            ...
                            Err(e) => {
                                println!(
                                    "Error finding input file {:#?} .Received {:#?}. You should check to see if the file exists.",
                                    input,
                                    e
                                );
                                input.to_path_buf()
