    ...
    ApplicabilityTag<I, String>: From<I>,
{
        ...
        && !matches!(transformer.current_token, LexerToken::EndFeature(_))
    {
        ...
        match &current_token {
            ...
            LexerToken::FeatureCase(position) => {
                //throw an error here
                error!(
                    "Feature Case found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
            ...
            LexerToken::EndFeature(position) => {
                error!(
                    "End Feature found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            ...
            LexerToken::ConfigurationCase(position) => {
                //throw an error here
                error!(
                    "Configuration Case found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationElse(position) => {
                error!(
                    "Configuration Else found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationElseIf(position) => {
                error!(
                    "Configuration Else If found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::EndConfiguration(position) => {
                //throw an error here
                error!(
                    "End Configuration found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            ...
            LexerToken::ConfigurationGroupCase(position) => {
                //throw an error here
                error!(
                    "Configuration Group Case found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationGroupElse(position) => {
                //throw an error here
                error!(
                    "Configuration Group Else found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
    ...
    match &current_token {
        ...
        LexerToken::ConfigurationGroupElseIf(position) => {
            //throw an error here
            error!(
                "Configuration Group Else If found at {:#?} to {:#?} in Feature Else at {:#?} to {:#?}",
                position.0, position.1, base_position.0, base_position.1
            );
        }
        LexerToken::EndConfigurationGroup(position) => {
