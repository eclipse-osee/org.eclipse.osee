        ...
        )
    {
        ...
        match &current_token {
            ...
            LexerToken::Feature(position) => {
                //throw an error here
                error!(
                    "Feature found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
            ...
            LexerToken::FeatureNot(position) => {
                //throw an error here
                error!(
                    "Feature Not found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::FeatureSwitch(position) => {
                //throw an error here
                error!(
                    "Configuration Group Switch found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::FeatureCase(position) => {
                error!(
                    "Feature Case found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::FeatureElse(position) => {
                //throw an error here
                error!(
                    "Feature Else found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::FeatureElseIf(position) => {
                //throw an error here
                error!(
                    "Feature Else If found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::EndFeature(position) => {
                //throw an error here
                error!(
                    "End Feature found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::Configuration(position) => {
                //throw an error here
                error!(
                    "Configuration found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationNot(position) => {
                //throw an error here
                error!(
                    "Configuration Not found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            ...
            LexerToken::ConfigurationCase(position) => {
                error!(
                    "Configuration Case found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationElse(position) => {
                //throw an error here
                error!(
                    "Configuration Else found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationElseIf(position) => {
                //throw an error here
                error!(
                    "Configuration Else If found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::EndConfiguration(position) => {
                //throw an error here
                error!(
                    "End Configuration found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationGroup(position) => {
                //throw an error here
                error!(
                    "Configuration Group found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationGroupNot(position) => {
                //throw an error here
                error!(
                    "Configuration Group Not found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            ...
            LexerToken::ConfigurationGroupElse(position) => {
                //throw an error here
                error!(
                    "Configuration Group Else found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::ConfigurationGroupElseIf(position) => {
                //throw an error here
                error!(
                    "Configuration Group Else If found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
            LexerToken::EndConfigurationGroup(position) => {
                //throw an error here
                error!(
                    "End Configuration Group found at {:#?} to {:#?} in Configuration Group Switch at {:#?} to {:#?}",
                    position.0, position.1, base_position.0, base_position.1
                );
            }
