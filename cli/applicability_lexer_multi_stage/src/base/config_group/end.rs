...
pub trait ConfigurationGroupEnd {
    fn is_config_group_end<I>(&self, input: I::Item) -> bool
    where
        I: Input,
        I::Item: AsChar;
    ...
    //TODO add default impl for transforming config_group_end into LexerToken
    fn take_till_config_group_end<'x, I, E>(&self) -> impl Parser<I, Output = I, Error = E>
    where
        I: Input + Compare<&'x str>,
        I::Item: AsChar,
        E: ParseError<I>,
    {
        take_till(|x| self.is_config_group_end::<I>(x))
    }
    fn config_group_end_tag<'x>(&self) -> &'x str{
        "End ConfigurationGroup"
    }
    fn take_until_config_group_end<'x, I, E>(
        &self,
