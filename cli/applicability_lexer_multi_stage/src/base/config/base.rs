...
pub trait ConfigurationBase {
    fn is_config_base<I>(&self, input: I::Item) -> bool
    where
        I: Input,
        I::Item: AsChar;
    ...
    //TODO add default impl for transforming config_base into LexerToken
    fn take_till_config_base<'x, I, E>(&self) -> impl Parser<I, Output = I, Error = E>
    where
        I: Input + Compare<&'x str>,
        I::Item: AsChar,
        E: ParseError<I>,
    {
        take_till(|x| self.is_config_base::<I>(x))
    }
    fn config_base_tag<'x>(&self) -> &'x str{
        "Configuration"
    }
    fn take_until_config_base<'x, I, E>(
        &self,
