...
pub trait ConfigurationElseIf {
    fn is_config_else_if<I>(&self, input: I::Item) -> bool
    where
        I: Input,
        I::Item: AsChar;
    ...
    //TODO add default impl for transforming config_else_if into LexerToken
    fn take_till_config_else_if<'x, I, E>(&self) -> impl Parser<I, Output = I, Error = E>
    where
        I: Input + Compare<&'x str>,
        I::Item: AsChar,
        E: ParseError<I>,
    {
        take_till(|x| self.is_config_else_if::<I>(x))
    }
    fn config_else_if_tag<'x>(&self) -> &'x str{
        "Configuration Else If"
    }
    fn take_until_config_else_if<'x, I, E>(
        &self,
