...
pub trait ConfigurationCase {
    fn is_config_case<I>(&self, input: I::Item) -> bool
    where
        I: Input,
        I::Item: AsChar;
    ...
    //TODO add default impl for transforming config_case into LexerToken
    fn take_till_config_case<'x, I, E>(&self) -> impl Parser<I, Output = I, Error = E>
    where
        I: Input + Compare<&'x str>,
        I::Item: AsChar,
        E: ParseError<I>,
    {
        take_till(|x| self.is_config_case::<I>(x))
    }
    fn config_case_tag<'x>(&self) -> &'x str{
        "Configuration Case"
    }
    fn take_until_config_case<'x, I, E>(
        &self,
