...
pub trait ConfigNotMultiLine {
    fn config_not<I, E>(&self) -> impl Parser<I, Output = Vec<LexerToken<I>>, Error = E>
    where
        I: Input
...
impl<T> ConfigNotMultiLine for T
where
    T: TagMultiLine + LexConfigurationNot + LexSpace + LexTab,
{
    fn config_not<I, E>(&self) -> impl Parser<I, Output = Vec<LexerToken<I>>, Error = E>
    where
        I: Input
        ...
        E: ParseError<I>,
    {
        ...
        self.lex_config_not_multi_line()
            .and(many0(self.lex_space().or(self.lex_tab())))
            .and(tag)
            .map(|((f, mut spaces), t)| {
                spaces.insert(0, f);
