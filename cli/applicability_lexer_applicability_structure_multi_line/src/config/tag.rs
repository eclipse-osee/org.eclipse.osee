...
use applicability_lexer_applicability_structure_base::config::{
...
pub trait ConfigTagMultiLine {
    fn config_tag_start<I, E>(&self) -> impl Parser<I, Output = Vec<LexerToken<I>>, Error = E>
    where
        I: Input
        ...
        + LexConfigurationSwitch,
{
    ...
    where
        ...
        E: ParseError<I>,
    {
        let config_base_tag = self.get_config_base();
        let config_not_tag = self.config_not();
        let config_case_tag = self.config_case();
        let config_else_if_tag = self.config_else_if();
        ...
        E: ParseError<I>,
    {
        ...
        config_not_tag
            .or(config_case_tag)
            .or(config_else_if_tag)
            .or(self.lex_config_else_multi_line().map(|x| vec![x]))
            .or(self.lex_config_end_multi_line().map(|x| vec![x]))
            .or(self.lex_config_switch_multi_line().map(|x| vec![x]))
            .or(config_base_tag)
    }
}
