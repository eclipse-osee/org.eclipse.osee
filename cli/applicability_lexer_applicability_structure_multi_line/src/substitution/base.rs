...
impl<T> SubstitutionMultiLine for T
where
    T: TagMultiLine + LexSubstitution + LexSpace + LexTab,
{
    fn substitution_multi_line<I, E>(
        &self,
    ) -> impl Parser<I, Output = Vec<LexerToken<I>>, Error = E>
...
mod tests {
    ...
    fn empty_eval() {
        ...
        let result: ResultType<&str> = Err(Err::Error(Error::from_error_kind(
    ...
    fn empty_eval_text() {
        ...
        let result: ResultType<&str> = Err(Err::Error(Error::from_error_kind(
            unsafe { LocatedSpan::new_from_raw_offset(4, 1, "[] abcd", ()) },
            ErrorKind::Verify,
        )));
        assert_eq!(parser.parse_complete(input), result)
    }
