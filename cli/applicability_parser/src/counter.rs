 ...
 **********************************************************************/
use nom::{character::complete::{line_ending, multispace1, space1}, combinator::{map, not, opt}, multi::{many0, many_till}, sequence::terminated, IResult};
...
pub fn count_new_lines<'a>()-> impl FnMut(&'a str) -> IResult<&'a str, u8>{
    map(many_till(terminated(opt(line_ending),many0(space1)), not(multispace1)),|(line_endings,_)|{
        line_endings.iter().map(|x| match x{
            Some(_) => 1,
            None => 0
        }).sum()
    })
