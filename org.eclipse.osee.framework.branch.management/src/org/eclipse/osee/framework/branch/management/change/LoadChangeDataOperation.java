...
public class LoadChangeDataOperation extends AbstractOperation {
	private static final String SELECT_SOURCE_BRANCH_CHANGES = "select txs.transaction_id, gamma_id, mod_type from osee_txs txs, osee_tx_details txd where txd.branch_id = ? and txd.transaction_id = txs.transaction_id and txd.tx_type = ? and txs.tx_current <> ?";

	private static final String SELECT_SOURCE_TRANSACTION_CHANGES = "select txs.transaction_id, gamma_id, mod_type from osee_txs txs, osee_tx_details txd where txd.transaction_id = ? and txd.transaction_id = txs.transaction_id and txd.tx_type = ?";
	...
	private final HashMap<Long, Pair<Integer, ModificationType>> changeByGammaId = new HashMap<Long, Pair<Integer, ModificationType>>();
	...
	public LoadChangeDataOperation(IOseeDatabaseServiceProvider oseeDatabaseProvider, int transactionNumber,
			TransactionRecord destinationTransactionId, Collection<ChangeItem> changeData) {
	...
	public LoadChangeDataOperation(IOseeDatabaseServiceProvider oseeDatabaseProvider, TransactionRecord sourceBranch,
			TransactionRecord destinationBranch, TransactionRecord mergeBranch, Collection<ChangeItem> changeData) {
	...
	private LoadChangeDataOperation(IOseeDatabaseServiceProvider oseeDatabaseProvider,
	...
	private void loadArtifactItemIdsBasedOnGammas(IProgressMonitor monitor, int queryId,
			HashMap<Integer, ChangeItem> changesByItemId) throws OseeDataStoreException {
		IOseeStatement chStmt = oseeDatabaseProvider.getOseeDatabaseService().getStatement();
		String query = "select art_id, txj.gamma_id from osee_artifact_version id, osee_join_transaction txj where id.gamma_id = txj.gamma_id and txj.query_id = ?";
		...
		try {
			...
			while (chStmt.next()) {
				...
				ArtifactChangeItem changeItem = new ArtifactChangeItem(chStmt.getLong("gamma_id"), txsTableData
						.getSecond(), txsTableData.getFirst(), chStmt.getInt("art_id"));
	...
	private void loadAttributeItemIdsBasedOnGammas(IProgressMonitor monitor, int queryId,
			HashMap<Integer, ChangeItem> changesByItemId) throws OseeDataStoreException {
		IOseeStatement chStmt = oseeDatabaseProvider.getOseeDatabaseService().getStatement();
		String query = "select art_id, attr_id, value, txj.gamma_id from osee_attribute id, osee_join_transaction txj where id.gamma_id = txj.gamma_id and txj.query_id = ?";
		...
		try {
			...
			while (chStmt.next()) {
				checkForCancelledStatus(monitor);
				Pair<Integer, ModificationType> txsTableData = changeByGammaId.get(chStmt.getLong("gamma_id"));
				AttributeChangeItem changeItem = new AttributeChangeItem(chStmt.getLong("gamma_id"), txsTableData
						.getSecond(), txsTableData.getFirst(), chStmt.getInt("attr_id"), chStmt.getInt("art_id"),
	...
	private void loadRelationItemIdsBasedOnGammas(IProgressMonitor monitor, int queryId,
			HashMap<Integer, ChangeItem> changesByItemId) throws OseeDataStoreException {
		IOseeStatement chStmt = oseeDatabaseProvider.getOseeDatabaseService().getStatement();
		String query = "select a_art_id, b_art_id, rel_link_id, rel_link_type_id, rationale, txj.gamma_id from osee_relation_link id, osee_join_transaction txj where id.gamma_id = txj.gamma_id and txj.query_id = ?";
		...
		try {
			...
			while (chStmt.next()) {
				checkForCancelledStatus(monitor);
				Pair<Integer, ModificationType> txsTableData = changeByGammaId.get(chStmt.getLong("gamma_id"));
				RelationChangeItem changeItem = new RelationChangeItem(chStmt.getLong("gamma_id"), txsTableData
						.getSecond(), txsTableData.getFirst(), chStmt.getInt("a_art_id"), chStmt.getInt("b_art_id"),
						chStmt.getInt("rel_link_id"), chStmt.getInt("rel_link_type_id"), chStmt.getString("rationale"));
	...
	private void loadByItemId(IProgressMonitor monitor, String tableName, String columnName,
			HashMap<Integer, ChangeItem> changesByItemId, String columnValueName) throws OseeCoreException {
	...
	private void loadCurrentData(IProgressMonitor monitor, String tableName, String columnName, IdJoinQuery idJoin,
			TransactionRecord destinationTransaction, HashMap<Integer, ChangeItem> changesByItemId)
		...
		try {
			switch (loadChangesEnum) {
			case FROM_ALL_BRANCH_TRANSACTIONS:
				query = "select txs.transaction_id, txs.gamma_id, txs.mod_type, item."
						+ columnName
						+ " from osee_join_id idj, " //
						+ tableName
						+ " item, osee_txs txs, osee_tx_details txd where idj.query_id = ? and idj.id = item."
						+ columnName
						+ //
	...
	private void loadNonCurrentSourceData(IProgressMonitor monitor, String tableName, String columnName,
			IdJoinQuery idJoin, HashMap<Integer, ChangeItem> changesByItemId, String columnValueName)
			throws OseeCoreException {
		IOseeStatement chStmt = oseeDatabaseProvider.getOseeDatabaseService().getStatement();
		String query;
		...
		try {
			query = "select "
					+ (columnValueName != null ? "item." + columnValueName + ", item." + columnName : "item."
							+ columnName)
					+ ", txs.gamma_id, txs.mod_type, txd.tx_type, txs.transaction_id from osee_join_id idj, " //
					+ tableName
					+ " item, osee_txs txs, osee_tx_details txd where idj.query_id = ? and idj.id = item."
					+ columnName
					+ //
			...
			while (chStmt.next()) {
				...
				int itemId = chStmt.getInt(columnName);
				boolean isBaseline = TransactionDetailsType.toEnum(chStmt.getInt("tx_type")) == TransactionDetailsType.Baselined;
				ChangeItem change = changesByItemId.get(itemId);
	...
	private void loadVersionData(IOseeStatement chStmt, ChangeVersion versionedChange, String columnValueName)
			throws OseeArgumentException, OseeDataStoreException {
