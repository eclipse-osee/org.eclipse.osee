...
public class AttributeDataStore {
   ...
   private static final String SELECT_ATTRIBUTE_BY_TAG =
         "select attr1.art_id, attr1.gamma_id, attr1.VALUE, attr1.uri, attrtype.tagger_id, txd1.branch_id from osee_define_attribute attr1, osee_define_attribute_type attrtype, osee_search_tags ost1, osee_define_txs txs1, osee_define_tx_details txd1 where attrtype.attr_type_id = attr1.attr_type_id AND ost1.gamma_id = attr1.gamma_id AND ost1.gamma_id = txs1.gamma_id AND txd1.transaction_id = txs1.transaction_id " + "AND 1 = (SELECT count(1) FROM osee.osee_join_search_tags jq1 WHERE jq1.query_id = ? AND ost1.coded_tag_id = jq1.coded_tag_id)";
   ...
   private static String getQuery(int branchId, Options options) {
      ...
      if (options.getBoolean("include deleted")) {
         toReturn.append(" AND txs1.tx_current <> 0");
      } else {
