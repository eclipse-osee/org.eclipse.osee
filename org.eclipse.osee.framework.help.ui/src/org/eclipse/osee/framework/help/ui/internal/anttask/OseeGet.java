/** 
 * Non-caching version of org.apache.tools.ant.taskdefs.Get
 */
...
/**
 * Gets a particular file from a URL source. Options include verbose reporting, timestamp based fetches and controlling
 * actions on failures. NB: access through a firewall only works if the whole Java runtime is correctly configured.
 * 
 * @since Ant 1.1
 * @ant.task category="network"
 */
public class OseeGet extends Task {
   ...
   /**
    * make a get request, with the supplied progress and logging info. All the other config parameters are set at the
    * task level, source, dest, ignoreErrors, etc.
    * 
    * @param logLevel level to log at, see {@link Project#log(String, int)}
    * @param progress progress callback; null for no-callbacks
    * @return true for a successful download, false otherwise. The return value is only relevant when
    *         {@link #ignoreErrors} is true, as when false all failures raise BuildExceptions.
    * @throws IOException for network trouble
    * @throws BuildException for argument errors, or other trouble when ignoreErrors is false.
    */
    ...
    * <p>
    * In this situation, the if-modified-since header is set so that the file is only fetched if it is newer than the
    * local file (or there is no local file) This flag is only valid on HTTP connections, it is ignored in other cases.
    * When the flag is set, the local copy of the downloaded file will also have its timestamp set to the remote file
    * time.
    * </p>
    * <p>
    * Note that remote files of date 1/1/1970 (GMT) are treated as 'no timestamp', and web servers often serve files
    * with a timestamp in the future by replacing their timestamp with that of the current time. Also, inter-computer
    * clock differences can cause no end of grief.
    * </p>
    * 
