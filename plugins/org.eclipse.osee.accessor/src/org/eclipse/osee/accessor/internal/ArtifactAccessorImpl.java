...
public class ArtifactAccessorImpl<T extends ArtifactAccessorResult> implements ArtifactAccessor<T> {
   ...
   public int getAllByPrefixFilterAndCount(BranchId branch, String filter, Collection<AttributeTypeId> attributes,
      ...
      return (int) results.stream().filter(a -> nameMatchesFilter(a, filterLower)).count();
   }
   ...
   public Collection<T> getAllByRelationThroughPartialFilter(BranchId branch, LinkedList<RelationTypeSide> relations,
      ...
      if (pageCount != 0L && pageSize != 0L) {
         int start = (int) ((pageCount - 1L) * pageSize);
         int end = Math.min(filtered.size(), start + (int) pageSize);
