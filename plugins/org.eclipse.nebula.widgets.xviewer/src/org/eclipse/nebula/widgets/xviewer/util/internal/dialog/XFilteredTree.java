...
public class XFilteredTree extends Composite {
   ...
   private TreeItem getFirstMatchingItem(TreeItem[] items) {
      for (int i = 0; i < items.length; i++) {
         if (patternFilter.isLeafMatch(treeViewer,
            items[i].getData()) && patternFilter.isElementSelectable(items[i].getData())) {
            return items[i];
         }
   ...
   protected void createFilterText(Composite parent) {
      ...
      filterText.getAccessible().addAccessibleListener(new AccessibleAdapter() {
         ...
         public void getName(AccessibleEvent e) {
            ...
            if (filterTextString.length() == 0 || filterTextString.equals(initialText)) {
               ...
               e.result = NLS.bind(XViewerText.get("XFilteredTree.matches"), //$NON-NLS-1$
      ...
      filterText.addListener(SWT.FocusOut, e -> {
         if (!useNewLook) {
            return;
         }
      ...
      if (useNewLook) {
         filterText.addListener(SWT.MouseDown, e -> {
            if (filterText.getText().equals(initialText)) {
   ...
   protected void textChanged() {
      narrowingDown = previousFilterText == null || previousFilterText.equals(
         XViewerText.get("XFilteredTree.filter")) || getFilterString().startsWith( //$NON-NLS-1$
            previousFilterText);
   ...
   private void createClearTextNew(Composite parent) {
      ...
      if ((filterText.getStyle() & SWT.ICON_CANCEL) == 0) {
         ...
         clearButton.addListener(SWT.MouseEnter, e -> clearButton.setImage(activeImage));

         clearButton.addListener(SWT.MouseExit, e -> clearButton.setImage(inactiveImage));
