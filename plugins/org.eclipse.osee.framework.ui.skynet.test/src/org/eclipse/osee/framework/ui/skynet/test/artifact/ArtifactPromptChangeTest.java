...
public class ArtifactPromptChangeTest {
   ...
   private static String DISP_NAME = "displayName";

   @Test
   public void test() {
      boolean persist = true;
      boolean mulitLine = false;
      Collection<? extends Artifact> artifacts = null;
      IBasicArtifact<?> user = null;

      Assert.assertFalse(ArtifactPromptChange.promptChangeAttribute(user, new MockAccessControlService(),
         new TestPromptChange("attributeNam", persist), ATTR_NAME, DISP_NAME, artifacts, persist, mulitLine));

      Assert.assertTrue(ArtifactPromptChange.promptChangeAttribute(user, new MockAccessControlService(),
         new TestPromptChange(ATTR_NAME, persist), ATTR_NAME, DISP_NAME, artifacts, persist, mulitLine));
   }
