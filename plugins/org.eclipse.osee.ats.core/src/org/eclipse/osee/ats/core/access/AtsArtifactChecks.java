...
public class AtsArtifactChecks extends ArtifactCheck {
   ...
   public XResultData isRenamable(Collection<ArtifactToken> artifacts, XResultData results) {
      ...
      for (ArtifactToken artifact : artifacts) {
         if (atsApi.getStoreService().isOfType(artifact,
            AtsArtifactTypes.Task) && atsApi.getAttributeResolver().getAttributesToStringList(artifact,
               CoreAttributeTypes.StaticId).contains(ChangeReportTasksUtil.AUTO_GENERATED_STATIC_ID)) {
   ...
   private String isWorkflowOrActionPermittedByAnyone(AtsApi atsApi, ArtifactToken art, Collection<ArtifactToken> allArtifacts) {
      if (atsApi.getStoreService().isOfType(art, AtsArtifactTypes.Action)) {
      ...
      if (atsApi.getStoreService().isOfType(art, AtsArtifactTypes.TeamWorkflow)) {
   ...
   private IStatus checkAtsWorkDefinitions(boolean isAtsAdmin, AtsApi atsApi, Collection<ArtifactToken> artifacts, XResultData results) {
      for (ArtifactToken art : artifacts) {
         ...
         if (atsApi.getStoreService().isOfType(art, AtsArtifactTypes.WorkDefinition)) {
