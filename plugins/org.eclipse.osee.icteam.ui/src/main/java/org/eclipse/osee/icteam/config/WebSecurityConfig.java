...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public LdapAuthenticationProvider ldapAuthenticationProvider() {
        LdapAuthenticationProvider ldapAuthenticationProvider = new LdapAuthenticationProvider(authentificator(),
                authPopulator());
    ...
    public DefaultSpringSecurityContextSource contextSource() {
        DefaultSpringSecurityContextSource contextSource = new DefaultSpringSecurityContextSource(Arrays.asList(
                    this.serverName), this.searchBase);
        String decryptedPwd = new String(Base64.decodeBase64(
                    this.ldapPassword.getBytes()));
    ...
    protected void configure(final HttpSecurity http) throws Exception {
        http.httpBasic().and().authorizeRequests()
            .antMatchers("index.html", "/", "/login", "/home", "/inline.**.js",
            "/polyfills.**.js", "/styles.**", "/iCTeam_logo.png",
            "/bosch_logo.png", "/bg-**.png", "/bg_**.png", "/vendor.**.js",
            "/main.**.js", "/glyphicons-halflings-regular.*", "/scripts.**.js",
            "/runtime.**.js").permitAll().anyRequest().authenticated().and()
