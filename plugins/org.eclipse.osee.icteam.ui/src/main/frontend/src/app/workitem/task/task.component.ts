...
export class TaskComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe((taskview: Params) => {
      ...
      this.workitemService.getTaskDetails(task).subscribe(
        (result) => {
          ...
          console.log(
            "User list before filter---->",
            this.userListBeforeFilter
          );
          let userData = [];
          ...
          this.taskDetails.attributeMap.states.push(
            this.taskDetails.currentState
          );
  ...
  submitCommnet(commetData: any) {
    ...
    commmntArtifact.attributeMap["userID"] = new Array(
      this.userDetails.username
    );
    commmntArtifact.attributeMap["comment"] = new Array<String>(commetData);
    commmntArtifact.attributeMap["currentState"] = new Array<String>(
      this.taskDetails.currentStateString
    );
  ...
  getAllTaskLinks(taskGuid: any) {
    let task: ProjectModel = new ProjectModel();
    task.guid = taskGuid;
    this.workitemService.getAllTasksLinked(task).subscribe(
      (result) => {
  ...
  deleteLink(link: any) {
    ...
    this.workitemService.deleteLink(task).subscribe(
      (result) => {
  ...
  onSubmit(taskData: NgForm) {
    ...
    for (let key in taskData.form.value) {
      ...
      if (key === "1152921504606847192") {
        ...
        taskDetails.attributeMap["toState"] = new Array(
          taskData.form.value[key]
        );
        taskDetails.attributeMap["toStateUserId"] = new Array(
          this.userDetails.username
        );
        taskDetails.attributeMap["asUserId"] = new Array(
          this.userDetails.username
        );
      ...
      if (key === "1152921504606847165;Date") {
        if (taskData.form.value[key] != null) {
          ...
          taskData.form.value[key] = new Date(
            taskData.form.value[key].year,
            taskData.form.value[key].month - 1,
            taskData.form.value[key].day
          ).toUTCString();
        } else taskData.form.value[key] = "NIL";
      }

      if (key === "sprint") {
        this.taskDetails.listVersionsDropDown.forEach((version) => {
          if (version.guid === taskData.form.value[key]) {
            taskDetails.relationMap[
          ...
          if (taskData.form.value[key] == "Product_Backlog") {
            taskDetails.attributeMap["Product_Backlog"] = new Array(
              taskData.form.value[key].toString()
            );
          }
        });
        continue;
      ...
      if (taskData.form.value[key]) {
        taskDetails.attributeMap[key] = new Array(
          taskData.form.value[key].toString()
        );
      }
    }
    ...
    this.workitemService.updateTask(taskDetails).subscribe(
      (result: any) => {
