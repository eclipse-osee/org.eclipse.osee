...
export class SprintViewComponent implements OnInit, OnDestroy {
  ...
  createNewSprint(task: any): SprintData {
    ...
    const selusersIdList = task.attributeMap["ats.Current State"][0]
      .split(";")[1]
      .split("><");
    ...
    selusersIdList[selusersIdList.length - 1] = selusersIdList[
      selusersIdList.length - 1
    ]
      .replace("<", "")
      .replace(">", "");
  ...
  updateStatus(updatedValue: any, taskDetails: any, resone: any) {
    ...
    artifact.attributeMap["asUserId"] = new Array<String>(
        ...
        if (response.list[0].attributeMap["status"][0] === "failed") {
          for (let i = 0; i <= this.dataSource.data.length - 1; i++) {
            ...
            if (dataTaskGuid === taskGuid) {
              this.dataSource.data[i].status =
                response.list[0].attributeMap["CurrentState"][0];
              break;
            }
          }
        } else {
  ...
  onUserSelect(item: any) {
    ...
    console.log(
      this.selectedUsers.map((option) =>
        this.usersList.find((o) => o.id === option.id)
      )
    );
  }
