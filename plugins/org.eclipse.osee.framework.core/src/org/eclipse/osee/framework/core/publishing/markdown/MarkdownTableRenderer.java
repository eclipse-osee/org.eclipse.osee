...
public class MarkdownTableRenderer {
    ...
    * @return PNG image bytes, or null if the rendered PDF has no pages
   ...
   private String getCssStyles() {
      try {
         return OseeInf.getResourceContents("markdown/" + CSS_RESOURCE + ".css", MarkdownTableRenderer.class);
      } catch (Exception ex) {
         // Fallback to basic table styling if resource not found
         return getDefaultTableCss();
      }
   }

   private static String getDefaultTableCss() {
      return "body { font-family: Calibri, Arial, sans-serif; font-size: 11pt; margin: 8px; }\n" //
         + "table { border-collapse: collapse; width: auto; }\n" //
         + "th, td { border: 1px solid #333; padding: 4px 8px; text-align: left; }\n" //
         + "th { background-color: #D9E2F3; font-weight: bold; }\n" //
         + "code { font-family: Consolas, monospace; background-color: #f4f4f4; padding: 1px 3px; }\n" //
         + "pre { font-family: Consolas, monospace; background-color: #f4f4f4; padding: 8px; }\n";
   }
}
