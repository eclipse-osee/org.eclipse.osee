...
public enum OseeSql {
   ...
   CHANGE_TX_ATTRIBUTE_IS("SELECT art.art_type_id, att.art_id, att.attr_id, att.gamma_id, att.attr_type_id, att.value as is_value, att.uri, txs_att.mod_type FROM osee_txs txs_art, osee_txs txs_att, osee_attribute att, osee_artifact art WHERE txs_art.branch_id = ? and txs_art.tx_current = 1 AND txs_art.gamma_id = art.gamma_id AND txs_att.branch_id = txs_art.branch_id and txs_att.transaction_id = ? AND txs_att.gamma_id = att.gamma_id AND att.art_id = art.art_id"),
