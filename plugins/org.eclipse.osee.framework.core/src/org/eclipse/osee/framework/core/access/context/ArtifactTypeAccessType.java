...
public class ArtifactTypeAccessType implements AccessType {
   ...
   public ArtifactTypeAccessType(AllowDeny allowDeny, ArtifactTypeToken... artifactTypes) {
      ...
      this.artifactTypes = Collections.asHashSet(artifactTypes);
   }
   ...
   public String toString() {
      return "ArtType [allow=" + allowDeny + ", artTypes=" + artifactTypes + "]";
   }
   ...
   public AccessTypeMatch computeMatch(ArtifactToken otherArt, AttributeTypeToken attrType, RelationTypeToken relType, IParentProvider parentProvider) {
      ...
      for (ArtifactTypeToken artType : artifactTypes) {
         if (otherArt.getArtifactType().inheritsFrom(artType)) {
            match = true;
            break;
         }
      }
