...
public interface UserService2 {
   ...
   OseeUser getUser(Long accountId);

   OseeUser getUser(ArtifactId accountId);
    ...
    */
   OseeUser getUserByUserId(String userId);
    ...
    */
   OseeUser getUserIfLoaded();

   OseeUser getUserIfLoaded(Long accountId);

   Collection<OseeUser> getUsers(IUserGroupArtifactToken userGroup);
   ...
   default boolean isActiveUser() {
      try {
         OseeUser userToken = this.getUser();
   ...
   default boolean isLoginUser() {
      try {
         ...
         List<String> loginIdsList = userToken.getLoginIds();
   ...
   OseeUser getUser(UserId userTok);

   Collection<OseeUser> getUsers();

   OseeUser getCurrentUser();

   default public Collection<OseeUser> getActiveUsers() {
      return getUsers().stream().filter(user -> user.isActive()).collect(Collectors.toList());
   }

   String getSafeUserName(UserId user);

   OseeUser create(UserToken user);
