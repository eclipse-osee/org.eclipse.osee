...
import org.eclipse.osee.framework.core.publishing.markdown.MarkdownTableToPdfImage;
...
public class MarkdownExcelCellRenderer {
   ...
   private final MarkdownTableToPdfImage tableRenderer;
   ...
   public MarkdownExcelCellRenderer(OrcsApi orcsApi, BranchId branch) {
      ...
      this.tableRenderer = new MarkdownTableToPdfImage();
   }
   ...
   private static class ImageLinkRef {
      ...
      ImageLinkRef(String artifactId) {
   ...
   private List<ImageLinkRef> extractImageLinks(String content) {
      ...
      while (matcher.find()) {
         links.add(new ImageLinkRef(matcher.group(2)));
      }
      return links;
   }
      ...
      for (ContentSegment segment : segments) {
         switch (segment.type) {
            ...
            case TABLE:
               ...
               if (tableImage != null && tableImage.length > 0) {
   ...
   private ImageEmbed resolveImageLink(String artifactIdStr) {
      try {
         ...
         try (InputStream inputStream = imageArt.getSoleAttributeValue(CoreAttributeTypes.NativeContent)) {
            imageData = Lib.inputStreamToBytes(inputStream);
         }
         ...
         int[] dims = getImageDimensions(imageData);
         if (dims.length == 0) {
         ...
         return new ImageEmbed(imageData, pictureType, width, height);
