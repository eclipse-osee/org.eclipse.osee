...
public class MimIcdGenerator {
            ...
            for (InterfaceStructureToken struct : structures) {
               ...
               if (structureInfoMap.values().stream().anyMatch(a -> a.sheetName.equals(
                  ...
                  if ((sheetName + "_" + hash).length() > 31) {
                     sheetName = sheetName.substring(1, sheetName.length() - hash.length() + 1) + "_" + hash;
                  } else {
   ...
   private void writeStructureNamesSheet(ExcelWorkbookWriter writer, List<InterfaceStructureToken> structureTokens,
      ConnectionValidationResult validation, List<ArtifactReadable> messages) {
         ...
         if (structure.getArtifactReadable().isValid()) {
            subMessage = new InterfaceSubMessageToken(structure.getArtifactReadable().getRelated(
               CoreRelationTypes.InterfaceSubMessageContent_SubMessage).getList().stream().filter(
                  a -> messages.stream().anyMatch(b -> b.getRelated(
                     CoreRelationTypes.InterfaceMessageSubMessageContent_SubMessage).getList().contains(a))).collect(
