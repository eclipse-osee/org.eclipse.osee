...
public class MimIcdGenerator {
   ...
   public void runOperation(OutputStream outputStream, BranchId branch, ArtifactId view, ArtifactId connectionId,
      ...
      createStructureInfo(branch, parentBranch, view, messages);
         ...
         for (InterfaceSubMessageToken subMessage : subMessages) {
            ...
            for (InterfaceStructureToken struct : structures) {
               ...
               if (!struct.isAutogenerated() && struct.getArtifactId().isValid() && structDiffItem != null) {
                  InterfaceStructureToken parentStructure =
                     interfaceStructureApi.get(parentBranch, struct.getArtifactId(), view);
                  ...
                  for (int i = 0; i < struct.getElements().size(); i++) {
                     ...
                     InterfaceStructureElementToken parentElement =
                        parentStructure.getElements().size() > i ? parentStructure.getElements().get(i) : null;
                     if (parentElement == null) {
                        byteChangeIndex = i;
                        break;
                     }
                     if (element.getElementSizeInBytes() != parentElement.getElementSizeInBytes()) {
                        byteChangeIndex = i;
                        break;
                     }
                  }
               ...
               StructureInfo structureInfo =
                  new StructureInfo(struct.getName(), struct.getNameAbbrev(), cat, msgRateText, minSim, maxSim, minBps,
                     ...
                     numElementsChanged, sizeInBytesChanged, false, byteChangeIndex, icdcn);
               if (struct.getId() == 0) {
   ...
   private int[] printDataElementRow(ExcelWorkbookWriter writer, AtomicInteger rowIndex, ArtifactReadable structure,
      ...
      CELLSTYLE byteStyle = elementAdded || elementStyle.equals(
         CELLSTYLE.GREEN) ? CELLSTYLE.GREEN : bytesChanged ? CELLSTYLE.YELLOW : CELLSTYLE.LIGHT_BLUE_XLSX;
