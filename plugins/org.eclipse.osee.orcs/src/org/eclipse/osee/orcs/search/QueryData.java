...
import org.eclipse.osee.orcs.search.ds.SelectData;
...
public final class QueryData implements QueryBuilder, HasOptions, HasBranch {
   ...
   public QueryData(QueryFactory queryFactory, QueryEngine queryEngine, CallableQueryFactoryApi artQueryFactory, OrcsTokenService tokenService, BranchToken branch) {
      this(queryFactory, queryEngine, artQueryFactory, tokenService, branch, ArtifactId.SENTINEL);
   }

   public QueryData(QueryFactory queryFactory, QueryEngine queryEngine, CallableQueryFactoryApi artQueryFactory, OrcsTokenService tokenService) {
   ...
   private void loadChildQueryData(QueryData queryData, QueryData parent) {
      ...
      for (Criteria criteria : queryData.getOnlyCriteriaSet()) {
         ...
         if (parent.hasCriteriaType(CriteriaFollowSearch.class)) {
            //this is strictly to create an invalid condition so child artWith queries have '' as order_value
            child.addCriteria(this.getAllCriteria().stream().filter(
               a -> a.getClass().equals(CriteriaFollowSearch.class)).findFirst().get());
         }
         if (this.hasCriteriaType(CriteriaAttributeSort.class)) {
            //this is strictly to create an invalid condition so child artWith queries have '' as order_value
            child.addCriteria(new CriteriaAttributeSort(-1L));
         }
      }
