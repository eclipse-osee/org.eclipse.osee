...
public class ArtifactEventTest {
   ...
   private void testArtifactRelationEvents__deleteArtifact(Artifact newArt) throws Exception {
      ...
      Set<EventBasicGuidArtifact> artifacts = listener.getArtifacts();
      Assert.assertEquals(3, artifacts.size());
      Assert.assertEquals(1, listener.getRelations().size());
      boolean deletedFound = false;
      boolean modifiedFound = false;
      for (EventBasicGuidArtifact guidArt1 : artifacts) {
         if (isRemoteTest()) {
            Assert.assertTrue(listener.getSender().isRemote());
         } else {
            Assert.assertTrue(listener.getSender().isLocal());
         }
         if (guidArt1.getModType() == EventModType.Deleted && newArt.getGuid().equals(guidArt1.getGuid())) {
            deletedFound = true;
            Assert.assertEquals(newArt.getArtifactType(), guidArt1.getArtifactType());
            Assert.assertTrue(newArt.isOnSameBranch(guidArt1));
         }
         if (guidArt1.getModType() == EventModType.Modified && newArt.getGuid().equals(guidArt1.getGuid())) {
            modifiedFound = true;
            Assert.assertEquals(newArt.getArtifactType(), guidArt1.getArtifactType());
            Assert.assertTrue(newArt.isOnSameBranch(guidArt1));
         }
   ...
   private Artifact remoteInjection_attributes_modifyName() throws Exception {
      ...
      int nameAttrId = injectArt.getAttributes().iterator().next().getIdIntValue();
   ...
   private Artifact remoteInjection_attributes_deleteAttribute(Artifact injectArt) throws Exception {
      ...
      RemoteAttributeChange1 remAttrChg = new RemoteAttributeChange1();
      int genStrAttrId =
         injectArt.getAttributes(CoreAttributeTypes.GeneralStringData).iterator().next().getIdIntValue();
