...
describe('OSEEAuthHeaderInterceptor', () => {
  ...
  it('should be created', () => {
    const interceptor: OSEEAuthHeaderInterceptor = TestBed.inject(OSEEAuthHeaderInterceptor);
  ...
  it('should add a header', () => {
    ...
    const request=httpTestingController.expectOne(req => (req.headers.has('osee.account.id') && req.headers.get('osee.account.id') === '0'))
    request.flush({});
  ...
  describe('should not add a header', () => {
    ...
    it("shouldn't send an account header to the authentication api", () => {
      ...
      const request=httpTestingController.expectOne(req => (!req.headers.has('osee.account.id')))
      request.flush({});
