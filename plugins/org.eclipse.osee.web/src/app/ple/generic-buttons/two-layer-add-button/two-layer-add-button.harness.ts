...
export class TwoLayerAddButtonHarness extends ComponentHarness{
    static hostSelector = 'two-layer-add-button';
    ...
    private _getNestedButtonContainer = this.locatorForOptional('.nested-add-buttons-container');
    private _getBaseButtonContainer = this.locatorForOptional('.base-add-button-container');
    ...
    async clickItem(options?: TwoLayerAddButtonNestedButtonHarnessFilters) {
        ...
        const buttons = options && await this.locatorForAll(TwoLayerAddButtonNestedButtonHarness.with(options))() || [] as TwoLayerAddButtonNestedButtonHarness[];
        if (!buttons.length) {
            throw Error(`Could not find item matching ${JSON.stringify(options)}`)
        }
        return buttons[0].click();

    }
}
export interface TwoLayerAddButtonNestedButtonHarnessFilters extends BaseHarnessFilters{
    text?: string | RegExp;
}
export class TwoLayerAddButtonNestedButtonHarness extends ComponentHarness{
    ...
    static with(options: TwoLayerAddButtonNestedButtonHarnessFilters) {
        return new HarnessPredicate(TwoLayerAddButtonNestedButtonHarness, options)
