...
export const addButtonIconTransition = trigger('addButton', [
	...
	transition(
		'open <=> closed',
		animate('{{time}}ms {{delay}}ms cubic-bezier(0.42, 0.0, 0.58, 1)')
	),
]);
export const slidingAddButtonAnim = trigger('slidingAddButton', [
	state(
		'hidden',
		style({
			transform: 'translate3d(0, {{hiddenYTranslation}}, 0)',
			display: 'block',
		}),
		{ params: { hiddenYTranslation: '90px' } }
	),
	state(
		'visible',
		style({
			transform: 'translate3d(0, {{visibleYTranslation}}, 0)',
			display: 'block',
		}),
		{ params: { visibleYTranslation: '-90px' } }
	),
	transition(
		'hidden=>*',
		animate(
