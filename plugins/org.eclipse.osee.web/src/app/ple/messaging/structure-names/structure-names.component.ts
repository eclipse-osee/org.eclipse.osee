...
export class StructureNamesComponent implements OnInit {
  ...
  names = combineLatest([this.structureService.names, this._filter]).pipe(
    map(([names,filter])=>filter!==""?names.filter((path)=>path.name.toLowerCase().includes(filter) ||path.paths.map((p)=>p.name.toLowerCase()).flat().includes(filter)):names)
  )
  basePath = combineLatest([this.uiService.id, this.uiService.type]).pipe(
    switchMap(([id, type]) => of(`../../../${type}/${id}`))
  );
  loading = this.loadingService.isLoading;
  constructor (private uiService: UiService, private structureService: CurrentStructureNamesService, private route: ActivatedRoute, private loadingService : HttpLoadingService) { 
  }
  ...
  applyFilter(event: Event) {
    this._filter.next((event.target as HTMLInputElement).value.toLowerCase());
  }
}
