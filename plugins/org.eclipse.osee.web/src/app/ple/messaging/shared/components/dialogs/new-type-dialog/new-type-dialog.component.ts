...
} from '../../../types/logicaltype.d';
import {
	logicalTypefieldValue,
	newPlatformTypeDialogReturnData,
} from '../../../types/newTypeDialogDialogData';
...
export class NewTypeDialogComponent implements OnInit {
		...
		tap((x) => {
			this.fields.push({
				...
				value:
					...
					Object.entries(this.preFillData[0])
						.map((v) => v[0])
						.some(
							(key) =>
								key.toLowerCase() ===
								x.attributeType.toLowerCase()
						)
						? Object.entries(this.preFillData[0]).filter(
								(entry) =>
									entry[0].toLowerCase() ===
									x.attributeType.toLowerCase()
						  )[0][1]
	...
	private _attributes = new BehaviorSubject(
		new Map<string, logicalTypefieldValue>()
	);

	enumSets = combineLatest([this._attributeList, this._enumSets]).pipe(
		switchMap(([attributes, enumSets]) =>
			of({
				bitSize: attributes.get('Bit Size') || '0',
				enumSets: enumSets,
			}).pipe(
				...
				.pipe(
					...
					filter(
						(logicalType) =>
							...
							this.preFillData.length > 0 &&
							logicalType.name ===
								this.preFillData[0].interfaceLogicalType
					),
					take(1),
					map((lt) => {
						this.setType(lt.id);
						this.type = lt.id;
						this._typeName = lt.name;
					})
				)
				.subscribe();
		}
	...
	get ReturnData() {
		...
		let returnValue: newPlatformTypeDialogReturnData = {
			...
			enumSetDescription:
				(this.enumSet?.description || '') + enumdescription,
			enumSetApplicability: this.enumSet?.applicability || {
				id: '1',
				name: 'Base',
			},
	...
	get isUnique() {
		return combineLatest([
			this.createNewEnum,
			this._attributesUnique,
			this._enumsUnique,
		]).pipe(
			switchMap(([newEnum, attributes, enums]) =>
				iif(
					() => newEnum,
					of([attributes, enums]).pipe(
						switchMap(([attr, enums]) =>
							iif(
								() => attr === '' && enums === true,
								of(true),
								of(false)
