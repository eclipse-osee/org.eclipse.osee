...
export class NewTypeFormComponent implements OnInit, OnChanges {
	...
	private _attributesUnique = this._formValues.pipe(
		...
		switchMap((values) =>
			of(values).pipe(
				concatMap((v) =>
					from(v.entries()).pipe(
						filter(
							([attribute, value]) =>
								value !== '' &&
								attribute !== ATTRIBUTETYPEID.NAME
						),
						map(
							([attribute, value]) =>
								new andQuery(
									attribute as ATTRIBUTETYPEID,
									value
								)
						)
					)
				...
				switchMap((results) =>
					iif(() => results.length > 0, of(true), of(false))
				)
			)
		)
	);
	...
	@Output() attributesUnique = combineLatest([
		this._attributesUnique,
		this.nameUnique,
	]).pipe(
		switchMap(([attributes, name]) =>
			iif(
				() => name || attributes,
				of(
					'Either the Name is not unique or the type already exists - please use types search page'
				),
				of('')
			)
		)
	...
	@Output() stepComplete = combineLatest([
		this.formInfo,
		this._formValues,
	]).pipe(
		switchMap(([formInfo, values]) =>
			iif(
				() =>
					values.size >=
					formInfo.fields.filter((v) => v.required).length,
				of(values).pipe(
	...
	updatedFormValue(event: logicalTypeFieldInfo, index: number) {
		...
		currentAttr.set(event.attributeType, {
			name: event.attributeType,
			value: event.value || '',
		});
