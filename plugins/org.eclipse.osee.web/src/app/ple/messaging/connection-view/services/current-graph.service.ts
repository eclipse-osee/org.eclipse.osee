...
export class CurrentGraphService {
	...
	private parseIntoNodesAndEdges(
		...
		changes.forEach((change) => {
			...
			if (
				change.itemTypeId === ARTIFACTTYPEID.CONNECTION &&
				...
				change.itemTypeId === ARTIFACTTYPEID.NODE &&
				!newConnectionsId.includes(change.artId) &&
				!newNodesId.includes(change.artId)
			) {
				//deleted node
	...
	private parseNodeChange(
		...
		if (change.changeType.name === changeTypeEnum.ATTRIBUTE_CHANGE) {
			...
			if (change.itemTypeId === ATTRIBUTETYPEID.NAME) {
				...
				change.itemTypeId === ATTRIBUTETYPEID.INTERFACENODEADDRESS
			) {
				node.data.changes.interfaceNodeAddress = changes;
			} else if (
				change.itemTypeId === ATTRIBUTETYPEID.INTERFACENODEBGCOLOR
			) {
	...
	private parseEdgeChange(
		...
		if (change.changeType.name === changeTypeEnum.ATTRIBUTE_CHANGE) {
			...
			if (change.itemTypeId === ATTRIBUTETYPEID.NAME) {
				...
				change.itemTypeId === ATTRIBUTETYPEID.INTERFACETRANSPORTTYPE
			) {
