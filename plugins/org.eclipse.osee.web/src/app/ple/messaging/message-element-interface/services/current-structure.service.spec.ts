...
import {
	structuresMock,
	structuresMock2,
} from '../../shared/mocks/structure.mock';
...
describe('CurrentStateService', () => {
	...
	it('should create a structure and get a transactionResultMock back', () => {
		scheduler.run(() => {
			...
			let expectedMarble = '(a|)';
			scheduler
				.expectObservable(service.createStructure(structuresMock[0]))
				.toBe(expectedMarble, expectedObservable);
		});
	});
	...
	it('should get available structures', () => {
		scheduler.run(() => {
			let expectedObservable = {
				...
				c: [structuresMock[0], structuresMock[0], structuresMock[0]],
				d: [
					structuresMock[0],
					structuresMock[0],
					structuresMock[0],
	...
	it('should return a structure', () => {
		scheduler.run(({ cold, expectObservable }) => {
			...
			cold(delayMarble).subscribe(() => (service.update = true));
			expectObservable(service.getStructureRepeating('abcdef')).toBe(
				expectedMarble,
				expectedObservable
			);
		});
