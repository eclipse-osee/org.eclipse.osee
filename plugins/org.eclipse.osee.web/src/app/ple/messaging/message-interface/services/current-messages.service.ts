...
export class CurrentMessagesService {
	...
	parseIntoMessagesAndSubmessages(
		...
		changes.forEach((change) => {
			...
			if (
				change.itemTypeId === ARTIFACTTYPEID.SUBMESSAGE &&
				...
				change.itemTypeId === ARTIFACTTYPEID.MESSAGE &&
				!newMessagesId.includes(change.artId) &&
				!newSubmessagesId.includes(change.artId)
			) {
				//deleted message
	...
	parseMessageDeletionChange(
		...
		if (change.changeType.name === changeTypeEnum.ATTRIBUTE_CHANGE) {
			...
			if (change.itemTypeId === ATTRIBUTETYPEID.NAME) {
				...
				change.itemTypeId === ATTRIBUTETYPEID.INTERFACEMESSAGENUMBER
			) {
				...
				change.itemTypeId === ATTRIBUTETYPEID.INTERFACEMESSAGERATE
			) {
				message.changes.interfaceMessageRate = changes;
			} else if (
				change.itemTypeId === ATTRIBUTETYPEID.INTERFACEMESSAGETYPE
			) {
	...
	parseSubMessageChange(
		...
		if (change.changeType.name === changeTypeEnum.ATTRIBUTE_CHANGE) {
			...
			if (change.itemTypeId === ATTRIBUTETYPEID.NAME) {
				...
				change.itemTypeId === ATTRIBUTETYPEID.INTERFACESUBMESSAGENUMBER
			) {
