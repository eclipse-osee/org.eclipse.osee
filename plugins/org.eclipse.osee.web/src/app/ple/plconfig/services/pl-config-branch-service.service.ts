...
import {
	cfgGroup,
	PlConfigBranchListingBranch,
} from '../types/pl-config-branch';
...
export class PlConfigBranchService {
	...
	public getBranches(
		type: string
	): Observable<PlConfigBranchListingBranch[]> {
		return this.http.get<PlConfigBranchListingBranch[]>(
			apiURL + '/ats/ple/branches/' + type
		);
	}
	...
	public getApplicabilityToken(
		...
		if (this.applicabilityTagCache.has(applicablityToken)) {
			return of(
				this.applicabilityTagCache.get(
					applicablityToken
				) as NameValuePair
			);
		} else {
