...
export class DialogService {
	...
	openConfigMenu(header: string, editable: string) {
		...
		return combineLatest([
			this.currentBranchService.isACfgGroup(header),
			this.currentBranchService.findViewByName(header),
			this.currentBranchService.branchApplicability,
		]).pipe(
			take(1),
			switchMap(([isCfgGroup, view, applicability]) =>
				iif(
					() => isCfgGroup === false && view !== undefined,
					of<[PlConfigApplicUIBranchMapping, view | undefined]>([
						applicability,
						view,
					]).pipe(
						take(1),
						filter(
							([app, view]) => (view as view) !== undefined
						) as OperatorFunction<
							[PlConfigApplicUIBranchMapping, view | undefined],
							[PlConfigApplicUIBranchMapping, view]
						>,
						switchMap(([app, view]) =>
							this.dialog
								.open<
								...
								>(EditConfigurationDialogComponent, {
									data: new PLEditConfigData(
										...
										isEditable,
										app.groups.filter((a) =>
											a.configurations.includes(view.id)
										)
									),
									minWidth: '60%',
								})
					...
					this.currentBranchService.findGroup(header).pipe(
						...
						switchMap((group) =>
							of(group).pipe(
								take(1),
								mergeMap((findViews) =>
									from(findViews.configurations).pipe(
										switchMap((config) =>
											this.currentBranchService.findViewById(
												config
											)
										)
									)
						...
						switchMap((dialogPrep) =>
							of({
								editable: editable === 'true',
								configGroup: dialogPrep,
							}).pipe(
								switchMap((group: CfgGroupDialog) =>
									this.dialog
										.open<
										...
										.pipe(
											...
											filter(
												...
												CfgGroupDialog | undefined,
												CfgGroupDialog
											>,
											switchMap((dialogResult) =>
												iif(
													() => dialogResult.editable,
													from(
														dialogResult.configGroup
															.views
													).pipe(
														reduce(
															(acc, curr) => [
