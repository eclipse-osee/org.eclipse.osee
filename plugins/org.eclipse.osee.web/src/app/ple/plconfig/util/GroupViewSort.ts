...
class sortedViews implements sortedViewsInterface {
	...
	findGroup(groupString: string) {
		return (
			this.groups.find((val) => val.name === groupString)?.getGroup() || {
				name: '',
				id: '',
				views: [],
			}
...
export class GroupViewSorter {
	...
	addGroup(group: configGroup) {
		let tempGroup = this.groupList.find(
			(val) => val.id === group.id && val.name === group.name
		);
		if (tempGroup) {
			tempGroup = group;
		} else {
	...
	addView(view: view) {
		let tempView = this.viewList.find(
			(val) => val.id === view.id && val.name === view.name
		);
		if (tempView) {
			tempView = view;
		} else {
	...
	sort() {
		...
		this.viewList.forEach((element) => {
			if (!this.getGroupListForView(element)) {
				...
				this.addViewToGrouping(
					element,
					this.getGroupListForView(element)
				);
			}
		});
	...
	resetDefaultGrouping() {
		let defaultView = this.viewObj.groups.find(
			(val) => val.name === 'No Group'
		);
	...
	addViewToDefaultGrouping(view: view) {
		let defaultGroup = this.viewObj.groups.find(
			(val) => val.name === 'No Group'
		);
	...
	addViewToGrouping(view: view | configGroup, groupName: string): void {
		let grouping = this.viewObj.groups.find(
			(val) => val.name === groupName
		);
		if (grouping) {
			let existingGrouping = grouping.views.find(
				(el) => el.id === view.id
			);
			if (existingGrouping) {
			...
			this.viewObj.groups.push(
				new group(
					groupName,
					undefined,
					this.getGroupFromName(groupName),
					this.getGroupFromName(groupName)?.id
				)
			);
			let newGroup = this.viewObj.groups.find(
				(val) => val.name === groupName
			);
			if (
				newGroup &&
				!newGroup.views.find((val) => val.name === view.name)
			) {
	...
	getSortedArrayOfConfigurations() {
		...
		this.viewObj.groups.forEach((value) => {
			...
			sortedArr.push(
				...value.views.map((a) =>
					(a as view).hasFeatureApplicabilities ? a.name : ''
				)
			);
		});
