...
} from '../../../services/action-state-button.service.mock';
...
describe('ActionDropDownComponent', () => {
  ...
  describe('Normal Conditions', () => {
    ...
    it('should transition to review', async () => {
      ...
      const btn = await loader.getHarness(
        MatButtonHarness.with({ text: new RegExp('Transition To Review') })
      );
    ...
    describe('non-component tests', () => {
      it('should add an action', () => {
        let dialogRefSpy = jasmine.createSpyObj({
          afterClosed: of(new PLConfigCreateAction(testDataUser)),
          close: null,
        });
        let dialogSpy = spyOn(
  ...
  describe('Non-Approved state', () => {
    ...
    it('should approve working branch', async () => {
      ...
      const btn = await loader.getHarness(
        MatButtonHarness.with({ text: new RegExp('Approve Working Branch') })
      );
      expect(btn).toBeDefined();
      await btn.click();
      expect(spy).toHaveBeenCalled();
  ...
  describe('Pre-commit state', () => {
    ...
    it('should commit working branch', async () => {
      ...
      const btn = await loader.getHarness(
        MatButtonHarness.with({ text: new RegExp('Commit Working Branch') })
      );
      expect(btn).toBeDefined();
      await btn.click();
      expect(spy).toHaveBeenCalled();
