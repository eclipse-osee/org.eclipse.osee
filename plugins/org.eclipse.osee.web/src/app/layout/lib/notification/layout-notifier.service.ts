...
export class LayoutNotifierService {
	...
	private _smallMatchStrings = range(0, 9600, asyncScheduler).pipe(
		...
		mergeMap(([num1, num2]) =>
			iif(
				...
				of(
					`(min-width: ${(num1 as number) / 10}px) and (max-width: ${
						(num2 as number) / 10
					}px)`
				),
				of(`(max-width: ${(num2 as number) / 100}px)`)
			).pipe()
		),
	...
	private _isXLarge = this.observer
	...
	private _recommendedCardColumnCount = combineLatest([
		this.isXSmall,
		this.isSmall,
		this.isMedium,
		this.isLarge,
	...
	private _tableColumnRecommendations: Observable<tableRecommendations> =
		combineLatest([
			this.isXSmall,
			this.isSmall,
			this.isMedium,
			...
			switchMap(([xsmall, small, medium, large, xlarge]) =>
				iif(
					() => xsmall,
					of({ width: '80px', columns: 4 }),
					iif(
						() => small,
						of({ width: '120px', columns: 4 }),
						iif(
							() => medium,
							of({ width: '137px', columns: 7 }),
							iif(
								() => large,
								of({ width: '300px', columns: 9 }),
								iif(
									() => xlarge,
	...
	private _tableColumnRecommendationColumnCount =
		this.recommendedTableColumns.pipe(map((x) => x.columns));
	private _pageState: Observable<pageState> = combineLatest<
		[
			boolean,
			boolean,
			boolean,
	...
	>([
		...
		this.isLarge,
		this.isXLarge,
		this.recommendedCardColumnCount,
		this.recommendedTableColumns,
	]).pipe(
		switchMap(
			([
				xsmall,
				...
				of({
					...
					large: large,
					xlarge: xlarge,
					recommendedCardColumnCount: cardColumnCount,
