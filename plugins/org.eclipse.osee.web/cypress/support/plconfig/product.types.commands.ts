...
Cypress.Commands.add(
  ...
  (
    ...
    values.forEach(({ name, enabled }) => {
      if (includeFlags) {
        cy.get(`[data-cy="field-product-type-${name}-${enabled}"]`).should(
          'exist'
        );
      } else if (enabled) {
        cy.get(`[data-cy="field-product-type-${name}"]`).should('exist');
      }
    });
  ...
  (
    ...
    isValues.forEach(({ name, enabled }, index) => {
      ...
      if (enabled !== wasValues[index].enabled) {
        ...
        // ).click();
        cy.selectProductTypes([{ name:name, enabled:enabled,currentState:wasValues[index].enabled }]);
      }
    });
    return cy.get('[data-cy="product-types-selection"]');
  }
