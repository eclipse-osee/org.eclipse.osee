...
Cypress.Commands.add(
  ...
  (
    ...
    newValues.values.forEach((value, index) => {
      cy.get(`[data-cy="field-value-${index}"]`).focus().clear().type(value);
    });
    cy.get('[data-cy="field-defaultValue"]')
      ...
      .click();
    return cy
      .editProductTypes(previousValues.productTypes, newValues.productTypes)
  ...
  (feature: {
    ...
    if (feature.productTypes) {
      feature.productTypes.forEach((productType) => {
        cy.get(`[data-cy="field-product-type-${productType}-false"]`).click();
      });
    }
    return cy
      .get('[data-cy="submit-btn"]')
