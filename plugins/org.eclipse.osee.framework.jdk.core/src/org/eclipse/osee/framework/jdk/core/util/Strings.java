...
public class Strings {
	...
	/**
	 * This method adjusts '&'-containing strings to break the keyboard shortcut
	 * ("Accelerator") feature some widgets offer, where &Test will make Alt+T a
	 * shortcut. This method breaks the accelerator by escaping ampersands.
	 * 
	...
	/**
	 * Will truncate string if necessary and add "..." to end if addDots and
	 * truncated
	 */
	public static String truncate(String value, int length, boolean addDots) {
		...
		if (Strings.isValid(value) && value.length() > length) {
			...
			toReturn = value.substring(0, Math.min(length, len))
					+ (addDots ? "..." : emptyString());
		}
		return toReturn;
	}
