...
public class ICTeamMailNotifier {
     ...
     */
    public static void notify(final OrcsApi orcs, final ArtifactReadable awa,
        final List<String> users, final List<String> _mailIds,
        final String updates, final String link,
        final ICTeamNotifyType... notifyTypes) {
        update = updates;
        ...
        List<String> mailIds = new ArrayList<String>(new HashSet<String>(
                    _mailIds));
        ...
        try {
            ...
            if (types.contains(ICTeamNotifyType.Subscribed)) {
                if (mailIds.size() > 0) {
                    ICTeamEmail emailMessage = new ICTeamEmail(mailIds,
                            fromAddresss, fromAddresss,
                            getNotificationEmailSubject(), getBody(),
                            BodyType.Html);
                    emailMessage.send();
                }
            }

            if (types.contains(ICTeamNotifyType.Cancelled) ||
                    types.contains(ICTeamNotifyType.Completed)) {
                if (mailIds.size() > 0) {
                    ICTeamEmail emailMessage = new ICTeamEmail(mailIds,
                            fromAddresss, fromAddresss,
                            getNotificationEmailSubject(), getBody(),
        ...
        try {
            ...
            if (types.contains(ICTeamNotifyType.Originator)) {
                ...
                buffer.append(AHTML.simplePage("Status : Subscribed"));
                    ...
                    types.contains(ICTeamNotifyType.predecessor)) {
                buffer.append(AHTML.simplePage(
                        "Status : Predecessor Task Completed"));
            } else if (types.contains(ICTeamNotifyType.Cancelled) &&
                    types.contains(ICTeamNotifyType.predecessor)) {
                buffer.append(AHTML.simplePage(
                        "Status : Predecessor Task Cancelled"));
            } else if (types.contains(ICTeamNotifyType.Cancelled)) {
            ...
            } else if (types.contains(ICTeamNotifyType.Updated) &&
                    !types.contains(ICTeamNotifyType.predecessor)) {
            ...
            if (artifact.getArtifactType().getName().equals("Team Workflow")) {
                ...
                ResultSet<ArtifactReadable> relatedProjects = artifact.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project);
    ...
    public static StringBuffer getContentForProject() {
        StringBuffer buffer = new StringBuffer();
        ArtifactReadable projectArtifact = artifact.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                                   .getExactlyOne();
        String shortName = projectArtifact.getAttributes(AtsAttributeTypes.Shortname)
                                          .getExactlyOne().toString();
        String workPackageId = artifact.getAttributes(AtsAttributeTypes.WorkPackage)
                                       .getExactlyOne().toString();
        String taskId = shortName + "-" + workPackageId;
        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("Task ID:" + taskId));
        ...
        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("Packages:          " + componentArt.getName()));

        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("CreatedBy:          " +
                getUserNameByUserId(artifact.getSoleAttributeAsString(
                        AtsAttributeTypes.CreatedBy))));

        if (assignees.isEmpty()) {
            List<ITransferableArtifact> assignees2 = CommonUtil.getAssignees(artifact.getSoleAttributeAsString(
                        AtsAttributeTypes.CurrentState));
            List<String> userList = new ArrayList<String>();

            for (ITransferableArtifact transferableArtifact : assignees2) {
                userList.add(transferableArtifact.getAttributes(
                        CoreAttributeTypes.UserId.toString()).get(0));
            }
        ...
        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("Assignee:          " + stringBuffer.toString()));
        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("Status:          " +
                artifact.getSoleAttributeAsString(
                    AtsAttributeTypes.CurrentState)
                        .substring(0,
                    artifact.getSoleAttributeAsString(
                        AtsAttributeTypes.CurrentState).indexOf(";"))));

        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("Request Type:          " +
                artifact.getSoleAttributeAsString(AtsAttributeTypes.ChangeType)));
        ...
        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("Team:          " + teamArt.getName()));

        buffer.append(AHTML.addSpace(5) +
            AHTML.simplePage("Description:          " +
                artifact.getSoleAttributeAsString(AtsAttributeTypes.Description)));
     ...
     */
    private static ArtifactReadable getTeamDefinitionByGuid(
        final String teamDefId) {
        ResultSet<ArtifactReadable> list = null;

        try {
            list = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON)
                          .andIsOfType(AtsArtifactTypes.TeamDefinition)
                          .andUuid(Long.valueOf(teamDefId)).getResults();
     ...
     */
    private static ArtifactReadable getActionalBleItemsByGuid(
        final String actionableItemId) {
        ResultSet<ArtifactReadable> list = null;

        try {
            list = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON)
                          .andIsOfType(AtsArtifactTypes.ActionableItem)
                          .andUuid(Long.valueOf(actionableItemId)).getResults();
     ...
     * @return
     */
    public static String createTable(final List<String> datas,
        final String[] headers, final int numColumns, final int cellPadding,
        final int border, final StringBuffer table) {
        if (datas == null) {
    ...
    public static void startBorderTable(final boolean active,
        final String caption, final StringBuffer buffer) {
        buffer.append(startBorderTable(TABLE_WIDTH,
                active ? activeColor : normalColor, caption));
    }

    public static String startBorderTable(final int width,
        final String bgcolor, final String caption) {
        String capStr = "";
        ...
        return "<table border=\"1\" align=\"left\" bgcolor=\"" + bgcolor +
     ...
     * @return
     */
    public static String addHeaderRowMultiColumnTable(final String[] str,
        final Integer[] width) {
    ...
    private static String getNotificationEmailSubject() {
        ...
        if (types.contains(ICTeamNotifyType.Originator)) {
            ...
            buffer = "ICTeam Notification : Subscribed - " +
                artifact.getName();
        } else if (types.contains(ICTeamNotifyType.Completed) &&
                types.contains(ICTeamNotifyType.predecessor)) {
            buffer = "ICTeam Notification : Predecessor Task Completed - " +
                artifact.getName();
        } else if (types.contains(ICTeamNotifyType.Cancelled) &&
                types.contains(ICTeamNotifyType.predecessor)) {
            buffer = "ICTeam Notification : Predecessor Task Cancelled - " +
                artifact.getName();
        } else if (types.contains(ICTeamNotifyType.Cancelled)) {
            ...
            buffer = "ICTeam Notification : Review Completed - " +
                artifact.getName();
        } else if (types.contains(ICTeamNotifyType.Updated) &&
                !types.contains(ICTeamNotifyType.predecessor)) {
            buffer = "ICTeam Notification : Updated - " + artifact.getName();
            ...
            buffer = "ICTeam Notification : User Created - " +
                artifact.getName();
        } else if (types.contains(ICTeamNotifyType.AdminUserCreated)) {
            buffer = "ICTeam Notification : Admin User Created - " +
                artifact.getName();
        } else if (types.contains(ICTeamNotifyType.PasswordChangeRequest)) {
            buffer = "ICTeam Notification : Password Modification Request Created - " +
                artifact.getName();
        }
     ...
     * @param rapLink
     */
    public static void notify(final OrcsApi orcsApi2,
        final ArtifactReadable readableArtifact,
        final String currentLoggedInUser, final String mailId,
        final String changes, final String rapLink, final ICTeamNotifyType type) {
        ...
        if (assignees.size() > 0) {
            ICTeamEmail emailMessage = new ICTeamEmail(assignees, senderMailId,
                    fromAddresss, getNotificationEmailSubject(),
                    getUserMailBody(), BodyType.Html);
            emailMessage.send();
        }
    }
     ...
     * @return
     */
    private static String getMailIdByType(final OrcsApi orcsApi2,
        final String currentLoggedInUser) {
        String buffer = "";

        if (types.contains(ICTeamNotifyType.UserCreated) ||
                types.contains(ICTeamNotifyType.AdminUserCreated)) {
