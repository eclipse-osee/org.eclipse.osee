...
public class TeamsResource  extends AbstractConfigResource{
    ...
    /**
     * This function is used to create a new Team artifact with all the attribute
     * values and relations sent from Client
     *
     * @param json String which contains artifact, its attributes values and
     *             relation
    ...
    public String createTeamWeb(String json) {
        try {
            ...
            for (String projectGuid : projectDetails) {
                project = orcsApi.getQueryFactory()
                                 .fromBranch(CoreBranches.COMMON)
                                 .andUuid(Long.valueOf(projectGuid)).getResults()
                                 .getExactlyOne();
            }
            ...
            TransactionBuilder tx = txFactory.createTransaction(CoreBranches.COMMON,
                    userId, "Add New Team");
            childArtifact = tx.createArtifact(AtsArtifactTypes.TeamDefinition,
                    artifact.getName());

            ResultSet<ArtifactReadable> results = orcsApi.getQueryFactory()
                                                         .fromBranch(CoreBranches.COMMON)
                                                         .andNameEquals("Teams")
                                                         .getResults();
            tx.addChild(results.getExactlyOne(), childArtifact);
            tx.relate(project,
                AtsRelationTypes.ProjectToTeamDefinition_TeamDefinition,
                childArtifact);
            ...
            for (String projectUser : projectUsers) {
                ResultSet<ArtifactReadable> projectMember = orcsApi.getQueryFactory()
                                                                   .fromBranch(CoreBranches.COMMON)
                                                                   .andUuid(Long.valueOf(
                            projectUser)).getResults();
                tx.relate(project, AtsRelationTypes.ProjectToUser_Project,
                    projectMember.getExactlyOne());
            }

            for (String team : teamMembers) {
                ResultSet<ArtifactReadable> teamMember = orcsApi.getQueryFactory()
                                                                .fromBranch(CoreBranches.COMMON)
                                                                .andUuid(Long.valueOf(
                            team)).getResults();
                tx.relate(childArtifact, AtsRelationTypes.TeamMember_Member,
                    teamMember.getExactlyOne());
                tx.relate(project, CoreRelationTypes.Users_User,
                    teamMember.getExactlyOne());
            }

            for (String teamLead : teamLeads) {
                ResultSet<ArtifactReadable> teamLeader = orcsApi.getQueryFactory()
                                                                .fromBranch(CoreBranches.COMMON)
                                                                .andUuid(Long.valueOf(
                            teamLead)).getResults();
                tx.relate(childArtifact, AtsRelationTypes.TeamLead_Lead,
                    teamLeader.getExactlyOne());
                tx.relate(project, CoreRelationTypes.Users_User,
                    teamLeader.getExactlyOne());

                ArtifactReadable atsAdmin = orcsApi.getQueryFactory()
                                                   .fromBranch(CoreBranches.COMMON)
                                                   .andIsOfType(CoreArtifactTypes.UserGroup)
                                                   .andNameEquals("AtsAdmin")
                                                   .getResults().getExactlyOne();
                ArtifactReadable oseeAdmin = orcsApi.getQueryFactory()
                                                    .fromBranch(CoreBranches.COMMON)
                                                    .andIsOfType(CoreArtifactTypes.UserGroup)
                                                    .andNameEquals("OseeAdmin")
                                                    .getResults().getExactlyOne();
                tx.relate(atsAdmin, CoreRelationTypes.Users_User,
                    teamLeader.getExactlyOne());
                tx.relate(oseeAdmin, CoreRelationTypes.Users_User,
                    teamLeader.getExactlyOne());
            }
            ...
            if ((attributes2 != null) && (attributes2.size() > 0)) {
                tx.setSoleAttributeFromString(childArtifact,
                    AtsAttributeTypes.WorkflowDefinition, attributes2.get(0));
            }

            tx.setSoleAttributeFromString(childArtifact,
                AtsAttributeTypes.TeamUsesVersions, "true");
            tx.commit();
            ...
            TranferableArtifactLoader.copyArtifactReadbleToTransferableArtifact(readableArtifact,
                ar);
    ...
    public String updateTeamWeb(String json) {
        try {
            ...
            UserId userId = UserId.valueOf(currentUser.getId());
            TransactionFactory txFactory = orcsApi.getTransactionFactory();
            TransactionBuilder tx = txFactory.createTransaction(CommonUtil.getCommonBranch(
                        orcsApi), userId, "Update Release Artifact");
            ...
            if (attribute != null) {
                ...
                for (ArtifactReadable artifactReadable : list) {
                    if (artifactReadable.getGuid().equals(attribute)) {
                        List<String> projectDetails = artifact.getAttributes(
                                "Project");
                        List<String> teamMembers = artifact.getAttributes(
                                "TeamMembers");
                        List<String> teamLeads = artifact.getAttributes(
                                "TeamLeads");
                        tx.unrelateFromAll(AtsRelationTypes.ProjectToTeamDefinition_Project,
                            artifactReadable);
                        tx.unrelateFromAll(AtsRelationTypes.TeamMember_Team,
                            artifactReadable);
                        tx.unrelateFromAll(AtsRelationTypes.TeamLead_Team,
                            artifactReadable);
                        ...
                        for (String project : projectDetails) {
                            ResultSet<ArtifactReadable> projects = orcsApi.getQueryFactory()
                                                                          ...
                                                                          .andGuid(project)
                                                                          .getResults();
                            tx.relate(projects.getExactlyOne(),
                                AtsRelationTypes.ProjectToTeamDefinition_Project,
                                artifactReadable);
                            relatedUserForProject = projects.getExactlyOne()
                                                            .getRelated(CoreRelationTypes.Users_User);
                            tx.unrelateFromAll(AtsRelationTypes.ProjectToUser_Project,
                                projects.getExactlyOne());
                            ...
                            for (Object s : st) {
                                if (projectUsers.indexOf(s) != projectUsers.lastIndexOf(
                                            s)) {
                                    projectUsers.remove(projectUsers.lastIndexOf(
                                            s));
                                }
                            }
                            ...
                            for (String projectUser : projectUsers) {
                                ResultSet<ArtifactReadable> projectMember = orcsApi.getQueryFactory()
                                                                                   ...
                                                                                   .getResults();
                                tx.relate(projects.getExactlyOne(),
                                    AtsRelationTypes.ProjectToUser_Project,
                                    projectMember.getExactlyOne());
                            }
                            ...
                            for (ArtifactReadable removeUser : removeUserFromProject) {
                                tx.unrelate(projects.getExactlyOne(),
                                    CoreRelationTypes.Users_User, removeUser);
                            }
                        }
                        ...
                        for (String teamLead : teamLeads) {
                            ...
                            for (ArtifactReadable artifactReadable2 : teamLeader) {
                                tx.relate(artifactReadable,
                                    AtsRelationTypes.TeamLead_Team,
                                    artifactReadable2);
                                ...
                                for (ArtifactReadable artifactReadable3 : atsAdminList) {
                                    if (artifactReadable3.getGuid()
                                                             .equals(artifactReadable2.getGuid())) {
                                        isAtsAdmin = true;
                                    }
                                }
                        ...
                        List<String> attributes2 = artifact.getAttributes(AtsAttributeTypes.WorkflowDefinition.getId()
                                                                                                              .toString());

                        if ((attributes2 != null) && (attributes2.size() > 0)) {
                            tx.setSoleAttributeFromString(artifactReadable,
                                AtsAttributeTypes.WorkflowDefinition,
                                attributes2.get(0));
                        }

                        tx.setSoleAttributeFromString(artifactReadable,
                            CoreAttributeTypes.Name, artifact.getName());
                        tx.commit();

                        ArtifactReadable ai = orcsApi.getQueryFactory()
                                                     .fromBranch(CoreBranches.COMMON)
                                                     .andGuid(attribute)
                                                     .excludeDeleted()
                                                     .getResults()
    ...
    public String getTeamsForProject(final String guidFromUi)
        ...
        TransferableArtifactsContainer container = new TransferableArtifactsContainer();
        final ResultSet<ArtifactReadable> teamArtifacts = projectArtifact.getRelated(AtsRelationTypes.ProjectToTeamDefinition_TeamDefinition);
        ...
        if (teamArtifacts != null) {
            for (ArtifactReadable artifactReadable : teamArtifacts) {
                ...
                TranferableArtifactLoader.copyBasicInfoToTransferableArtifact(artifactReadable,
                    ar);
                artifactReadable = orcsApi.getQueryFactory()
                                          .fromBranch(CoreBranches.COMMON)
                                          .andGuid(artifactReadable.getGuid())
                                          .andIsOfType(AtsArtifactTypes.TeamDefinition)
                                          .getResults().getExactlyOne();

                ResultSet<?extends AttributeReadable<Object>> attributes = artifactReadable.getAttributes(AtsAttributeTypes.WorkflowDefinition);
                ...
                for (ArtifactReadable artLeads : relatedLeads) {
                    ...
                    TranferableArtifactLoader.copyBasicInfoToTransferableArtifact(artLeads,
                        arChild);

                    AttributeReadable<Object> exactlyOne = artLeads.getAttributes(CoreAttributeTypes.UserId)
                                                                   .getExactlyOne();
                ...
                ar.putRelations(AtsRelationTypes.TeamLead_Lead.getName(),
                    artLeadList);
                ...
                for (ArtifactReadable artMem : relatedMembs) {
                    TransferableArtifact arChild = new TransferableArtifact();
                    TranferableArtifactLoader.copyBasicInfoToTransferableArtifact(artMem,
                        arChild);

                    AttributeReadable<Object> exactlyOne = artMem.getAttributes(CoreAttributeTypes.UserId)
                                                                 .getExactlyOne();
                    arChild.putAttributes(CoreAttributeTypes.UserId.getName(),
                        Arrays.asList(exactlyOne.getValue().toString()));
                ...
                ar.putRelations(AtsRelationTypes.TeamMember_Member.getName(),
                    artMembers);
                listTras.add(ar);
            }
