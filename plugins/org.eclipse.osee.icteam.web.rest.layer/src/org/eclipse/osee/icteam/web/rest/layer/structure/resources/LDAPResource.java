 ...
 *
 * LDAP resource providing LDAP user details
 */
@Path("LDAP")
public class LDAPResource {
    ...
    public String getLDAPUserDetails(String json) {
        if ((json != null) && !json.equals("")) {
            String userId = json.substring(json.indexOf(UID) + UID.length() +
                    1, json.indexOf(PWD));
            String pwd = json.substring(json.indexOf(PWD) + PWD.length() + 1,
                    json.indexOf(DOMAIN));
            String domain = json.substring(json.indexOf(DOMAIN) +
                    DOMAIN.length() + 1, json.length());
