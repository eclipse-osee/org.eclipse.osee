...
public class TeamWorkflowResource extends AbstractConfigResource{
     ...
     *
     * @param artifact : artifact.getGuid --> User GUID artifact.getParentGuid() -->
     *                 Sprint/Release GUID
     *
     * @return This method will return the teamworkflow resource for a given sprint
     *         and user. Team workflow resources will also have next States for
    ...
    public String getTeamWorkflowForCurrentUserAndSprint(
        final String currentUser) throws OseeCoreException {
        ...
        TransferableArtifact artifact = gson.fromJson(currentUser,
                TransferableArtifact.class);
        ...
        ArtifactReadable sprintArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(sprintGuid,
        ...
        for (String filter : filterList) {
            QueryBuilder query = orcsApi.getQueryFactory()
                                        ...
                                        .and(AtsAttributeTypes.CurrentStateType,
                    filter, QueryOption.EXACT_MATCH_OPTIONS)
                                        .andRelatedTo(AtsRelationTypes.TeamWorkflowTargetedForVersion_Version,
                    sprintArtifact);

            if (filter.equals("Working")) {
                if ((userGuid != null) && (userGuid.length() > 0)) {
                    ArtifactReadable userArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(userGuid,
                            CoreBranches.COMMON, orcsApi);
                    query = query.and(AtsAttributeTypes.CurrentState,
                            userArtifact.getSoleAttributeAsString(
                                CoreAttributeTypes.UserId),
            ...
            ArtifactReadable mileStoneArtifact = orcsApi.getQueryFactory()
                                                        .fromBranch(CoreBranches.COMMON)
                                                        .andRelatedTo(CoreRelationTypes.DefaultHierarchical_Child,
                    sprintArtifact).getResults().getExactlyOne();
            ArtifactReadable projectArtifact = orcsApi.getQueryFactory()
                                                      .fromBranch(CoreBranches.COMMON)
                                                      .andRelatedTo(AtsRelationTypes.ProjectToVersion_Version,
                    mileStoneArtifact).getResults().getExactlyOne();
            List<ITransferableArtifact> usersFromTeam = getTeamUsers(projectArtifact);

            if ((tasks != null) && (tasks.size() > 0)) {
                for (ArtifactReadable task : tasks) {
                    IAtsWorkItem workItem = OseeCoreData.getAtsServer()
                                                        .getWorkItemService()
                                                        .getWorkItem(task);
                    List<IAtsStateDefinition> toStates = workItem.getStateDefinition()
                                                                 .getToStates();
                    ...
                    String shortname = task.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                           .getExactlyOne()
                                           .getAttributes(AtsAttributeTypes.Shortname)
                                           .getExactlyOne().toString();
                    String workPackage = task.getAttributes(AtsAttributeTypes.WorkPackage)
                                             .getExactlyOne().toString();
                    String taskId = shortname + "-" + workPackage;
                    TransferableArtifact ar = new TransferableArtifact();
                    TranferableArtifactLoader.copyBasicTaskInfoToTransferableArtifact(task,
                        ar);
                    ar.putAttributes("states", states);
                    /**
                     * The below method getUsersFromGroupWeb is commented because, Group concept is
                     * removed from ICTeam tool. But if we have to get groups into ICteam, then we
                     * have to use the method again to get the assignee for dropdowm
                     */
     ...
     *
     * @param projectArtifact ArtifactReadable Project artifact to get Team users
     *                        (Team Members and Team Leads)
     * @return assigneeList List<ITransferableArtifact> List of all Team Users of a
     *         Project
     */
    private List<ITransferableArtifact> getTeamUsers(
        ArtifactReadable projectArtifact) {
        OrcsApi orcsApi = OseeCoreData.getOrcsApi();
        ResultSet<ArtifactReadable> teams = orcsApi.getQueryFactory()
                                                   .fromBranch(CoreBranches.COMMON)
                                                   .andIsOfType(AtsArtifactTypes.TeamDefinition)
                                                   .andRelatedTo(AtsRelationTypes.ProjectToTeamDefinition_Project,
        ...
        for (ArtifactReadable user : userArtifactList) {
            ...
            TranferableArtifactLoader.copyBasicInfoToTransferableArtifact(user,
                userTemp);
            userTemp.putAttributes(CoreAttributeTypes.UserId.toString(),
                Arrays.asList(user.getAttributes(CoreAttributeTypes.UserId)
                                  .getExactlyOne().toString()));
     ...
     *
     * @param projectArtifact {@linkplain ArtifactReadable} Project artifact to get
     *                        related Teamworkflows
     * @return artifactReadables List<ArtifactReadable> List of all Teamworkflows
     *         realted to Project
     */
    private List<ArtifactReadable> getTeamworkFlowArtifacts(
        final ArtifactReadable projectArtifact) {
        List<ArtifactReadable> artifactReadables = new ArrayList<ArtifactReadable>();

        try {
            final ResultSet<ArtifactReadable> teamWorkflowArtifacts = projectArtifact.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_TeamWorkflow);
    ...
    /**
     * Method called locally to update Teamworkflow attributes and relation values
     * sent from Client
     *
    ...
    private void updateTeamWorkFlow(final ITransferableArtifact artifact) {
        ...
        TransactionBuilder tx = txFactory.createTransaction(CoreBranches.COMMON,
                UserId.valueOf(CommonUtil.getCurrentUser(orcsApi,
                        artifact.getCurrentLoggedInUser()).getId()),
                "Update Team Artifact");
        String guid = artifact.getUuid();
        ArtifactReadable teamwfartifact = CommonUtil.getArtifactFromIdExcludingDeleted(guid,
                CoreBranches.COMMON, orcsApi);
                ...
                if ((split.length == 2) && split[1].equals("Date")) {
                    if (string.equals("NIL")) {
                        tx.deleteSoleAttribute(teamwfartifact,
                            AttributeTypeToken.valueOf(Long.parseLong(split[0]),
                                "Attribute"));
                    } else {
                        ...
                        if (date != null) {
                            tx.setSoleAttributeValue(teamwfartifact,
                                AttributeTypeToken.valueOf(Long.parseLong(
                                        split[0]), "Attribute"), date);

                            if (Long.parseLong(AtsAttributeTypes.EstimatedCompletionDate.getId()
                    ...
                    if (split[0].matches(".*[1-9].*")) {
                        if (Long.parseLong(AtsAttributeTypes.CurrentState.getId()
                                                                             .toString()) == Long.parseLong(
                                    split[0])) {
                            if (!(string.contains("Cancelled") ||
                                ...
                                for (ITransferableArtifact transferableArtifact : assignees2) {
                                    users.add(transferableArtifact.getAttributes(
                                            CoreAttributeTypes.UserId.toString())
                                                                  .get(0));
                                }
                                ...
                                if (estimatedDate == null) {
                                    AttributeReadable<Object> estDate = teamwfartifact.getAttributes(AtsAttributeTypes.EstimatedCompletionDate)
                                                                                      .getAtMostOneOrNull();

                                    if (estDate != null) {
                                        if (estDate.getValue() != null) {
                                            String estdate = estDate.getValue()
                                                                    .toString();
                        ...
                        if (Long.parseLong(AtsAttributeTypes.Rank.getId()
                                                                         .toString()) == Long.parseLong(
                                    split[0])) {
                        }
                        ...
                        tx.setSoleAttributeFromString(teamwfartifact,
                            AtsAttributeTypes.Information, string);
                    } else if (split[0].equalsIgnoreCase("req.Condition")) {
                        tx.setSoleAttributeFromString(teamwfartifact,
                            AtsAttributeTypes.Condition,
                            string);
                    }
                }
            }
        ...
        if (artifact.getAttributes("Product_Backlog") != null) {
            tx.unrelateFromAll(AtsRelationTypes.TeamWorkflowTargetedForVersion_TeamWorkflow,
                teamwfartifact);
            ...
            List<ITransferableArtifact> list = artifact.getRelatedArtifacts(AtsRelationTypes.TeamWorkflowTargetedForVersion_Version.toString());
            ...
            if ((list != null) && (list.size() > 0)) {
                ...
                ArtifactReadable wfArtifact = orcsApi.getQueryFactory()
                                                     .fromBranch(CoreBranches.COMMON)
                                                     .andGuid(guid).getResults()
                                                     .getExactlyOne();
                ...
                if (((updatedReleases != null) && (updatedReleases.size() > 0)) ||
                        ((relatedVersions != null) &&
                        (relatedVersions.size() > 0))) {
                    if ((relatedVersions != null) &&
                            (relatedVersions.size() > 0)) {
                        for (TransferableArtifact transferableArtifact : relatedVersions) {
                            ArtifactReadable releaseArtifact = orcsApi.getQueryFactory()
                                                                      .fromBranch(CoreBranches.COMMON)
                                                                      .andGuid(transferableArtifact.getUuid())
                                                                      .getResults()
                                                                      .getExactlyOne();
        ...
        List<ITransferableArtifact> relatedSuperCedes = artifact.getRelatedArtifacts(CoreRelationTypes.Supercedes_Supercedes.toString());
        ...
        if (relatedSuperCedes != null) {
            for (ITransferableArtifact transferableArtifact : relatedSuperCedes) {
                ArtifactReadable tmWfArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(transferableArtifact.getUuid(),
                        CoreBranches.COMMON, orcsApi);
                tx.relate(tmWfArtifact,
                    CoreRelationTypes.Supercedes_Supercedes, teamwfartifact);
            }
        }

        List<ITransferableArtifact> relatedSuperCeded = artifact.getRelatedArtifacts(CoreRelationTypes.Supercedes_SupercededBy.toString());

        if (relatedSuperCeded != null) {
            for (ITransferableArtifact transferableArtifact : relatedSuperCeded) {
                ArtifactReadable tmWfArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(transferableArtifact.getUuid(),
                        CoreBranches.COMMON, orcsApi);
                tx.relate(teamwfartifact,
                    CoreRelationTypes.Supercedes_SupercededBy, tmWfArtifact);
            }
        }

        List<ITransferableArtifact> relatedAgileTask = artifact.getRelatedArtifacts(
                "agileTaskLink");
        ...
        if (relatedAgileTask != null) {
            for (ITransferableArtifact transferableArtifact : relatedAgileTask) {
                ArtifactReadable tmWfArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(transferableArtifact.getUuid(),
                        CoreBranches.COMMON, orcsApi);
                tx.relate(tmWfArtifact,
                    AtsRelationTypes.AgileTaskLink_TeamWorkflowA, teamwfartifact);
            }
        }

        List<ITransferableArtifact> relatedComponents = artifact.getRelatedArtifacts(AtsRelationTypes.ActionableItemWorkFlow_ActionableItem.toString());

        if (relatedComponents != null) {
            tx.unrelateFromAll(AtsRelationTypes.ActionableItemWorkFlow_TeamWorkflow,
                teamwfartifact);
            ...
            for (ITransferableArtifact transferableArtifact : relatedComponents) {
                ArtifactReadable tmWfArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(transferableArtifact.getUuid(),
                        CoreBranches.COMMON, orcsApi);
                tx.relate(tmWfArtifact,
                    AtsRelationTypes.ActionableItemWorkFlow_ActionableItem,
                    teamwfartifact);
            }
        }
        ...
        teamwfartifact = CommonUtil.getArtifactFromIdExcludingDeleted(teamwfartifact.getIdString(),
                CoreBranches.COMMON, orcsApi);

        String url = artifact.getAttributes("URLInfo").get(0);
        String projectGuid = teamwfartifact.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                           .getExactlyOne().getGuid();
        String rapLink = url + "/icteam-web/#/dashboard/" + projectGuid + "/" +
            teamwfartifact.getGuid();

        if (sendMailToAssignee) {
            changes = changes + "\n" +
                String.format("You have been set as assignee to the task \"%s\"",
                    teamwfartifact.getName());
            sendMail(orcsApi, createdUserId, assignees, teamwfartifact,
                estimatedDate, ICTeamNotifyType.Updated, changes, rapLink);
        }
    }
    ...
    private void updateProjectAttributes(
        final ArtifactReadable teamwfartifact, final TransactionBuilder tx,
        final String attrGuid, final String value) {
        ArtifactReadable activeProject = teamwfartifact.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                                       .getAtMostOneOrNull();
        tx.setSoleAttributeFromString(activeProject,
            AttributeTypeToken.valueOf(Long.parseLong(attrGuid), "Attribute"),
            value);
    }

    /**
     * Send mail notifications to created user and assignees of a task when a task
     * is created and task status is updated
     *
     ...
     */
    public static void sendMail(final OrcsApi orcsApi,
        final String createdUserId, final List<String> assignees,
        final ArtifactReadable teamwfartifact, final Date estimatedDate,
        final ICTeamNotifyType notify, final String changes, final String link) {
        ...
        ICTeamMailNotifier.notify(orcsApi, teamwfartifact, assignees, mailIds,
            changes, link, notify);
    }
     ...
     */
    public static ArtifactId createNewTask(final TransactionBuilder tx,
        final TransferableArtifact artifact, final OrcsApi orcsApi,
        Date estimatedDate, String notify, String createdUserId,
        final List<String> assignees, String productBacklog) {
        String artName = "";

        if (artifact.getName() == null) {
            List<String> artNameList = artifact.getAttributes(CoreAttributeTypes.Name.getId()
                                                                                     .toString());
        ...
        ArtifactId childArtifact = tx.createArtifact(AtsArtifactTypes.TeamWorkflow,
                artName);
        ArtifactId childAction = tx.createArtifact(AtsArtifactTypes.Action,
                artName);
        tx.setSoleAttributeFromString(childAction,
            AtsAttributeTypes.Description, "des");
        tx.setSoleAttributeFromString(childAction,
            AtsAttributeTypes.ChangeType, "Problem");
        tx.setSoleAttributeFromString(childAction,
            AtsAttributeTypes.Priority, "2");
            ...
            for (String string : value) {
                if ((split.length == 2) && split[1].equals("Date")) {
                    ...
                    tx.setSoleAttributeFromString(childArtifact,
                        AtsAttributeTypes.WorkflowDefinition, "WorkDef_ICTeam");
                    ...
                    if (!split[0].equals("raplink") &&
                            !split[0].equals("Product_Backlog") &&
                            !split[0].equals("ArtifactGuid") &&
                            !split[0].equals("ArtifactBranchGuid") &&
                            !split[0].equals("CreateTaskFromLinkGuid")) {
                        tx.setSoleAttributeFromString(childArtifact,
                            AttributeTypeToken.valueOf(Long.parseLong(split[0]),
                                "Attribute"), string);
                        ...
                        if (Long.parseLong(AtsAttributeTypes.CurrentState.getId()
                            ...
                            String assigneesStr = string.substring(string.indexOf(
                                        "<") + 1, string.lastIndexOf(">"));
                            String[] _assignees = ((assigneesStr != null) &&
                                !assigneesStr.isEmpty())
                                ? assigneesStr.split("><") : new String[0];
                            assignees.addAll(Arrays.asList(_assignees));
                        } else if (Long.parseLong(AtsAttributeTypes.CreatedBy.getId()
                                                                                 .toString()) == Long.parseLong(
                                    split[0])) {
         ...
         */
        String uuid = AtsRelationTypes.ProjectToTeamWorkFlow_Project.getGuid()
                                                                   .toString();
        String type = AtsRelationTypes.ProjectToTeamWorkFlow_Project.getRelationType()
                                                                   .toString();
        String side = AtsRelationTypes.ProjectToTeamWorkFlow_Project.getSide()
                                                                   .toString();
        String key = "RelationTypeSide - uuid=[" + uuid + "] type=[" + type +
            "] side=[" + side + "]";
        ...
        if ((projArtifactList != null) && !projArtifactList.isEmpty()) {
            ...
            ResultSet<ArtifactReadable> listCommon = orcsApi.getQueryFactory()
                                                            .fromBranch(BranchId.valueOf(
                        projArt.getBranchGuid()))
                                                            .andUuid(Long.valueOf(
                        projectGUID)).getResults();
            ...
            if (taskIDCountForProject != null) {
                ...
                tx.setSoleAttributeFromString(childArtifact,
                    AttributeTypeToken.valueOf(Long.parseLong(
                            AtsAttributeTypes.WorkPackage.getId().toString()),
                        "Attribute"), taskCountAttr);

                if (project.getArtifactType()
                               .equals(AtsArtifactTypes.AgileProject)) {
                    tx.setSoleAttributeFromString(childArtifact,
                        AttributeTypeToken.valueOf(Long.parseLong(
                                AtsAttributeTypes.Rank.getId().toString()),
                            "Attribute"), taskCountAttr);
                }

                ResultSet<ArtifactReadable> list = orcsApi.getQueryFactory()
                                                          .fromBranch(CoreBranches.COMMON)
                                                          .andIsOfType(AtsArtifactTypes.WorkDefinition)
                                                          .getResults();
                tx.setSoleAttributeFromString(project,
                    AttributeTypeToken.valueOf(
                        AtsAttributeTypes.TaskCountForProject.getId(),
                        "Attribute"), String.valueOf(taskCount));
            }
        }

        if (productBacklog == null) {
            String uuid1 = AtsRelationTypes.TeamWorkflowTargetedForVersion_Version.getGuid()
                                                                                  .toString();
            String type1 = AtsRelationTypes.TeamWorkflowTargetedForVersion_Version.getRelationType()
                                                                                  .toString();
            String side1 = AtsRelationTypes.TeamWorkflowTargetedForVersion_Version.getSide()
                                                                                  .toString();
            String key1 = "RelationTypeSide - uuid=[" + uuid1 + "] type=[" +
                type1 + "] side=[" + side1 + "]";
        ...
        if (relatedComponents != null) {
            for (ITransferableArtifact transferableArtifact : relatedComponents) {
                ArtifactReadable projectArt = CommonUtil.getArtifactFromIdExcludingDeleted(transferableArtifact.getUuid(),
                        CoreBranches.COMMON, orcsApi);
                tx.relate(projectArt,
                    AtsRelationTypes.ActionableItemWorkFlow_ActionableItem,
                    childArtifact);
            }
        }

        String uuid2 = AtsRelationTypes.ProjectToTeamWorkFlow_Project.getGuid()
                                                                    .toString();
        String type2 = AtsRelationTypes.ProjectToTeamWorkFlow_Project.getRelationType()
                                                                    .toString();
        String side2 = AtsRelationTypes.ProjectToTeamWorkFlow_Project.getSide()
                                                                    .toString();
        String key2 = "RelationTypeSide - uuid=[" + uuid2 + "] type=[" + type2 +
            "] side=[" + side2 + "]";
        ...
        if (relatedproject != null) {
            for (ITransferableArtifact transferableArtifact : relatedproject) {
                ArtifactReadable tmWfArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(transferableArtifact.getUuid(),
                        CoreBranches.COMMON, orcsApi);
                tx.relate(tmWfArtifact,
                    AtsRelationTypes.ProjectToTeamWorkFlow_Project, childArtifact);
            }
        }

        tx.relate(childAction, AtsRelationTypes.ActionToWorkflow_TeamWorkflow,
            childArtifact);
     ...
     * @return {@link String}
     */
    private static String getMailIdByUserId(final OrcsApi orcsApi,
        final String userId) {
        try {
            ArtifactReadable artifactReadable = UserUtility.getUserById(orcsApi,
                    userId);
    ...
    public String getNextStates(final String str) throws OseeCoreException {
        Gson gson = new GsonBuilder().registerTypeAdapter(ITransferableArtifact.class,
                new InterfaceAdapter<TransferableArtifact>()).create();
        TransferableArtifact transferableArtifact = gson.fromJson(str,
                TransferableArtifact.class);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Path("transitionWeb")
    public String transitionStateWeb(final String str)
        throws OseeCoreException {
        OrcsApi orcsApi = OseeCoreData.getOrcsApi();
        Gson gson = new GsonBuilder().registerTypeAdapter(ITransferableArtifact.class,
                new InterfaceAdapter<TransferableArtifact>()).create();
        ...
        String guid = transferableArtifact.getUuid();
        ResultSet<ArtifactReadable> rootArtifact = orcsApi.getQueryFactory()
                                                          .fromBranch(CoreBranches.COMMON)
                                                          .andUuid(Long.valueOf(
                    guid)).getResults();
        ...
        TransactionBuilder tx1 = txFactory1.createTransaction(CoreBranches.COMMON,
                UserId.valueOf(CommonUtil.getCurrentUser(orcsApi,
                        transferableArtifact.getCurrentLoggedInUser()).getId()),
                "Update Task Status");
        AttributeReadable<Object> currentStateR = parentArtifact.getAttributes(AtsAttributeTypes.CurrentState)
                                                                .getAtMostOneOrNull();
        ...
        List<String> toStateUsers = transferableArtifact.getAttributes(
                "toStateUserId");
        ...
        ArtifactReadable projectArtifact = orcsApi.getQueryFactory()
                                                  .fromBranch(CoreBranches.COMMON)
                                                  .andUuid(Long.valueOf(
                    projectGuid)).getResults().getAtMostOneOrNull();
        String cancelComment = "";
        List<String> cancelCommentAttr = transferableArtifact.getAttributes(AtsAttributeTypes.StateNotes.getId()
                                                                                                        .toString());
        ...
        if (guid.matches("[0-9]+")) {
            action = (ArtifactReadable) OseeCoreData.getAtsServer()
                                                    .getQueryService()
                                                    .getArtifact(Long.valueOf(
                        guid));
        } else {
            action = (ArtifactReadable) OseeCoreData.getAtsServer()
                                                    .getQueryService()
                                                    .getArtifactByGuid(guid);
        }

        if (toState.equalsIgnoreCase("Completed")) {
            if (!(parentArtifact.getAttributes(
                        AtsAttributeTypes.CurrentStateType).getExactlyOne()
                                    .toString().equalsIgnoreCase("Completed"))) {
                tx1.setSoleAttributeFromString(parentArtifact,
                    AtsAttributeTypes.CurrentStateType, "Completed");
                isCommit = true;
            }
        }
        ...
        if (toState.equalsIgnoreCase("Cancelled")) {
            if (!(parentArtifact.getAttributes(
                        AtsAttributeTypes.CurrentStateType).getExactlyOne()
                                    .toString().equalsIgnoreCase("Cancelled"))) {
                tx1.setSoleAttributeFromString(parentArtifact,
                    AtsAttributeTypes.CurrentStateType, "Cancelled");
                isCommit = true;
            }
        }
        ...
        if (toState.equalsIgnoreCase("InProgress")) {
            if (!(parentArtifact.getAttributes(
                        AtsAttributeTypes.CurrentStateType).getExactlyOne()
                                    .toString().equalsIgnoreCase("Working"))) {
                tx1.setSoleAttributeFromString(parentArtifact,
        ...
        IAtsWorkItem workItem = OseeCoreData.getAtsServer().getWorkItemService()
                                            .getWorkItem(action);
        AtsUser asAtsUser = OseeCoreData.getAtsServer().getUserService()
                                         .getUserByUserId(asUserId);
        IAtsChangeSet changes = OseeCoreData.getAtsServer().getStoreService()
                                            .createAtsChangeSet("transition Action",
                asAtsUser);
        TransitionHelper helper = null;

        if (admin.equals("true")) {
            helper = new TransitionHelper("Transition " + guid,
                    Collections.singleton(workItem), toState, assigneesList,
                    reason, changes, OseeCoreData.getAtsServer(),
        ...
        if (!results.isEmpty() || changes.isEmpty()) {
            ...
            JSONDeserializer<TransferableArtifactsContainer> deserializer1 = new JSONDeserializer<TransferableArtifactsContainer>();
            TransferableArtifactsContainer transCont = deserializer1.deserialize(nextStates);
            ITransferableArtifact transferableArtifact2 = transCont.getArtifactList()
                                                                   .get(0);
            transferableArtifact2.putAttributes("status",
                Arrays.asList("failed"));
            transferableArtifact2.putAttributes("message",
                Arrays.asList(results.toString()));
        ...
        ResultSet<ArtifactReadable> result = orcsApi.getQueryFactory()
                                                    .fromBranch(CoreBranches.COMMON)
                                                    .andIds(action).getResults();
        ...
        if (toState.equals("Completed") || toState.equals("Cancelled")) {
            if (toState.equals("Completed") && !(assignees.isEmpty())) {
                String completeChanges = String.format("Task \"%s\" completed by %s on %s",
                        transArtifact.getName(),
                        ICTeamMailNotifier.getUserNameByUserId(asUserId),
                        DateUtil.getMMDDYY(new Date()));
                String rapServer = transferableArtifact.getAttributes("raplink")
                                                       .get(0);
                String gUid = transArtifact.getGuid();
                String artifactType = transArtifact.getArtifactType();
                String project = action.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                       .getExactlyOne().getGuid();
                String rapLink = url + "/icteam-web/#/dashboard/" + project +
                    "/" + transArtifact.getGuid();
                List<String> assigneeMailIds = new ArrayList<String>();
                List<String> userIdList = new ArrayList<String>();

                for (ITransferableArtifact user : assignees) {
                    String userId = user.getAttributes(CoreAttributeTypes.UserId.getName())
                                        .get(0);
                ...
                ICTeamMailNotifier.notify(orcsApi, exactlyOne, userIdList,
                    assigneeMailIds, completeChanges, rapLink,
            ...
            if (toState.equals("Cancelled") && !(assignees.isEmpty())) {
                ...
                String completeChanges = String.format("Task \"%s\" cancelled by %s due to reason: %s on %s",
                        transArtifact.getName(),
                        ICTeamMailNotifier.getUserNameByUserId(asUserId),
                        reason, DateUtil.getMMDDYY(new Date()));
                String project = action.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                       .getExactlyOne().getGuid();
                String rapLink = url + "/icteam-web/#/dashboard/" + project +
                    "/" + transArtifact.getGuid();
                ...
                for (ITransferableArtifact transferableArtifact2 : assignee) {
                    String userId = transferableArtifact2.getAttributes(CoreAttributeTypes.UserId.getName())
                                                         .get(0);
                    String assigneeMailId = getMailIdByUserId(orcsApi, userId);
        ...
        if (!toState.equals("Cancelled") && !toState.equals("Completed") &&
                !(assignees.isEmpty())) {
            String createdBy = exactlyOne.getSoleAttributeAsString(AtsAttributeTypes.CreatedBy);
            String createdUserId = "";
            ...
            String completeChanges = String.format("Task \"%s\" state changed by %s  on %s",
                    transArtifact.getName(),
                    ICTeamMailNotifier.getUserNameByUserId(asUserId),
                    DateUtil.getMMDDYY(new Date()));
            String project = action.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
        ...
        if (guid.matches("[0-9]+")) {
            exactlyOne1 = orcsApi.getQueryFactory()
                                 .fromBranch(CoreBranches.COMMON)
                                 .andUuid(Long.valueOf(guid)).getResults()
                                 .getExactlyOne();
        } else {
            exactlyOne1 = orcsApi.getQueryFactory()
                                 .fromBranch(CoreBranches.COMMON).andGuid(guid)
        ...
        if ((cancelComment != null) && (cancelComment.length() > 0)) {
            ...
            TransactionBuilder tx = txFactory.createTransaction(CoreBranches.COMMON,
                    UserId.valueOf(CommonUtil.getCurrentUser(orcsApi,
                            artifact.getCurrentLoggedInUser()).getId()),
                    "Update Team Artifact");
            tx.setSoleAttributeFromString(exactlyOne1,
                AtsAttributeTypes.StateNotes, cancelComment);
            tx.commit();
        }
        ...
        if (projectArtifact != null) {
            AttributeReadable<Object> currentStateR1 = exactlyOne1.getAttributes(AtsAttributeTypes.CurrentState)
                                                                  .getAtMostOneOrNull();
            String currentStateStr1 = (String) currentStateR1.getValue();
            String[] split1 = currentStateStr1.split(";");
            String currentState1 = split1[0];
            AttributeReadable<Object> currentStateType = exactlyOne1.getAttributes(AtsAttributeTypes.CurrentStateType)
                                                                    .getAtMostOneOrNull();
            ...
            if ((teams != null) && (teams.size() > 0)) {
                for (ArtifactReadable team : teams) {
                    ...
                    for (ArtifactReadable userArtifact : artifacts) {
                        AttributeReadable<Object> userId = userArtifact.getAttributes(CoreAttributeTypes.UserId)
                                                                       .getAtMostOneOrNull();
            ...
            ResultSet<ArtifactReadable> teams1 = orcsApi.getQueryFactory()
                                                        .fromBranch(CoreBranches.COMMON)
                                                        .andIsOfType(AtsArtifactTypes.TeamDefinition)
                                                        .andRelatedTo(AtsRelationTypes.ProjectToTeamDefinition_Project,
                    projectArtifact).getResults();
            ...
            if (userIdStringList.size() > 0) {
                ...
                for (ITransferableArtifact assignee : assignees) {
                    ...
                    userString = userString + "<" +
                        assignee.getUserId().toString() + ">";
                }

                Object[] st = userIdStringList.toArray();

                for (Object s : st) {
                    if (userIdStringList.indexOf(s) != userIdStringList.lastIndexOf(
                                s)) {
                ...
                if (!currentStateStr1.equalsIgnoreCase(currentState1)) {
                    tx.setSoleAttributeFromString(exactlyOne1,
                        AtsAttributeTypes.CurrentState, currentState1);
                    tx.commit();
                }
            }
        }
    ...
    public String updateCommentforTeamWorkFlow(String json) {
        try {
            Gson gson = new GsonBuilder().registerTypeAdapter(ITransferableArtifact.class,
                    new InterfaceAdapter<TransferableArtifact>()).create();
            TransferableArtifact artifact = gson.fromJson(json,
                    TransferableArtifact.class);
            OrcsApi orcsApi = OseeCoreData.getOrcsApi();
            TransactionFactory txFactory = orcsApi.getTransactionFactory();
            TransactionBuilder tx = txFactory.createTransaction(CoreBranches.COMMON,
            ...
            String guid = artifact.getUuid();
            ResultSet<ArtifactReadable> commentArtifact = orcsApi.getQueryFactory()
                                                                 .fromBranch(CoreBranches.COMMON)
                                                                 .andGuid(guid)
                                                                 .getResults();
            String uuid = commentArtifact.getExactlyOne().getIdString();
            ArtifactReadable teamwfartifact = CommonUtil.getArtifactFromIdExcludingDeleted(uuid,
                    CoreBranches.COMMON, orcsApi);
            ...
            CommentItem noteItem = new CommentItem(CommentType.Comment.name(),
                    currentState.get(0), String.valueOf(new Date().getTime()),
                    split[0], commentList.get(0));
            String xml = "";
            ResultSet<?extends AttributeReadable<Object>> attributes = teamwfartifact.getAttributes(AtsAttributeTypes.StateNotes);
            ...
            if (xml.length() > 0) {
                tx.setSoleAttributeFromString(teamwfartifact,
                    AtsAttributeTypes.StateNotes, xml);
                tx.commit();
            }

            teamwfartifact = CommonUtil.getArtifactFromIdExcludingDeleted(uuid,
                    CoreBranches.COMMON, orcsApi);
                    ...
                    if ((commentAttributes != null) &&
                        ...
                        for (AttributeReadable<Object> comment : commentAttributes) {
                            ...
                            for (CommentItem commentItem : fromXml) {
                                ...
                                AttributeReadable<Object> exactlyOne = userFromGivenUserId.getAttributes(CoreAttributeTypes.Name)
                                                                                          .getExactlyOne();
     ...
     *
     * @param artifact {@link TransferableArtifact} to update the values for
     *                 artifact
     * @return ar {@link CustomizedTeamWorkFlowArtifact} updated teamworkflow
     *         artifact
     */
    private CustomizedTeamWorkFlowArtifact getUpdatedTeamWorkFlowForWeb(
        TransferableArtifact artifact) {
        OrcsApi orcsApi = OseeCoreData.getOrcsApi();
        ResultSet<ArtifactReadable> result = null;

        if (artifact.getUuid().matches("[0-9]+")) {
            result = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON)
                            .andUuid(Long.valueOf(artifact.getUuid()))
                            .getResults();
        } else {
            result = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON)
                            .andGuid(artifact.getUuid()).getResults();
        }
    ...
    public String getTaskDetailsByGuidWeb(String task) {
        ...
        Gson gson = new GsonBuilder().registerTypeAdapter(ITransferableArtifact.class,
                new InterfaceAdapter<TransferableArtifact>()).create();
        TransferableArtifact artifact = gson.fromJson(task,
                TransferableArtifact.class);
        String guid = artifact.getUuid();
        ArtifactReadable teamwfartifact = CommonUtil.getArtifactFromIdExcludingDeleted(guid,
                CoreBranches.COMMON, orcsApi);
        ...
        if (teamwfartifact != null) {
            ArtifactReadable projectArtifact = teamwfartifact.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                                             .getExactlyOne();
            AttributeReadable<Object> shortname = projectArtifact.getAttributes(AtsAttributeTypes.Shortname)
                                                                 .getExactlyOne();
            shortname.getValue();

            ArtifactReadable teamartifact = projectArtifact.getRelated(AtsRelationTypes.ProjectToTeamDefinition_TeamDefinition)
                                                           .getExactlyOne();

            if (teamartifact != null) {
                ResultSet<ArtifactReadable> relatedMileStones = teamartifact.getRelated(AtsRelationTypes.TeamDefinitionToVersion_Version);

                for (ArtifactReadable mileStone : relatedMileStones) {
                    ResultSet<ArtifactReadable> relatedSprints = mileStone.getRelated(CoreRelationTypes.DefaultHierarchical_Child);

                    if ((relatedSprints != null) &&
                            (relatedSprints.size() > 0)) {
                            ...
                            if ((releasedAttribute != null) &&
                                ...
                                for (AttributeReadable<Object> attributeReadable : releasedAttribute) {
                                    ...
                                    if (releasedAttributeString == false) {
                                        TransferableArtifact ar = new TransferableArtifact();
                                        TranferableArtifactLoader.copyBasicInfoToTransferableArtifact(artifactReadable2,
                                            ar);

                                        String displayName = mileStone.getName() +
                                            "-->" +
                                            artifactReadable2.getName();
                                        ar.putAttributes("displayName",
                ...
                ResultSet<ArtifactReadable> relatedPackages = teamartifact.getRelated(AtsRelationTypes.TeamActionableItem_ActionableItem);
                ...
                if ((relatedPackages != null) && (relatedPackages.size() > 0)) {
                    for (ArtifactReadable artifactReadable : relatedPackages) {
                        ...
                        TranferableArtifactLoader.copyBasicInfoToTransferableArtifact(artifactReadable,
                            arTemp1);
            ...
            List<CommentArtifact> listComment = new ArrayList<CommentArtifact>();
            ResultSet<?extends AttributeReadable<Object>> commentAttributes = teamwfartifact.getAttributes(AtsAttributeTypes.StateNotes);
            ...
            Collection<org.eclipse.osee.ats.api.data.enums.token.AgileChangeTypeAttributeType.ChangeTypeEnum> enumValues = AtsAttributeTypes.AgileChangeType.getEnumValues();
            ...
            ar.putAttributes(AtsAttributeTypes.Shortname.toString(),
                Arrays.asList(shortname.toString()));
            ar.putAttributes("WorkPackage",
                Arrays.asList(teamwfartifact.getAttributes(
                        AtsAttributeTypes.WorkPackage).getExactlyOne().toString()));

            List<ITransferableArtifact> listReview = new ArrayList<ITransferableArtifact>();
            ResultSet<ArtifactReadable> relatedReview = teamwfartifact.getRelated(AtsRelationTypes.TeamWorkflowToReview_Review);
            ...
            ar.putRelations(AtsRelationTypes.TeamWorkflowToReview_Review.getName(),
                listReview);
            ...
            if ((taginformationAttributes != null) &&
                    (taginformationAttributes.size() > 0)) {
                AttributeReadable<Object> exactlyOne = taginformationAttributes.getExactlyOne();
                String string = exactlyOne.getValue().toString();
                ar.putAttributes(AtsAttributeTypes.Information.getName(),
            ...
            if ((reqConditionAttributes != null) &&
                    (reqConditionAttributes.size() > 0)) {
                AttributeReadable<Object> exactlyOne = reqConditionAttributes.getExactlyOne();
                String string = exactlyOne.getValue().toString();
                ar.putAttributes(AtsAttributeTypes.Condition.getName(),
                    Arrays.asList(string));
            } else {
                ar.putAttributes(AtsAttributeTypes.Condition.getName(),
                    Arrays.asList(""));
    ...
    public String getTeamWorkflowForProjectBackLog(String json)
        throws OseeCoreException {
        OrcsApi orcsApi = OseeCoreData.getOrcsApi();
        Gson gson = new GsonBuilder().registerTypeAdapter(ITransferableArtifact.class,
                new InterfaceAdapter<TransferableArtifact>()).create();
        TransferableArtifact artifact = gson.fromJson(json,
                TransferableArtifact.class);
        String guid = artifact.getUuid();
        ArtifactReadable projectArtifact = CommonUtil.getArtifactFromIdExcludingDeleted(guid,
                CoreBranches.COMMON, orcsApi);
        String shortName = projectArtifact.getAttributes(AtsAttributeTypes.Shortname)
                                          .getExactlyOne().toString();
        TransferableArtifactsContainer container = new TransferableArtifactsContainer();
        ...
        if (teamworkFlowArtifacts != null) {
            for (ArtifactReadable artifactReadable : teamworkFlowArtifacts) {
                ResultSet<ArtifactReadable> relatedSprints = artifactReadable.getRelated(AtsRelationTypes.TeamWorkflowTargetedForVersion_Version);

                if (relatedSprints.size() == 0) {
                    String workPackageId = artifactReadable.getAttributes(AtsAttributeTypes.WorkPackage)
                                                           .getExactlyOne()
                                                           .toString();
                    String taskId = shortName + "-" + workPackageId;
                    TransferableArtifact ar = new TransferableArtifact();
                    ArrayList<String> states = getNextStateList(artifactReadable.getGuid());
                    TranferableArtifactLoader.copyBasicTaskInfoToTransferableArtifact(artifactReadable,
                        ar);
                    ar.putAttributes("states", states);
                    ar.putAttributes("TaskId", Arrays.asList(taskId));

                    List<ITransferableArtifact> usersFromGroupWeb = getAssigneesForCombo(projectArtifact.getIdString(),
                            ar.getUuid());
    ...
    public String getTeamWorkFlowFromGuidWeb(final String task)
        throws OseeCoreException {
        OrcsApi orcsApi = OseeCoreData.getOrcsApi();
        Gson gson = new GsonBuilder().registerTypeAdapter(ITransferableArtifact.class,
                new InterfaceAdapter<TransferableArtifact>()).create();
        TransferableArtifact artifact = gson.fromJson(task,
                TransferableArtifact.class);
        ArtifactReadable teamwfartifact = CommonUtil.getArtifactFromIdExcludingDeleted(artifact.guid,
                CoreBranches.COMMON, orcsApi);
        List<ITransferableArtifact> listTras = new ArrayList<ITransferableArtifact>();
        ...
        if (teamwfartifact != null) {
            ...
            TranferableArtifactLoader.copyArtifactReadbleToTransferableArtifact(teamwfartifact,
                ar);
            listTras.add(ar);
        }
    ...
    public String createTask(final String json) {
        ...
        try {
            ...
            List<String> workspaceArtifact = artifact.getAttributes(
                    "ArtifactGuid");
            String artifactGuid = null;

            if ((workspaceArtifact != null) && (workspaceArtifact.size() > 0) &&
                    !workspaceArtifact.isEmpty()) {
                artifactGuid = workspaceArtifact.get(0);
            }

            List<String> workspaceBranch = artifact.getAttributes(
                    "ArtifactBranchGuid");
            String artifactBranchGuid = null;

            if ((workspaceBranch != null) && (workspaceBranch.size() > 0) &&
                    !workspaceBranch.isEmpty()) {
                artifactBranchGuid = workspaceBranch.get(0);
            }

            List<String> linkTaskInTaskPage = artifact.getAttributes(
                    "CreateTaskFromLinkGuid");
            String linkTaskGuid = null;

            if ((linkTaskInTaskPage != null) &&
                    (linkTaskInTaskPage.size() > 0) &&
                    !linkTaskInTaskPage.isEmpty()) {
            ...
            Date estimatedDate = null;
            String notify = "";
            String createdUserId = artifact.getAttributesOrElse(AtsAttributeTypes.CreatedBy.getId()
                                                                                           .toString())
                                           .get(0);
            List<String> assignees = new ArrayList<String>();
            List<String> listTemp = new ArrayList<String>();
            listTemp.add(new Date().toString());
            artifact.putAttributes(AtsAttributeTypes.EstimatedCompletionDate.getId()
                                                                            .toString() +
                ";Date", listTemp);

            List<String> listTemp1 = new ArrayList<String>();
            listTemp1.add(new Date().toString());
            artifact.putAttributes(AtsAttributeTypes.CreatedDate.getId()
                                                                .toString() +
                ";Date", listTemp1);
            childArtifact = createNewTask(tx, artifact, orcsApi, estimatedDate,
                    notify, createdUserId, assignees, productBacklog);
            childArtifact.getUuid();
            tx.setSoleAttributeFromString(childArtifact,
                AtsAttributeTypes.CurrentStateType, "Working");
            tx.setSoleAttributeFromString(childArtifact,
                AtsAttributeTypes.Information, "");
            tx.setSoleAttributeFromString(childArtifact,
                AtsAttributeTypes.Condition, "");
            tx.commit();

            if (artifactBranchGuid != null) {
                IOseeBranch branchNew = orcsApi.getQueryFactory().branchQuery()
                                               .getResultsAsId().getList()
                                               .get(Integer.valueOf(
                            artifactBranchGuid));
                ...
                ArtifactReadable wokrspaceArtifactReadableList = orcsApi.getQueryFactory()
                                                                        .fromBranch(branchNew)
                                                                        .andGuid(artifactGuid)
                                                                        .getResults()
                                                                        .getExactlyOne();
                ResultSet<?extends AttributeReadable<Object>> attributes2 = wokrspaceArtifactReadableList.getAttributes(AtsAttributeTypes.GUID);
                String taskGuid = ";";

                if ((attributes2.size() > 0) && !attributes2.isEmpty()) {
                    taskGuid = attributes2.getExactlyOne().getValue().toString() +
                        taskGuid;
                    taskGuid = taskGuid + childArtifact.getId();
                    workspaceTx.setSoleAttributeFromString(wokrspaceArtifactReadableList,
                        AtsAttributeTypes.GUID, taskGuid);
                } else {
                    workspaceTx.setSoleAttributeFromString(wokrspaceArtifactReadableList,
                        AtsAttributeTypes.GUID,
            ...
            if (linkTaskGuid != null) {
                ResultSet<ArtifactReadable> taskOneList = orcsApi.getQueryFactory()
                                                                 .fromBranch(CoreBranches.COMMON)
                                                                 .andIsOfType(AtsArtifactTypes.TeamWorkflow)
                                                                 .andGuid(linkTaskGuid)
                                                                 .getResults();
                ArtifactReadable taskOne = taskOneList.getExactlyOne();
                ResultSet<ArtifactReadable> taskTwoList = orcsApi.getQueryFactory()
                                                                 .fromBranch(CoreBranches.COMMON)
                                                                 .andIsOfType(AtsArtifactTypes.TeamWorkflow)
                                                                 .andUuid(childArtifact.getId())
                                                                 .getResults();
                ArtifactReadable taskTwo = taskTwoList.getExactlyOne();
                ArtifactReadable currentUser = CommonUtil.getCurrentUser(orcsApi,
                        artifact.getCurrentLoggedInUser());
                UserId userId = UserId.valueOf(currentUser.getId());
                TransactionBuilder linkTaskTx = txFactory.createTransaction(CoreBranches.COMMON,
                        userId, "Link Tasks From Task Page");
                linkTaskTx.relate(taskOne, AtsRelationTypes.TaskLink_From,
                    taskTwo);
                linkTaskTx.commit();
            }
            ...
            if (childArtifact != null) {
                ...
                String projectGuid = readableArtifact.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                                     .getExactlyOne().getGuid();
                System.out.println(projectGuid);

                String changes = String.format("New Task \"%s\" is created by %s on %s",
                        readableArtifact.getName(),
                        ICTeamMailNotifier.getUserNameByUserId(createdUserId),
                        DateUtil.getMMDDYY(new Date()));
                String guid = readableArtifact.getGuid();
                String rapLink = url + "/icteam-web/#/dashboard/" +
                    projectGuid + "/" + guid;
                System.out.println(rapLink);
                sendMail(orcsApi, createdUserId, assignees, readableArtifact,
                    estimatedDate, ICTeamNotifyType.Created, changes, rapLink);
     ...
     * @param guid        {@link String} Guid of the task to fecth the current state
     *                    string
     * @return assigneeList {@link List<ITransferableArtifact>} List of assigness
     *         for a task
     */
    public List<ITransferableArtifact> getAssigneesForCombo(
        String projectGuid, String guid) {
        try {
            OrcsApi orcsApi = OseeCoreData.getOrcsApi();
            ArtifactReadable teamWFArtifact = orcsApi.getQueryFactory()
                                                     .fromBranch(CoreBranches.COMMON)
                                                     .andUuid(Long.valueOf(guid)).getResults()
                                                     .getExactlyOne();
            ArtifactReadable projectArtifact = orcsApi.getQueryFactory()
                                                      .fromBranch(CoreBranches.COMMON)
                                                      .andUuid(Long.valueOf(projectGuid))
                                                      .getResults()
                                                      .getExactlyOne();
            AttributeReadable<Object> currentStateR = teamWFArtifact.getAttributes(AtsAttributeTypes.CurrentState)
                                                                    .getAtMostOneOrNull();
            String currentStateStr = (String) currentStateR.getValue();
            String[] split = currentStateStr.split(";");
            String currentState = split[0];
        ...
        try {
            ...
            for (ArtifactReadable artifactReadable : results) {
                if (artifactReadable.getGuid() != taskGuid) {
                    ...
                    ResultSet<ArtifactReadable> related = artifactReadable.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project);
                    ...
                    if ((related.size() > 0) && !related.isEmpty()) {
                        ...
                        if ((attributes.size() > 0) && !attributes.isEmpty()) {
                            ...
                            taskId = project.getAttributes(AtsAttributeTypes.Shortname)
                                            .getExactlyOne().toString() + "-" +
                                taskId;

                            String taskName = artifactReadable.getName();

                            if (idList.get(0).toString().equalsIgnoreCase(taskId) &&
                                    nameList.get(0).toString()
                                                .equalsIgnoreCase(taskName)) {
                                tx.relate(taskOne,
                                    AtsRelationTypes.TaskLink_From,
                                    artifactReadable);
                            }
    ...
    public String getTaskInfoForLinking(String json) {
        ...
        try {
            ...
            ResultSet<ArtifactReadable> projectReadable = orcsApi.getQueryFactory()
                                                                 .fromBranch(CoreBranches.COMMON)
                                                                 .andIsOfType(AtsArtifactTypes.AgileProject)
                                                                 .andNameEquals(projectFilter)
                                                                 .getResults();
            ResultSet<ArtifactReadable> taskList = orcsApi.getQueryFactory()
                                                          .fromBranch(CoreBranches.COMMON)
                                                          .andIsOfType(AtsArtifactTypes.TeamWorkflow)
                                                          .andRelatedTo(AtsRelationTypes.ProjectToTeamWorkFlow_Project,
            ...
            for (ArtifactReadable artifactReadable : taskList) {
                if (!(artifactReadable.getGuid().equals(taskGuid))) {
                    ...
                    if (typeFilter.equalsIgnoreCase(artifactReadable.getAttributes(
                                    AtsAttributeTypes.ChangeType).getExactlyOne()
                                                                        .toString())) {
                        TransferableArtifact trans = new TransferableArtifact();
                        ...
                        if (!linktask.isEmpty() && (linktask.size() > 0)) {
                            ...
                            String taskId = artifactReadable.getAttributes(AtsAttributeTypes.WorkPackage)
                                                            .getExactlyOne()
                                                            .toString();
                            taskId = projectReadable.getExactlyOne()
                                                    .getAttributes(AtsAttributeTypes.Shortname)
                                                    .getExactlyOne().toString() +
                                "-" + taskId;
    ...
    public String getAllProjectsForTaskLinking(String json) {
        ...
        try {
            ...
            ResultSet<ArtifactReadable> projects = orcsApi.getQueryFactory()
                                                          .fromBranch(CoreBranches.COMMON)
                                                          .andIsOfType(AtsArtifactTypes.AgileProject)
                                                          .getResults();
            ...
            for (ArtifactReadable project : projects) {
                TransferableArtifact ar = new TransferableArtifact();
                TranferableArtifactLoader.copyBasicInfoToTransferableArtifact(project,
                    ar);
     ...
     * @return json1 {@link String} contains the project name related to the given
     *         task
     */
    @PUT
    @Produces(MediaType.APPLICATION_JSON)
    ...
    public String getselectedProject(String json) {
        ...
        try {
            ...
            TransferableArtifact artifact = gson.fromJson(json,
                    TransferableArtifact.class);
            String taskGuid = artifact.getUuid();
            ResultSet<ArtifactReadable> task = orcsApi.getQueryFactory()
                                                      .fromBranch(CoreBranches.COMMON)
                                                      .andIsOfType(AtsArtifactTypes.TeamWorkflow)
                                                      .andUuid(Long.valueOf(
                        taskGuid)).getResults();
            String projectName = task.getExactlyOne()
                                     .getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
     ...
     *
     * @param json {@link String} contains the uuid of the task to fetch its linked
     *             tasks
        ...
        try {
            ...
            for (ArtifactReadable artifactReadable : task) {
                ...
                for (ArtifactReadable artifactReadable2 : related) {
                    ...
                    TranferableArtifactLoader.copyArtifactReadbleToTransferableArtifact(artifactReadable2,
                        ar);

                    ArtifactReadable project = artifactReadable2.getRelated(AtsRelationTypes.ProjectToTeamWorkFlow_Project)
                                                                .getExactlyOne();
                    String shortName = project.getAttributes(AtsAttributeTypes.Shortname)
                                              .getExactlyOne().toString();
                    String workpackage = artifactReadable2.getAttributes(AtsAttributeTypes.WorkPackage)
                                                          .getExactlyOne()
                                                          .toString();
                    String taskId = shortName + "-" + workpackage;
                    ar.putAttributes("TaskId", Arrays.asList(taskId));
    ...
    public String deleteLink(String json) {
        ...
        try {
            ...
            ResultSet<ArtifactReadable> task1 = orcsApi.getQueryFactory()
                                                       .fromBranch(CoreBranches.COMMON)
                                                       .andIsOfType(AtsArtifactTypes.TeamWorkflow)
                                                       .andUuid(Long.valueOf(
                        taskGuid)).getResults();
            ResultSet<ArtifactReadable> task2 = orcsApi.getQueryFactory()
                                                       .fromBranch(CoreBranches.COMMON)
                                                       .andIsOfType(AtsArtifactTypes.TeamWorkflow)
                                                       .andUuid(Long.valueOf(
                        secondTaskGuid)).getResults();
            TransactionFactory txFactory = orcsApi.getTransactionFactory();
            TransactionBuilder tx = txFactory.createTransaction(CoreBranches.COMMON,
                    UserId.valueOf(CommonUtil.getCurrentUser(orcsApi,
                            artifact.getCurrentLoggedInUser()).getId()),
                    "Unrelate Tasks");
            tx.unrelate(task1.getExactlyOne(),
                AtsRelationTypes.TaskLink_To, task2.getExactlyOne());
            tx.unrelate(task1.getExactlyOne(),
                AtsRelationTypes.TaskLink_From, task2.getExactlyOne());
            tx.commit();
        } catch (JsonSyntaxException e) {
            e.printStackTrace();
        } catch (OseeCoreException e) {
     ...
     * @throws OseeCoreException
     */
    private static String setAndVerifyRange(final int runningNumber)
        throws OseeCoreException {
