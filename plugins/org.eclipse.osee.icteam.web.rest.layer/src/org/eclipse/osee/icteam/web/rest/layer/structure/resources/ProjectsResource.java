...
public class ProjectsResource extends AbstractConfigResource{
        ...
        try {
            ...
            if (attribute != null) {
                ...
                for (ArtifactReadable artifactReadable : releases) {
                    ...
                    TranferableArtifactLoader.copyArtifactReadbleToTransferableArtifact(artifactReadable,
                        ar);
                    listTras.add(ar);

                    ResultSet<?extends AttributeReadable<Object>> attributes = artifactReadable.getAttributes(AtsAttributeTypes.BaselineBranchGuid);

                    if (attributes != null) {
                        for (AttributeReadable<Object> attributeReadable : attributes) {
                            String branchGuid = attributeReadable.getValue()
                                                                 .toString();
                            Branch andGuid = (Branch) OseeCoreData.getOrcsApi()
                                                                  .getQueryFactory()
                                                                  .branchQuery()
                                                                  .getResultsAsId()
                                                                  .getList()
                                                                  .get(Integer.valueOf(
                                        branchGuid));
                            ar.putAttributes("BaseLineBranchName",
                                Arrays.asList(andGuid.getName()));
                        }
                    }
                }
    ...
    public String getAssociatedComponentForProject(final String attribute) {
        ...
        try {
            ...
            if (attribute != null) {
                ArtifactReadable project = CommonUtil.getArtifactFromIdExcludingDeleted(attribute,
                        CommonUtil.getCommonBranch(orcsApi), orcsApi);
                ResultSet<ArtifactReadable> teamArtifacts = project.getRelated(AtsRelationTypes.ProjectToTeamDefinition_TeamDefinition);
                ...
                if (!listTeam.isEmpty()) {
                    ResultSet<ArtifactReadable> components = orcsApi.getQueryFactory()
                                                                    .fromBranch(CoreBranches.COMMON)
                                                                    .andIsOfType(AtsArtifactTypes.ActionableItem)
                                                                    .andRelatedTo(AtsRelationTypes.TeamActionableItem_TeamDefinition,
                            listTeam).getResults();
    ...
    /**
     * This function is used to create a new Project artifact with all the attribute
     * values sent from Client
     *
     ...
     *         duplicate project name and/or shortname present or else Failure
     *         status is returned
     */
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    ...
    public String createProject(final String json) {
        ...
        try {
            ...
            ResultSet<ArtifactReadable> results = orcsApi.getQueryFactory()
                                                         ...
                                                         .andNameEquals(artifact.getName())
                                                         .getResults();
            ResultSet<ArtifactReadable> shortnameResults = orcsApi.getQueryFactory()
                                                                  .fromBranch(CoreBranches.COMMON)
                                                                  .andIsOfType(AtsArtifactTypes.AgileProject)
                                                                  .getResults();
            ...
            for (ArtifactReadable artifactReadable : shortnameResults) {
                if (!artifactReadable.getAttributes(
                            AtsAttributeTypes.Shortname).isEmpty()) {
                    if (artifactReadable.getAttributes(
                                AtsAttributeTypes.Shortname)
                                            .getExactlyOne().toString()
                                            .equals(shortName.get(0))) {
                        status = false;
            ...
            if (results.isEmpty() && status) {
                if (AtsArtifactTypes.AgileProject.getName()
                                                             .equals(projectType)) {
                    childArtifact = tx.createArtifact(AtsArtifactTypes.AgileProject,
                            artifact.getName());
                } else {
                    childArtifact = tx.createArtifact(AtsArtifactTypes.Project,
                            artifact.getName());
                }
                    ...
                    if (!type.equalsIgnoreCase("ShortName")) {
                        for (String string : value) {
                            tx.setSoleAttributeFromString(childArtifact,
                                AttributeTypeToken.valueOf(Long.parseLong(type),
                                    "Attribute"), string);
                        }
                    } else {
                        tx.setSoleAttributeFromString(childArtifact,
                ...
                String type = CoreRelationTypes.Users_User.getRelationType()
                                                          .toString();
                ...
                for (ITransferableArtifact user : createdUserList) {
                    ResultSet<ArtifactReadable> list = orcsApi.getQueryFactory()
                                                              .fromBranch(CoreBranches.COMMON)
                                                              .andIsOfType(CoreArtifactTypes.User)
                                                              .andGuid(user.getUuid())
                                                              .getResults();

                    tx.relate(childArtifact, CoreRelationTypes.Users_User,
                        list.getExactlyOne());
                    tx.relate(childArtifact,
                        AtsRelationTypes.ProjectToUser,
                        list.getExactlyOne());
                }

                ResultSet<ArtifactReadable> result1 = orcsApi.getQueryFactory()
                                                             .fromBranch(CoreBranches.COMMON)
                ...
                if (!status) {
                    ar.putAttributes("Message",
                        Arrays.asList("ShortName already exists!!!!"));
                }

                if (!results.isEmpty()) {
                    ar.putAttributes("Message",
                        Arrays.asList("Project Name already exists!!!!"));
                }

                if (!status && !results.isEmpty()) {
                    ar.putAttributes("Message",
                        Arrays.asList(
                            "Project Name and Short Name already exists!!!!"));
                }
    ...
    public String getUseeSpecificProjects(final String uuid) {
        try {
            ...
            if (uuid != null) {
                ResultSet<ArtifactReadable> userList = orcsApi.getQueryFactory()
                                                              .fromBranch(CoreBranches.COMMON)
                                                              .andIsOfType(CoreArtifactTypes.User)
                                                              .andGuid(uuid)
                                                              .getResults();
                ...
                ResultSet<ArtifactReadable> projects = orcsApi.getQueryFactory()
                                                              .fromBranch(CoreBranches.COMMON)
                                                              .andIsOfType(AtsArtifactTypes.Project)
                                                              .getResults();
                ...
                for (ArtifactReadable project : projects) {
                    if (!project.getName().endsWith(CommonConstants.MANGLED_NAME)) {
                        ...
                        for (ArtifactReadable createdUser : createdUsers) {
                            if (createdUser.getUuid()
                                               .equals(LoggedInUser.getUuid())) {
                                addProject = true;
                            }
                        }
                        ...
                        if (addProject) {
                            TransferableArtifact ar = new TransferableArtifact();
                            TranferableArtifactLoader.copyAllInfoToTransferableArtifact(project,
                                ar);
        ...
        try {
            ...
            if (attribute != null) {
                ResultSet<ArtifactReadable> list = orcsApi.getQueryFactory()
                            ...
                            attribute)).getResults();
                List<ITransferableArtifact> listTras = new ArrayList<ITransferableArtifact>();

                for (ArtifactReadable artifactReadable : list) {
                    if (artifactReadable.getUuid().toString().equals(attribute)) {
                        TransferableArtifact ar = new TransferableArtifact();
                        TranferableArtifactLoader.copyProjectArtifactReadbleToTransferableArtifact(artifactReadable,
                            ar);
                        listTras.add(ar);
