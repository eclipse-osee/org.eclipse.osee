...
public class ReleasesResource extends AbstractConfigResource{
    ...
    public String getSprintByUuid(final String sprintUuid) {
        try {
            ...
            TransferableArtifact artifact = gson.fromJson(sprintUuid,
            ...
            if (artifact.getUuid() != null) {
                ResultSet<ArtifactReadable> list = orcsApi.getQueryFactory()
                                                          ...
                                                          .andIsOfType(AtsArtifactTypes.Version)
                                                          .andUuid(Long.valueOf(
                            artifact.getUuid())).getResults();
                ...
                for (ArtifactReadable artifactReadable : list) {
                    ...
                    ar.putAttributes(AtsAttributeTypes.Released.getName(),
                        Arrays.asList(artifactReadable.getSoleAttributeAsString(
    ...
    /**
     * This function is used to create a new Release artifact (Milestone or Sprint)
     * with all the attribute values sent from Client
     *
     * @param json String which contains artifact and its attributes values
     * @return json String with the created artifact,its attribute values if no
     *         duplicate release present or else Failure status is returned
     */
    @POST
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    ...
    public String createReleaseWeb(final String json) {
        try {
            ...
            ArtifactReadable artifactReadable1 = CommonUtil.getCurrentUser(orcsApi,
            ...
            TransactionBuilder tx = txFactory.createTransaction(CommonUtil.getCommonBranch(
                        orcsApi), userId, "Add New Release");
            ...
            if (parentGuid != null) {
                parentRelease = orcsApi.getQueryFactory()
                                       .fromBranch(CommonUtil.getCommonBranch())
                                       .andUuid(Long.valueOf(parentGuid))
                ...
                String uuid = AtsRelationTypes.ProjectToVersion_Project.getGuid()
                                                                      .toString();
                String type1 = AtsRelationTypes.ProjectToVersion_Project.getRelationType()
                                                                       .toString();
                String side = AtsRelationTypes.ProjectToVersion_Project.getSide()
                                                                      .toString();
                String key = "RelationTypeSide - uuid=[" + uuid + "] type=[" +
                    type1 + "] side=[" + side + "]";
                ITransferableArtifact project = artifact.getRelatedArtifacts(key)
                                                        .get(0);

                if (project != null) {
                    projectReadable = orcsApi.getQueryFactory()
                                             .fromBranch(CoreBranches.COMMON)
                                             .andUuid(Long.valueOf(
                                project.getUuid())).getResults();

                    ResultSet<ArtifactReadable> mileStoneList = projectReadable.getExactlyOne()
                                                                               .getRelated(AtsRelationTypes.ProjectToVersion_Version);

                    for (ArtifactReadable mileStone : mileStoneList) {
                        if (mileStone.getName()
                                         .equalsIgnoreCase(artifact.getName())) {
                            isMileStone = true;
                            status = false;
            ...
            if (status) {
                childArtifact = tx.createArtifact(AtsArtifactTypes.Version,
                        artifact.getName());
                ...
                if ((attributes != null) && (attributes.size() > 0)) {
                    ...
                    tx.setSoleAttributeValue(childArtifact,
                        AtsAttributeTypes.ReleaseDate, date);
                }
                ...
                if ((baselineBranchGuid != null) &&
                        (baselineBranchGuid.size() > 0)) {
                    tx.setSoleAttributeFromString(childArtifact,
                        AtsAttributeTypes.BaselineBranchGuid,
                ...
                if ((description != null) && (description.size() > 0)) {
                    tx.setSoleAttributeFromString(childArtifact,
                        AtsAttributeTypes.Description, description.get(0));
                }

                tx.setSoleAttributeFromString(childArtifact,
                    AtsAttributeTypes.Released, "false");
                tx.setSoleAttributeFromString(childArtifact,
                    AtsAttributeTypes.AllowCreateBranch, "true");
                tx.setSoleAttributeFromString(childArtifact,
                    AtsAttributeTypes.AllowCommitBranch, "true");

                if (parentGuid != null) {
                    tx.relate(parentRelease,
                        CoreRelationTypes.DefaultHierarchical_Child,
                        childArtifact);
                } else {
                    // Relation to Project and Team should be only for MileStone and
                    // Not Sprint
                    if ((projectReadable != null) &&
                            (projectReadable.size() == 1)) {
                        tx.relate(projectReadable.getExactlyOne(),
                        		AtsRelationTypes.ProjectToVersion_Project,
                            childArtifact);
                    }

                    String uuid = AtsRelationTypes.TeamDefinitionToVersion_TeamDefinition.getGuid()
                                                                                         .toString();
                    String type1 = AtsRelationTypes.TeamDefinitionToVersion_TeamDefinition.getRelationType()
                                                                                          .toString();
                    String side = AtsRelationTypes.TeamDefinitionToVersion_TeamDefinition.getSide()
                                                                                         .toString();
                    String key = "RelationTypeSide - uuid=[" + uuid +
                        "] type=[" + type1 + "] side=[" + side + "]";
                    ...
                    for (ITransferableArtifact transferableArtifact : relatedArtifacts) {
                        ResultSet<ArtifactReadable> result = orcsApi.getQueryFactory()
                                                                    .fromBranch(CoreBranches.COMMON)
                                                                    .andUuid(Long.valueOf(
                                    transferableArtifact.getUuid())).getResults();

                        if ((result != null) && (result.size() == 1)) {
                            tx.relate(result.getExactlyOne(),
            ...
            if (status) {
                ar.putAttributes(CommonConstants.STATUS,
                    Arrays.asList(CommonConstants.SUCCESS));
            ...
            if (!status) {
                ar.putAttributes(CommonConstants.STATUS,
                    Arrays.asList(CommonConstants.FAILURE));

                if (isMileStone) {
                    ar.putAttributes("Message",
                        Arrays.asList("Milestone name already exists!!!!"));
                } else {
                    ar.putAttributes("Message",
                        Arrays.asList(
                            "Sprint name already exists in this Milestone!!!!"));
                }
            }
                ...
                for (ArtifactReadable artifactReadable : list) {
                    if (artifactReadable.getIdString().equals(attribute)) {
                        ...
                        if ((name != null) && (name.size() > 0)) {
                            tx.setSoleAttributeFromString(artifactReadable,
                                CoreAttributeTypes.Name, name.get(0));
                        }
                        ...
                        if ((released != null) && (released.size() > 0)) {
                            tx.setSoleAttributeFromString(artifactReadable,
                                AtsAttributeTypes.Released, released.get(0));
                        }
    ...
    public String releaseReleaseWeb(final String json) {
        ...
        if (attributes != null) {
            try {
                ...
                if (version != null) {
                    ResultSet<ArtifactReadable> teamWorkFlows = orcsApi.getQueryFactory()
                                                                       .fromBranch(CoreBranches.COMMON)
                                                                       .andIsOfType(AtsArtifactTypes.TeamWorkflow)
                                                                       .andRelatedTo(AtsRelationTypes.TeamWorkflowTargetedForVersion_TeamWorkflow,
                            version).getResults();
                    ResultSet<ArtifactReadable> relatedTeamWF = version.getRelated(AtsRelationTypes.TeamWorkflowTargetedForVersion_TeamWorkflow);
    ...
    public String getAssociatedsCloseReleasesForProjectWeb(
        ...
        try {
            ...
            if (attribute != null) {
                ...
                for (ArtifactReadable artifactReadable : mileStones) {
                    ResultSet<ArtifactReadable> relatedSprints = artifactReadable.getRelated(CoreRelationTypes.DefaultHierarchical_Child);
                            ...
                            if ((releasedAttribute != null) &&
                                ...
                                for (AttributeReadable<Object> attributeReadable : releasedAttribute) {
                                    ...
                                    if (releasedAttributeString == true) {
                                        ...
                                        String displayName = artifactReadable.getName() +
                                            "-->" +
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Path("openReleaseWeb")
    public String getAssociatedOpenReleasesForProjectWeb(
        final String projectGuid) {
        try {
            Gson gson = new GsonBuilder().registerTypeAdapter(ITransferableArtifact.class,
                    new InterfaceAdapter<TransferableArtifact>()).create();
