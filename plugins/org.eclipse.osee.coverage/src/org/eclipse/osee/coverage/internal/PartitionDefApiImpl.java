 ...
 */
public class PartitionDefApiImpl implements PartitionDefApi {
   private ArtifactAccessor<PartitionDefToken> accessor;
   private final List<AttributeTypeId> attributes;
   public PartitionDefApiImpl(OrcsApi orcsApi) {
      this.setAccessor(new PartitionDefAccessor(orcsApi));
      attributes = new LinkedList<AttributeTypeId>();
      attributes.add(CoreAttributeTypes.Name);
   }

   private void setAccessor(ArtifactAccessor<PartitionDefToken> scriptDefTypeAccessor) {
      this.accessor = scriptDefTypeAccessor;
   }

   @Override
   public PartitionDefToken get(BranchId branch, ArtifactId scriptDefTypeId) {
      try {
         return this.accessor.get(branch, scriptDefTypeId);
      } catch (Exception ex) {
         return new PartitionDefToken(ex);
      }
   }

   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch) {
      return this.getAll(branch, ArtifactId.SENTINEL);
   }

   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch, ArtifactId viewId) {
   ...
   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch, AttributeTypeId orderByAttribute) {
      return this.getAll(branch, ArtifactId.SENTINEL, orderByAttribute);
   }

   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch, ArtifactId viewId, AttributeTypeId orderByAttribute) {
      return this.getAll(branch, viewId, new LinkedList<>(), 0L, 0L, orderByAttribute);
   }

   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch, ArtifactId viewId, List<FollowRelation> followRelations,
      AttributeTypeId orderByAttribute) {
      return this.getAll(branch, viewId, followRelations, 0L, 0L, orderByAttribute);
   }

   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch, long pageNum, long pageSize) {
      return this.getAll(branch, ArtifactId.SENTINEL, new LinkedList<>(), pageNum, pageSize, AttributeTypeId.SENTINEL);
   }

   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch, ArtifactId viewId, long pageNum, long pageSize) {
   ...
   public Collection<PartitionDefToken> getAll(BranchId branch, ArtifactId viewId, List<FollowRelation> followRelations,
      ...
      try {
         ...
         return new LinkedList<PartitionDefToken>();
      }
   }

   @Override
   public Collection<PartitionDefToken> getAll(BranchId branch, Collection<FollowRelation> followRelations,
   ...
   @Override
   public Collection<PartitionDefToken> getAllByFilter(BranchId branch, String filter) {
   ...
   @Override
   public Collection<PartitionDefToken> getAllByFilter(BranchId branch, ArtifactId viewId, String filter) {
   ...
   @Override
   public Collection<PartitionDefToken> getAllByFilter(BranchId branch, String filter, long pageNum, long pageSize) {
      return this.getAllByFilter(branch, ArtifactId.SENTINEL, filter, pageNum, pageSize);
   }

   @Override
   public Collection<PartitionDefToken> getAllByFilter(BranchId branch, ArtifactId viewId, String filter, long pageNum,
      long pageSize) {
   ...
   public Collection<PartitionDefToken> getAllByFilter(BranchId branch, String filter,
      ...
      try {
         ...
         Collection<PartitionDefToken> failedList = new LinkedList<>();
         failedList.add(new PartitionDefToken(ex));
         return failedList;
      }
   }
