 ...
 */
public class PartitionChartDataApiImpl implements PartitionChartDataApi {
   private ArtifactAccessor<PartitionChartDataToken> accessor;
   private final List<AttributeTypeId> attributes;
   public PartitionChartDataApiImpl(OrcsApi orcsApi) {
      this.setAccessor(new PartitionChartDataAccessor(orcsApi));
      attributes = new LinkedList<AttributeTypeId>();
      attributes.add(CoreAttributeTypes.Name);
   }

   private void setAccessor(ArtifactAccessor<PartitionChartDataToken> scriptDefTypeAccessor) {
      this.accessor = scriptDefTypeAccessor;
   }

   @Override
   public PartitionChartDataToken get(BranchId branch, ArtifactId scriptDefTypeId) {
      try {
         return this.accessor.get(branch, scriptDefTypeId);
      } catch (Exception ex) {
         return new PartitionChartDataToken(ex);
      }
   }

   @Override
   public Collection<PartitionChartDataToken> getAll(BranchId branch) {
      return this.getAll(branch, ArtifactId.SENTINEL);
   }

   @Override
   public Collection<PartitionChartDataToken> getAll(BranchId branch, ArtifactId viewId) {
   ...
   @Override
   public Collection<PartitionChartDataToken> getAll(BranchId branch, AttributeTypeId orderByAttribute) {
   ...
   @Override
   public Collection<PartitionChartDataToken> getAll(BranchId branch, long pageNum, long pageSize) {
      return this.getAll(branch, ArtifactId.SENTINEL, new LinkedList<>(), pageNum, pageSize, AttributeTypeId.SENTINEL);
   }

   @Override
   public Collection<PartitionChartDataToken> getAll(BranchId branch, ArtifactId viewId, long pageNum, long pageSize) {
   ...
   public Collection<PartitionChartDataToken> getAll(BranchId branch, ArtifactId viewId,
      ...
      try {
         ...
         return new LinkedList<PartitionChartDataToken>();
      }
   }

   @Override
   public Collection<PartitionChartDataToken> getAll(BranchId branch, Collection<FollowRelation> followRelations,
   ...
   @Override
   public Collection<PartitionChartDataToken> getAllByFilter(BranchId branch, String filter) {
   ...
   @Override
   public Collection<PartitionChartDataToken> getAllByFilter(BranchId branch, ArtifactId viewId, String filter) {
   ...
   @Override
   public Collection<PartitionChartDataToken> getAllByFilter(BranchId branch, String filter, long pageNum,
      long pageSize) {
   ...
   public Collection<PartitionChartDataToken> getAllByFilter(BranchId branch, String filter,
      ...
      try {
         ...
         Collection<PartitionChartDataToken> failedList = new LinkedList<>();
         failedList.add(new PartitionChartDataToken(ex));
         return failedList;
      }
   }
