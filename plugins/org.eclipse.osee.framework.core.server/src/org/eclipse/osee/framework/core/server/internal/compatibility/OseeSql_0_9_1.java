 ...
 *******************************************************************************/
package org.eclipse.osee.framework.core.server.internal.compatibility;
...
public enum OseeSql_0_9_1 {
   ...
   TX_GET_PREVIOUS_TX_NOT_CURRENT_ARTIFACTS("SELECT txs.transaction_id, txs.gamma_id FROM osee_artifact_version arv, osee_txs txs WHERE arv.art_id = ? AND arv.gamma_id = txs.gamma_id AND txs.branch_id = ? AND txs.tx_current <> " + TxChange.NOT_CURRENT.getValue()),
   ...
   MERGE_GET_ARTIFACTS_FOR_BRANCH("SELECT arv.art_id FROM osee_txs txs, osee_artifact_version arv WHERE txs.branch_id = ? and txs.gamma_id = arv.gamma_id"),
   ...
   IS_ARTIFACT_ON_BRANCH("SELECT%s count(1) from osee_artifact_version av1, osee_txs txs1 where av1.art_id = ? and av1.gamma_id = txs1.gamma_id and txs1.branch_id = ?", Strings.HINTS__ORDERED__FIRST_ROWS),
   ...
   private OseeSql_0_9_1(String sql, String hints) {
      this.sql = sql;
      this.hints = hints;
   }

   private OseeSql_0_9_1(String sql) {
      this(sql, null);
   }
