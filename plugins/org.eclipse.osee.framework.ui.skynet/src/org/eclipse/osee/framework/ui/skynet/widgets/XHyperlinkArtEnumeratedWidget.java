...
public class XHyperlinkArtEnumeratedWidget extends XAbstractHyperlinkLabelValueSelWidget {

   public static WidgetId ID = WidgetId.XHyperlinkArtEnumeratedWidget;

   protected List<String> checked = new ArrayList<>();

   public XHyperlinkArtEnumeratedWidget() {
      this(ID, "");
   }

   public XHyperlinkArtEnumeratedWidget(WidgetId widgetId, String label) {
   ...
   public boolean handleSelection() {
      try {
         String title =
            "Select " + (Strings.isValid(getLabel()) ? getLabel() : getAttributeType().getUnqualifiedName());
         ...
         if (selectable.isEmpty()) {
            AWorkbench.popupf("No [%s] options configured for this workflow", getLabel());
            return false;
         }
         boolean multiSelect = isMultiSelect();
         if (getArtifactType().isValid() && getAttributeType().isValid()) {
            multiSelect = getArtifactType().getMax(getAttributeType()) > 1;
         }
         if (multiSelect) {
