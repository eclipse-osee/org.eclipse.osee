...
public class CommitTransactions extends DatabaseHealthOperation {
   ...
   private static final String UPDATE_NEW_TRANSACTIONS_TO_CURRENT =
      "UPDATE osee_txs SET mod_type = 1 WHERE transaction_id = ? AND mod_type <> 1 AND gamma_id IN (SELECT tx1.gamma_id FROM osee_txs tx1, osee_arts av1 WHERE tx1.transaction_id = ? AND tx1.gamma_id = av1.gamma_id AND av1.art_id NOT IN (SELECT av2.art_id FROM osee_tx_details td2, osee_txs tx2, osee_arts av2 WHERE td2.osee_comment like '%Commit%' AND tx2.transaction_id < tx1.transaction_id AND tx2.transaction_id = tx2.transaction_id AND tx2.gamma_id = av2.gamma_id) AND av1.art_id NOT IN (SELECT av3.art_id FROM osee_txs tx3, osee_arts av3 WHERE tx3.branch_id = tx1.branch_id AND tx3.transaction_id < tx1.transaction_id AND tx3.mod_type = 1 AND tx3.gamma_id = av3.gamma_id))";
   private static final String DELETE_ORPHAN_ATTRIBUTES =
      "DELETE FROM osee_attribute WHERE gamma_id IN (SELECT t3.gamma_id FROM osee_txs t2, osee_attribute t3 WHERE t2.transaction_id = ? AND t2.gamma_id = t3.gamma_id AND t3.art_id NOT IN (SELECT art_id FROM osee_txs t4, osee_arts t5 WHERE t4.transaction_id = t2.transaction_id AND t4.gamma_id = t5.gamma_id))";
