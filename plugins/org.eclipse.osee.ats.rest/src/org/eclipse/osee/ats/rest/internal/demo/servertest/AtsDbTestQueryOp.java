...
public class AtsDbTestQueryOp {
   ...
   public XResultData run() {
      ...
      testTransactionUpdatesOnAttrAndRelChangeUsingAsArtifact();
      testTransactionUpdatesOnAttrAndRelChangeUsingGetArtifact();
   ...
   private void testTransactionUpdatesOnAttrAndRelChangeUsingAsArtifact() {
      ...
      ArtifactId testFolder =
         createTx.createArtifact(CoreArtifactTypes.Folder, "Tx Test Folder " + System.currentTimeMillis());
      ...
      ArtifactReadable loaded = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON).andId(
         testFolder).includeTransactionDetails().asArtifact();
      ...
      // Make sure created and loaded tx are equal
      rd.assertEquals(createTxResult.getId(), initialTx.getId());
      ...
      ArtifactReadable afterAttrLoad = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON).andId(
         testFolder).includeTransactionDetails().asArtifact();
      ...
      ArtifactReadable afterRelLoad = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON).andId(
         testFolder).includeTransactionDetails().asArtifact();
   ...
   private void testTransactionUpdatesOnAttrAndRelChangeUsingGetArtifact() {
      rd.log("testTransactionUpdatesOnAttrAndRelChangeGetArtifact - Start");

      // Create a folder artifact on Common
      TransactionBuilder createTx = orcsApi.getTransactionFactory().createTransaction(CoreBranches.COMMON,
         "Create test folder for tx getArtifact test");
      ArtifactId testFolder =
         createTx.createArtifact(CoreArtifactTypes.Folder, "Tx Test Folder GA " + System.currentTimeMillis());
      TransactionToken createTxResult = createTx.commit();
      if (!createTxResult.isValid()) {
         rd.errorf("testTransactionUpdatesGetArtifact - Create tx should be valid\n");
         return;
      }

      // Load artifact using getArtifact() and capture initial transaction
      ArtifactReadable loaded = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON).andId(
         testFolder).includeTransactionDetails().getArtifact();
      rd.assertNotNull(loaded, "testTransactionUpdatesGetArtifact - Loaded artifact should not be null");
      TransactionId initialTx = loaded.getLastModifiedTransaction();
      if (!initialTx.isValid()) {
         rd.errorf("testTransactionUpdatesGetArtifact - Initial tx should be valid\n");
      }
      ...
      // --- Attribute change: modify description, commit, reload, verify tx changes ---
      TransactionBuilder attrTx =
         orcsApi.getTransactionFactory().createTransaction(CoreBranches.COMMON, "Attr change for tx getArtifact test");
      attrTx.setSoleAttributeValue(testFolder, CoreAttributeTypes.Description,
         "Test description for tx getArtifact test");
      TransactionToken attrTxResult = attrTx.commit();
      if (!attrTxResult.isValid()) {
         rd.errorf("testTransactionUpdatesGetArtifact - Attr tx should be valid\n");
      }

      // Reload via getArtifact() and verify tx changed
      ArtifactReadable afterAttrLoad = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON).andId(
         testFolder).includeTransactionDetails().getArtifact();
      TransactionId afterAttrTx = afterAttrLoad.getLastModifiedTransaction();
      if (initialTx.equals(afterAttrTx)) {
         rd.errorf("testTransactionUpdatesGetArtifact - Tx should change after attr persist, was [%s]\n", initialTx);
      }

      // --- Relation change: relate as supporting info to OSEE Configuration folder ---
      TransactionBuilder relTx = orcsApi.getTransactionFactory().createTransaction(CoreBranches.COMMON,
         "Relation change for tx getArtifact test");
      relTx.relate(CoreArtifactTokens.OseeConfiguration, CoreRelationTypes.SupportingInfo_SupportingInfo, testFolder);
      TransactionToken relTxResult = relTx.commit();
      if (!relTxResult.isValid()) {
         rd.errorf("testTransactionUpdatesGetArtifact - Rel tx should be valid\n");
      }

      // Reload via getArtifact() and verify tx changed after relation-only change
      ArtifactReadable afterRelLoad = orcsApi.getQueryFactory().fromBranch(CoreBranches.COMMON).andId(
         testFolder).includeTransactionDetails().getArtifact();
      TransactionId afterRelTx = afterRelLoad.getLastModifiedTransaction();
      if (afterAttrTx.equals(afterRelTx)) {
         rd.errorf(
            "testTransactionUpdatesGetArtifact - Tx should change after relation persist, afterAttrTx [%s] afterRelTx [%s]\n",
            afterAttrTx, afterRelTx);
      }

      // Cleanup: delete the test artifact
      TransactionBuilder cleanupTx =
         orcsApi.getTransactionFactory().createTransaction(CoreBranches.COMMON, "Cleanup tx getArtifact test folder");
      cleanupTx.deleteArtifact(testFolder);
      cleanupTx.commit();

      rd.log("testTransactionUpdatesOnAttrAndRelChangeGetArtifact - End");
   }
