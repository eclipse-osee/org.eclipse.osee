...
public class AtsConfigurationsService implements IAtsConfigurationsService {
   ...
   private AtsConfigurations getAtsConfigurationsFromDb() {
      ...
      ResultSet<ArtifactReadable> artifacts =
         orcsApi.getQueryFactory().fromBranch(COMMON).andTypeEquals(Configuration).getResults();
      ...
      List<ArtifactReadable> configArts =
         orcsApi.getQueryFactory().fromBranch(atsApi.getAtsBranch()).andIsOfType(TeamDefinition, Version,
            ActionableItem).getResults().getList();
