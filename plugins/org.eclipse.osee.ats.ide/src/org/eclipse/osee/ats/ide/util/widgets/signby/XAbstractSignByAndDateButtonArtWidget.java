...
import org.eclipse.osee.framework.ui.skynet.widgets.XAbstractButtonWithLabelArtWidget;
 ...
 */
public abstract class XAbstractSignByAndDateButtonArtWidget extends XAbstractButtonWithLabelArtWidget {

   protected AttributeTypeToken dateAttrType;
   protected AttributeTypeToken byAttrType;

   public XAbstractSignByAndDateButtonArtWidget(WidgetId widgetId, AttributeTypeToken attrType1, AttributeTypeToken attrType2) {
      super(widgetId, (attrType1.isDate() ? attrType2.getUnqualifiedName() : attrType1.getUnqualifiedName()),
         "Sign or Clear", null);
   ...
   public String getResultsText() {
      ...
      if (date != null) {
         UserToken user = OseeApiService.userSvc().getUser(
            getArtifact().getSoleAttributeValue(byAttrType, SystemUser.UnAssigned.getId()));
         if (user != null) {
            ...
            protected IStatus run(IProgressMonitor monitor) {
               if (res == 2) {
                  if (userHasPermission()) {
                     setSigned(getArtifact(), dateAttrType, byAttrType, getLabel(), false);
                     refresh();
                  }
               } else if (res == 0) {
                  if (userHasPermission()) {
                     setSigned(getArtifact(), dateAttrType, byAttrType, getLabel(), true);
                     refresh();
                  }
               }
   ...
   protected XResultData isAuthorizedToChange() {
      ...
      if (rd.isSuccess()) {
         UserGroupAuthorization.hasUserGroupAuthorization(widData.getUserGroup(), getLabel(), rd);
      }
      if (rd.isSuccess()) {
         for (IAtsWorkItemHook wiHook : AtsApiService.get().getWorkItemService().getWorkItemHooks()) {
            wiHook.isModifiableAttribute(getArtifact(), getSignByAttrType(), rd);
         }
      }
      return rd;
   }
   ...
   public IStatus isValid() {
      if (isRequiredEntry() && getArtifact().getSoleAttributeValue(getSignDateAttrType(), null) == null) {
         Date date = getArtifact().getSoleAttributeValue(dateAttrType, null);
         if (date == null) {
   ...
   public void setUnsigned() {
      XAbstractSignByAndDateButtonArtWidget.setSigned(getArtifact(), dateAttrType, byAttrType, getLabel(), false);
   }

   public void setSigned() {
      XAbstractSignByAndDateButtonArtWidget.setSigned(getArtifact(), dateAttrType, byAttrType, getLabel(), true);
   }
   ...
   protected void createControls(Composite parent, int horizontalSpan) {
      ...
      if (getOseeImage() != null) {
         ...
         getbutton().setImage(getImageOrDefault().createImage());
      }
   }

   @Override
   protected ImageDescriptor getImageOrDefault() {
      return super.getImageOrDefault();
   }
