...
public class XXTargetedVersionWidget extends XAbstractXXVersionWidget {

   public static final WidgetId ID = WidgetIdAts.XXTargetedVersionWidget;

   public XXTargetedVersionWidget() {
      super(ID, "Targeted Version", AtsRelationTypes.TeamWorkflowTargetedForVersion_Version);
   }

   @Override
   public void setWidData(XWidgetData widData) {
      super.setWidData(widData);
      widData.setOseeImage(AtsImage.VERSION);
   }
   ...
   public void refresh() {
      ...
      if (Widgets.isAccessible(labelHyperlink) && xxWid.isTeamWf()) {
         ...
         if (teamWf != null && teamWf.getWorkDefinition().hasOption(WorkDefOption.RequireTargetedVersion)) {
