...
import org.eclipse.osee.framework.ui.skynet.widgets.XHyperlinkArtEnumeratedWidget;
import org.eclipse.osee.framework.ui.skynet.widgets.XHyperlinkLabelDateWidget;
import org.eclipse.osee.framework.ui.skynet.widgets.XHyperlinkTriStateBooleanWidget;
 ...
 */
public abstract class AbstractCreateNewChangeRequestBlam extends AbstractAtsBlam implements ISelectableValueProvider {
   ...
   protected XHyperlinkArtEnumeratedWidget cogPriorityWidget;
   protected final AtsApi atsApi;
   protected XWidgetBuilder wb;
   protected XHyperlinkWfdForProgramAiWidget programWidget;
   private XHyperlinkTriStateBooleanWidget crashWidget;
   private XHyperlinkLabelDateWidget needByWidget;
   private String overrideTitle;
   private NewActionData data;
   private NewActionData newData;

   public AbstractCreateNewChangeRequestBlam(String name) {
   ...
   public List<XWidgetData> getXWidgetItems() {
      ...
      wb.andWidget(PROGRAM, XHyperlinkWfdForProgramAiWidget.class.getSimpleName()).andValueProvider(
      ...
      // 6 columns
      wba.andChangeType().andComposite(6).andRequired().endWidget();
      if (includePriority()) {
         wba.andPriority().andComposite(6).andRequired().endWidget();
      }
   ...
   public void widgetCreated(XWidget xWidget, FormToolkit toolkit, Artifact art, XWidgetSwtRenderer swtXWidgetRenderer,
      ...
      if (xWidget.getLabel().equals(TITLE)) {
         ...
         cogPriorityWidget = (XHyperlinkArtEnumeratedWidget) xWidget;
      } else if (xWidget.getLabel().equals(NEED_BY)) {
         needByWidget = (XHyperlinkLabelDateWidget) xWidget;
      } else if (xWidget.getLabel().equals(AtsAttributeTypes.CrashOrBlankDisplay.getUnqualifiedName())) {
         crashWidget = (XHyperlinkTriStateBooleanWidget) xWidget;
      } else if (xWidget.getLabel().equals(PROGRAM)) {

         if (xWidget instanceof XHyperlinkWfdForProgramAiWidget) {
            programWidget = (XHyperlinkWfdForProgramAiWidget) xWidget;
   ...
   public void runOperation(VariableMap variableMap, IProgressMonitor monitor) throws Exception {
      ...
      if (priorityWidget != null) {
         priority = priorityWidget.getSelectedFirst();
